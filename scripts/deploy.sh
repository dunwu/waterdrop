...
if [[ ${GITHUB_TOKEN} && ${GITEE_TOKEN} ]]; then
  msg='自动部署'
  GITHUB_URL=https://dunwu:${GITHUB_TOKEN}@github.com/dunwu/waterdrop.git
  GITEE_URL=https://turnon:${GITEE_TOKEN}@gitee.com/turnon/waterdrop.git
  git config --global user.name "dunwu"
  git config --global user.email "forbreak@163.com"
else
  msg='手动部署'
  GITHUB_URL=git@github.com:dunwu/waterdrop.git
  GITEE_URL=git@gitee.com:turnon/waterdrop.git
fi
git init
git add -A
git commit -m "${msg}"
