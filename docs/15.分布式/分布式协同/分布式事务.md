...
![](https://raw.githubusercontent.com/dunwu/images/master/archive/2024/12/17afd0b535254f879029bcb19e856bb2.png)
...
![](https://raw.githubusercontent.com/dunwu/images/master/archive/2024/12/4d3b1a8fa69145cf812feb187b49d5d6.png)

上图中包含了库存和订单两个独立的微服务，每个微服务维护了自己的数据库。在交易系统的业务逻辑中，一个商品在下单之前需要先调用库存服务，进行扣除库存，再调用订单服务，创建订单记录。

![](https://raw.githubusercontent.com/dunwu/images/master/archive/2024/12/9223ed8c58564133bf4dfee968798f15.png)
...
![](https://raw.githubusercontent.com/dunwu/images/master/archive/2023/10/03200c03078941cdaa148d3dfc516771.png)

CAP 就是取 Consistency、Availability、Partition Tolerance 的首字母而命名。

![](https://raw.githubusercontent.com/dunwu/images/master/archive/2021/11/4ee88ac0d2ac4548b13bfe907e8af203.png)
...
7. 在断网或者是生产者应用重启的特殊情况下，若服务端未收到发送者提交的二次确认结果，或服务端收到的二次确认结果为 Unknown 未知状态，经过固定时间后，服务端将对消息生产者即生产者集群中任一生产者实例发起消息回查。 **说明** 服务端回查的间隔时间和最大回查次数，请参见 [参数限制](https://rocketmq.apache.org/zh/docs/introduction/03limits)。
8. 生产者收到消息回查后，需要检查对应消息的本地事务执行的最终结果。
9. 生产者根据检查到的本地事务的最终状态再次提交二次确认，服务端仍按照步骤 4 对半事务消息进行处理。

**事务消息生命周期**

![事务消息](https://raw.githubusercontent.com/dunwu/images/master/archive/2026/02/0806b0484be843fa9df5b42d6fce849d.png)
...
分布式事务在微服务架构中广泛存在，以下是一些典型应用场景：

- **电商下单** - 下单流程涉及订单服务（创建订单）、库存服务（扣减库存）、优惠券服务（核销优惠券）、支付服务（支付扣款），这些服务通常有独立的数据库，需要保证数据一致性。
- **转账汇款** - 银行或支付系统中，A 账户扣款与 B 账户加款可能分布在不同的服务节点，必须保证要么都成功、要么都失败，绝不能出现"钱扣了却没到账"的情况。
- **库存超卖防护** - 秒杀场景下，扣减库存与生成订单需要原子性，否则可能出现超卖。
- **跨服务数据同步** - 用户注册后需要同步创建用户画像、积分账户、消息账号等，这些操作跨多个服务，需要保证最终一致性。
- **订单状态流转** - 订单从"待支付"到"已支付"再到"已发货"，涉及支付、物流、通知等多个服务的状态联动。
- **积分 / 优惠券发放** - 支付成功后发放积分、优惠券，若发放失败需要重试或补偿。

### 方案选型矩阵

| 场景特点 | 推荐方案 | 理由 |
| -------- | -------- | ---- |
| 强一致性要求、低并发 | 2PC / XA | 强一致，但性能差 |
| 金融转账、实时性高 | TCC | 准实时、最终一致、可补偿 |
| 长流程、多步骤业务 | SAGA | 适合长活事务，补偿灵活 |
| 异步解耦、最终一致 | 本地消息表 / MQ 事务 | 性能高、耦合低 |
| 跨库同进程 | Seata AT | 无侵入、自动补偿 |

## 最佳实践

### 案例 1：基于 Seata AT 模式的电商下单

Seata 是阿里开源的分布式事务解决方案，AT（Auto Transaction）模式对业务零侵入，自动生成回滚 SQL，适合 CRUD 类业务。
...
（3）业务代码 - 使用 `@GlobalTransactional` 标注全局事务入口
...
public class OrderService {
    ...
    public boolean createOrder(OrderDTO dto) {
        // 1. 扣减库存（远程调用库存服务）
        boolean stockResult = stockFeignClient.deduct(dto.getProductId(), dto.getCount());
        if (!stockResult) {
            throw new RuntimeException("库存扣减失败");
        }

        // 2. 扣减账户余额（远程调用账户服务）
        boolean accountResult = accountFeignClient.debit(dto.getUserId(), dto.getMoney());
        if (!accountResult) {
...
说明：`@GlobalTransactional` 由 Seata TC（事务协调者）协调，每个分支事务自动生成 `undo_log` 用于回滚。远程调用的服务需同样配置 Seata 并注册到同一 TC。

### 案例 2：基于 RocketMQ 事务消息的积分发放

支付成功后异步发放积分，使用 RocketMQ 事务消息保证"本地事务"与"消息发送"的最终一致性。
...
rocketmq:
  ...
  producer:
    group: points-producer-group
    send-message-timeout: 3000
```

（3）事务消息生产者 - 实现 `TransactionListener` 接口执行本地事务并回查
...
public class PointsTransactionListener implements RocketMQLocalTransactionListener {
    ...
    @Resource
    private PointsMessageLogService pointsMessageLogService;

    /**
     * 执行本地事务：更新支付状态为已支付，并记录消息日志
     */
    @Override
    public RocketMQLocalTransactionState executeLocalTransaction(Message msg, Object arg) {
        ...
        try {
            // 本地事务：1. 更新支付状态 2. 记录事务消息日志
            paymentService.markPaid(orderId, txId);
            pointsMessageLogService.save(txId, orderId, "SENT");
    ...
    /**
     * 事务回查：根据事务日志判断本地事务是否成功
     */
    @Override
    public RocketMQLocalTransactionState checkLocalTransaction(Message msg) {
        String txId = (String) msg.getHeaders().get(RocketMQHeaders.TRANSACTION_ID);
        PointsMessageLog log = pointsMessageLogService.findByTxId(txId);
        if (log != null && "SENT".equals(log.getStatus())) {
            return RocketMQLocalTransactionState.COMMIT;
        }
        // 如果支付已成功但日志不存在，也要提交
        if (paymentService.isPaidByTxId(txId)) {
            return RocketMQLocalTransactionState.COMMIT;
        }
        return RocketMQLocalTransactionState.ROLLBACK;
    }
...
public class PointsConsumer implements RocketMQListener<String> {
    ...
    public void onMessage(String message) {
        ...
        pointsService.addPoints(userId, points, orderId); // 需保证幂等
    }
}
```

说明：RocketMQ 事务消息通过 half 消息 + 本地事务 + 回查机制，保证本地事务与消息发送的最终一致性，避免了"本地成功消息丢失"或"本地失败消息已发"的问题。

### 案例 3：基于本地消息表 + 定时任务的订单通知

当不使用支持事务消息的 MQ（如使用 Kafka 早期版本）时，可采用本地消息表方案。

（1）建表

```sql
-- 本地消息表，与业务表在同一数据库，利用本地事务保证一致性
...
public class OrderNotificationService {
    ...
    public OrderNotificationService(OrderMapper orderMapper,
    ...
    public void createOrderWithMessage(Order order) {
        ...
        msg.setBizKey(order.getOrderNo());
        msg.setTopic("order-created");
        msg.setTag("notify");
        msg.setBody(toJson(order));
        msg.setStatus(0);
        msg.setRetryCount(0);
    ...
    private String toJson(Order order) {
        // 简化示例，实际用 Jackson / Fastjson
        return String.format("{\"orderNo\":\"%s\",\"userId\":%d,\"amount\":%s}",
                order.getOrderNo(), order.getUserId(), order.getAmount().toPlainString());
    }
}
```
...
public class MessageSendScheduler {
    ...
    public void sendPendingMessages() {
        ...
        for (LocalMessage msg : list) {
            try {
                kafkaTemplate.send(msg.getTopic(), msg.getBizKey(), msg.getBody()).get();
                ...
                if (retry >= 5) {
                    ...
                    messageMapper.incrRetry(msg.getId(), retry,
                            LocalDateTime.now().plusSeconds(retry * 30L));
                }
            }
        }
    }
...
说明：本地消息表方案将消息持久化在业务库，利用本地事务保证业务与消息的一致性；通过定时任务扫描重试保证最终投递。消费端需做幂等处理。

## 常见问题

### 问题 1：TCC 事务中 Confirm/Cancel 未幂等导致数据错乱

**问题描述**：某系统采用 TCC 模式，由于网络抖动，TCC 框架对 `Confirm` 操作进行了重试，导致库存被扣减了两次，出现超卖。

**原因分析**：TCC 的 `Confirm` 和 `Cancel` 操作必须保证幂等性。当 TCC 协调者未收到分支的 `Confirm` 响应时，会进行重试。如果 `Confirm` 操作不幂等（如每次都执行 `UPDATE stock = stock - ?`），重试会导致重复扣减。

**解决方案**：在 `Try` 阶段记录事务分支状态，`Confirm`/`Cancel` 执行前检查状态，已执行则直接返回成功。
...
public class StockTccActionImpl implements StockTccAction {
    ...
    public StockTccActionImpl(StockMapper stockMapper,
                              TccTransactionLogMapper tccLogMapper) {
    ...
    @Override
    @Transactional(rollbackFor = Exception.class)
    public boolean tryDeduct(BusinessActionContext ctx, String productId, int count) {
        // 幂等检查：防止 Try 重复执行
        if (tccLogMapper.exists(ctx.getXid(), ctx.getBranchId(), "TRY")) {
            return true;
        }
        ...
        tccLogMapper.insert(ctx.getXid(), ctx.getBranchId(), "TRY");
        return true;
    }
    ...
    public boolean confirm(BusinessActionContext ctx) {
        // 幂等检查
        if (tccLogMapper.exists(ctx.getXid(), ctx.getBranchId(), "CONFIRM")) {
            return true;
        }
        ...
        tccLogMapper.insert(ctx.getXid(), ctx.getBranchId(), "CONFIRM");
        return true;
    }
    ...
    public boolean cancel(BusinessActionContext ctx) {
        // 幂等检查
        if (tccLogMapper.exists(ctx.getXid(), ctx.getBranchId(), "CANCEL")) {
            return true;
        }
        // 空回滚检查：Try 未执行时，Cancel 也不应执行业务
        if (!tccLogMapper.exists(ctx.getXid(), ctx.getBranchId(), "TRY")) {
            tccLogMapper.insert(ctx.getXid(), ctx.getBranchId(), "CANCEL");
            return true;
        }
        String productId = (String) ctx.getActionContext("productId");
        int count = (int) ctx.getActionContext("count");
        // 解冻：frozen -= count, available += count
...
说明：该实现同时处理了三个 TCC 常见问题：
1. **幂等性**：通过 `tcc_transaction_log` 记录每个分支的 `TRY`/`CONFIRM`/`CANCEL` 状态，重复执行直接返回。
2. **空回滚**：`Cancel` 时若 `Try` 未执行，记录日志后直接返回，不执行解冻。
3. **悬挂**：若 `Cancel` 已先于 `Try` 执行（网络原因），`Try` 执行前应检查 `Cancel` 是否已执行，避免冻结后无法解冻。

### 问题 2：SAGA 事务补偿顺序错误导致数据不一致

**问题描述**：某系统采用 SAGA 模式，订单流程包含 `T1 扣减库存 -> T2 创建订单 -> T3 支付`。当 T3 支付失败时，补偿顺序错误地先执行了 `C1 恢复库存` 再执行 `C2 取消订单`，导致在恢复库存的瞬间，已取消的订单仍可见，出现数据短暂不一致。

**原因分析**：SAGA 的补偿必须严格按照正向执行的**逆序**进行，即 `T1, T2, ..., Tj(失败), Cj, ..., C2, C1`。若补偿顺序错误，可能在补偿过程中产生中间不一致状态。

**解决方案**：SAGA 协调器必须维护正向执行的子事务栈，失败时按 LIFO（后进先出）顺序补偿。
...
public class SagaCoordinator {
     ...
     * 正向执行子事务
     */
    public void execute(SagaStep[] steps) {
        for (SagaStep step : steps) {
            try {
                ...
                executedSteps.push(step); // 成功后入栈
            } catch (Exception e) {
                compensate();
                throw new RuntimeException("SAGA 事务失败，已补偿", e);
            }
        }
    }

    /**
     * 逆序补偿：按 LIFO 顺序执行已成功步骤的补偿
     */
    private void compensate() {
        while (!executedSteps.isEmpty()) {
            ...
            try {
                ...
                // 补偿失败，重试或记录待人工处理
                // 实际项目中应记录到补偿表，由定时任务重试
                step.markCompensateFailed(e);
            }
        }
    }
    ...
    public interface SagaStep {
        boolean execute();
        void compensate();
        void markCompensateFailed(Exception e);
    }
}
```

说明：使用 `Deque`（双端队列）作为栈，`execute` 成功后 `push` 入栈，`compensate` 时 `pop` 出栈，天然保证 LIFO 逆序补偿。补偿失败时应记录到持久化存储，由后台任务重试，保证最终一致。

### 问题 3：2PC 协调者宕机导致参与者资源锁定

**问题描述**：系统使用 XA 协议（2PC）进行分布式事务，协调者在阶段 1 完成后、阶段 2 下发前宕机，参与者一直处于 `PREPARED` 状态，持有的数据库锁长时间不释放，导致其他事务阻塞。

**原因分析**：2PC 的协调者是单点。当协调者在阶段 1 与阶段 2 之间宕机，参与者无法获知应该 `COMMIT` 还是 `ROLLBACK`，只能持续等待，期间持有资源锁。若协调者无法恢复，参与者将永久阻塞。

**解决方案**：

1. **协调者高可用**：协调者本身做集群部署，主节点宕机后从节点接管，并通过事务日志恢复未决事务。
2. **引入 3PC**：3PC 在参与者引入超时机制，超时后自动提交（但仍有数据不一致风险）。
3. **XA 事务超时**：数据库层面设置 XA 事务超时，超时后自动回滚。

下面演示 MySQL XA 事务超时与手动处理：

```sql
-- 1. 设置 XA 事务超时为 30 秒
XA START 'xid_order_pay';
SET @@session.xa_trust_protocol = ON;

-- 2. 执行分支事务 SQL
UPDATE account SET balance = balance - 100 WHERE user_id = 1;

-- 3. 结束 XA 事务并 PREPARE
XA END 'xid_order_pay';
XA PREPARE 'xid_order_pay';

-- 4. 若协调者宕机，可通过以下命令查看悬挂的 XA 事务
XA RECOVER;

-- 5. 根据业务决策，手动提交或回滚
-- XA COMMIT 'xid_order_pay';
-- XA ROLLBACK 'xid_order_pay';
```
...
public class JtaOrderService {
    ...
    public void createOrderWithTimeout() throws Exception {
        UserTransaction utx = tm.getUserTransaction();
        utx.setTransactionTimeout(30); // 30 秒超时
        utx.begin();
        try {
            // 业务操作...
            utx.commit();
...
说明：通过 `setTransactionTimeout` 设置全局事务超时，超时后事务管理器自动回滚，避免参与者长时间锁定资源。生产环境中应配合协调者集群（如 Atomikos / Narayana）保证协调者高可用。
