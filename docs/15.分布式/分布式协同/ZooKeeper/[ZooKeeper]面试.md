...
ZAB 是 ZooKeeper 专门设计的**支持故障恢复的原子广播协议**，定义了两个可以**无限循环**的流程：**选主**（故障恢复，保高可用）和**原子广播**（主从同步，保数据一致性）。所有写请求都转发给 Leader，半数以上 Follower 持久化成功后才提交，类似两阶段提交。

#### ⚡记忆卡片

- **口诀**：选主管保命，广播管一致；写走 Leader，过半 ACK 才提交
- **关键词**：原子广播 ／ 选主 ／ ZXID ／ Leader ／ Follower ／ 半数提交
- **链路**：Leader 故障 → 基于过半选举机制选新 Leader → 其他节点从新 Leader 同步状态 → 过半同步完成后退出选举模式进入原子广播 → 写请求经 Leader 生成 Proposal（带 ZXID）广播 → 半数以上 Follower 持久化成功 → Leader 提交并响应客户端

#### 📖 核心知识

**不是 Paxos**：ZooKeeper 并没有直接采用 Paxos 算法，而是采用了名为 ZAB 的一致性协议。**ZAB 协议不是 Paxos 算法**，只是比较类似，二者在操作上并不相同。Multi-Paxos 实现的是一系列值的共识，不关心最终达成共识的值是什么，不关心各值的顺序。而 ZooKeeper 需要确保操作的顺序性。
...
**选主**：ZooKeeper 集群采用一主（称为 Leader）多从（称为 Follower）模式，主从节点通过副本机制保证数据一致。
...
**原子广播**：ZooKeeper 通过副本机制来实现高可用，而实现副本机制的答案就是 ZAB 协议的原子广播。

![](https://raw.githubusercontent.com/dunwu/images/master/cs/java/javaweb/distributed/rpc/zookeeper/zookeeper_3.png)

ZAB 协议的原子广播要求：

_**所有的写请求都会被转发给 Leader，Leader 会以原子广播的方式通知 Follow。当半数以上的 Follow 已经更新状态持久化后，Leader 才会提交这个更新，然后客户端才会收到一个更新成功的响应**_。这有些类似数据库中的两阶段提交协议。
...
ZXID 是 64 位事务 ID，高 32 位为 epoch（Leader 任期编号），低 32 位为该任期内的递增计数器。选举时比较 ZXID 就能选出数据最新的节点，保证新 Leader 拥有最新的事务历史，与 Raft 比较 (term, index) 的选举限制思想同源。
...
> 📚 延伸阅读：[A Simple Totally Ordered Broadcast Protocol](https://diyhpl.us/~bryan/papers2/distributed/distributed-systems/zab.totally-ordered-broadcast-protocol.2008.pdf)（概述 ZooKeeper 的全序广播协议 Zab）
> 📚 延伸阅读：[ZAB 协议](https://dunwu.github.io/waterdrop/pages/6f4382d3/)
> 📚 延伸阅读：[详解分布式协调服务 ZooKeeper](https://draveness.me/zookeeper-chubby)

#### 🔀 发散问题

**Q1：ZAB 和两阶段提交（2PC）有什么区别？**
ZAB 的提交流程形似 2PC，但关键差异在容错：2PC 协调者单点故障会阻塞全部参与者；ZAB 依赖多数派，少数 Follower 故障不影响提交，Leader 故障后可重新选主继续服务。

**Q2：ZAB 和 Raft 是什么关系？**
两者都是强 Leader + 多数派提交的日志复制协议，思想同源；区别在于 ZAB 专为 ZooKeeper 的主备原子广播设计（强调全序广播与故障恢复），Raft 是通用、定义更完整的共识算法。详细对比见《分布式理论面试》『Paxos、Raft、ZAB 有什么区别与联系？』。

**Q3：Leader 宕机后的恢复流程是怎样的？**
基于过半选举机制选出新 Leader，过半机器完成状态同步后退出选举进入广播模式。详见本文档『Zab 协议中故障恢复的流程是怎样的？』。

**Q4：正常写入时数据如何同步？**
走原子广播：Leader 生成 Proposal 广播，过半持久化后提交。详见本文档『Zab 协议中原子广播的流程是怎样的？』。
