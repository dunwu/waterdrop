...
### 案例一：基于 Curator 实现分布式锁

**场景说明**：在分布式系统中，多个服务节点需要互斥地访问共享资源（如库存扣减、订单生成）。利用 ZooKeeper 的临时顺序节点 + Watcher 机制可以优雅地实现公平分布式锁。
    ...
    public static void main(String[] args) throws Exception {
        ...
        try {
            ...
            if (lock.acquire(3, java.util.concurrent.TimeUnit.SECONDS)) {
                try {
                    System.out.println(Thread.currentThread().getName() + " 成功获取到分布式锁，开始执行业务逻辑");
                    // 模拟业务操作：扣减库存
                    deductStock();
                } finally {
                    // 4. 业务执行完毕，释放锁
...
- `InterProcessMutex` 是 Curator 提供的可重入分布式锁实现，内部基于临时顺序节点和 Watcher 机制。
- 获取锁的节点会监听前一个节点，形成公平锁队列，避免"羊群效应"。
- 会话断开时临时节点自动删除，避免死锁。

### 案例二：基于 ZooKeeper 实现配置中心

**场景说明**：微服务集群中，多个服务实例共享同一份动态配置（如数据库连接信息、限流阈值）。通过 ZooKeeper 的 Watcher 机制，配置变更时实时推送到所有客户端。
...
public class ConfigCenterDemo implements Watcher {
    ...
    public static void main(String[] args) throws Exception {
        // 1. 连接 ZooKeeper
        zk = new ZooKeeper(ZK_ADDRESS, 5000, new ConfigCenterDemo());
        connectedLatch.await();

        // 2. 初始化配置节点（如果不存在）
        Stat stat = zk.exists(CONFIG_PATH, false);
        if (stat == null) {
            zk.create(CONFIG_PATH, "jdbc:mysql://localhost:3306/db".getBytes(StandardCharsets.UTF_8),
     ...
     * 读取配置并注册 Watcher
     */
    private void readConfig() throws KeeperException, InterruptedException {
        byte[] data = zk.getData(CONFIG_PATH, this, null);
        System.out.println("当前配置：" + new String(data, StandardCharsets.UTF_8));
    }
    ...
    public void process(WatchedEvent event) {
        ...
        if (event.getType() == Event.EventType.NodeDataChanged) {
            System.out.println("检测到配置变更，正在重新加载...");
            try {
                readConfig();
            } catch (Exception e) {
                e.printStackTrace();
...
- `getData` 注册的 Watcher 是一次性的，触发后需要在回调中重新注册。
- 配置节点使用 `PERSISTENT` 类型，避免会话结束导致配置丢失。
- 生产环境建议使用 Curator 的 `Cache` 机制（如 `CuratorCache`）简化监听管理。

### 案例三：基于 ZooKeeper 实现 Master 选举

**场景说明**：主从架构的分布式系统（如定时任务调度）需要选举出唯一 Master 执行核心逻辑，Master 宕机时自动切换。利用 ZooKeeper 的临时节点唯一性可轻松实现。
...
public class MasterElectionDemo {
    ...
    private static final String MASTER_PATH = "/elections/master";
    private static final String ZK_ADDRESS = "localhost:2181";
    ...
    public static void main(String[] args) {
        ...
        LeaderSelectorListener listener = new LeaderSelectorListener() {
            ...
            public void takeLeadership(CuratorFramework client) throws Exception {
                // 只有成为 Master 的节点才会执行此方法
                System.out.println("节点 [" + Thread.currentThread().getName() + "] 成为 Master，开始执行任务");
                try {
                    // 模拟 Master 执行核心业务
                    Thread.sleep(10000);
                } finally {
                    // 方法返回后即放弃 Master 身份
                    System.out.println("节点 [" + Thread.currentThread().getName() + "] 释放 Master 身份");
                }
            }
        ...
        // 启动选举
        LeaderSelector selector = new LeaderSelector(client, MASTER_PATH, listener);
        selector.autoRequeue(); // 放弃 Master 后自动重新排队参与下一轮选举
        selector.start();
...
- `LeaderSelector` 基于 ZooKeeper 临时节点实现，Master 宕机会话失效后节点自动删除，触发重新选举。
- `autoRequeue()` 让放弃 Master 身份的节点自动重新排队，适合轮询场景。
- 若需要"一旦成为 Master 就一直担任"的语义，可使用 `LeaderLatch`。

## 常见问题

### 问题一：Watcher 监听丢失导致业务感知不到数据变更

**问题描述**：客户端通过 `getData` 注册 Watcher 监听节点变化，但偶尔出现节点数据已变更而客户端未收到通知的情况，导致业务逻辑异常。

**原因分析**：

ZooKeeper 原生的 Watcher 是**一次性**的。客户端收到一次通知后，Watcher 即被销毁。如果在客户端重新注册 Watcher 之前（存在短暂时间窗口），节点又发生了变更，这期间的变化就会丢失。另外，客户端与服务端断连重连期间，注册的 Watcher 也可能失效。

**解决方案**：

使用 Curator 提供的 `CuratorCache`（旧版 `NodeCache` / `PathChildrenCache` / `TreeCache`）实现持续监听，它内部自动重新注册 Watcher，并缓存节点数据。
...
public class PersistentWatcherDemo {
    public static void main(String[] args) throws Exception {
        ...
        CuratorCacheListener listener = CuratorCacheListener.builder()
                .forCreates(child -> System.out.println("节点创建: " + child.getPath()))
                .forChanges((oldNode, node) -> System.out.println(
                        "节点变更: " + node.getPath() + " 新数据: " + new String(node.getData())))
                .forDeletes(child -> System.out.println("节点删除: " + child.getPath()))
                .forInitialized(() -> System.out.println("缓存初始化完成"))
                .build();
...
### 问题二：Session 超时导致临时节点被误删

**问题描述**：服务节点实际仍在运行，但 ZooKeeper 服务端因 GC 停顿或网络抖动未收到心跳，判定 Session 过期，删除了该服务注册的临时节点，导致注册中心误认为服务下线。

**原因分析**：

- Session 超时时间设置过短。
- 网络抖动导致心跳包丢失。
- JVM 长时间 Full GC 导致心跳线程无法执行。

**解决方案**：

1. 合理设置 `sessionTimeout`，生产环境建议 30~60 秒。
2. 优化 JVM 参数避免长时间 GC。
3. 客户端实现重连与节点重建逻辑。
...
public class SessionRecoverDemo {
    public static void main(String[] args) throws Exception {
        CuratorFramework client = CuratorFrameworkFactory.builder()
                .connectString("localhost:2181")
                .sessionTimeoutMs(30000)  // 设置 30 秒 Session 超时
                .connectionTimeoutMs(5000)
                .retryPolicy(new ExponentialBackoffRetry(1000, 3))
                .build();
        ...
        client.getConnectionStateListenable().addListener(new ConnectionStateListener() {
            ...
            public void stateChanged(CuratorFramework client, ConnectionState newState) {
                switch (newState) {
                    case RECONNECTED:
                        System.out.println("重连成功，重新注册临时节点");
                        reRegisterNode(client);
                        break;
                    case SUSPENDED:
                        System.out.println("连接断开，业务降级");
                        break;
                    case LOST:
                        System.out.println("Session 丢失，停止服务");
                        break;
                    default:
                        break;
                }
            ...
            private void reRegisterNode(CuratorFramework client) {
                try {
                    String path = client.create()
                            .creatingParentsIfNeeded()
                            .withMode(org.apache.zookeeper.CreateMode.EPHEMERAL)
                            .forPath("/services/payment-node-1", "alive".getBytes());
                    System.out.println("重新注册节点: " + path);
                } catch (Exception e) {
                    e.printStackTrace();
                }
            }
...
**问题描述**：ZooKeeper 集群因网络分区出现两个 Leader，各自处理写请求，导致不同客户端看到的数据不一致。

**原因分析**：

ZooKeeper 的 ZAB 协议基于"过半机制"（Quorum）保证一致性。只有拥有过半节点支持的 Leader 才能处理写请求。网络分区时，少数派分区无法凑齐过半 ACK，会进入选举状态，无法写入；多数派分区才能选出新 Leader 继续服务。因此**标准 ZAB 协议本身不会出现两个 Leader 同时写数据的情况**。

如果出现"双 Leader"，通常是部署配置错误（如 `server.x` 配置不一致）或使用了非标准版本。集群节点数必须是奇数（3、5、7），偶数节点数会在分区时出现两个"过半"分区。

**解决方案**：

1. 集群节点数保持奇数（推荐 3 或 5 个节点）。
2. 确保所有节点的 `zoo.cfg` 中 `server.x` 配置完全一致。
3. 部署跨机房时，节点数分布要保证主机房凑齐过半。

```properties
# zoo.cfg - 5 节点集群配置示例（保证任意 3 个节点凑齐过半）
tickTime=2000
initLimit=10
syncLimit=5
dataDir=/usr/local/zookeeper/data
clientPort=2181
# 格式：server.<myid>=<host>:<集群通信端口>:<选举端口>
server.1=zoo-node1:2888:3888
server.2=zoo-node2:2888:3888
server.3=zoo-node3:2888:3888
server.4=zoo-node4:2888:3888
server.5=zoo-node5:2888:3888
```

```bash
# 每个节点 dataDir 下创建 myid 文件，内容为对应数字
echo "1" > /usr/local/zookeeper/data/myid   # 在 zoo-node1 上执行
echo "2" > /usr/local/zookeeper/data/myid   # 在 zoo-node2 上执行
# 以此类推
```
