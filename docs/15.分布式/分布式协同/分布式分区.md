...
| 特性 | 说明 |
| --- | --- |
| **水平扩展** | 通过增加节点线性扩展存储容量和吞吐量，突破单机瓶颈 |
| **数据局部性** | 每条数据只属于一个分区，查询可精确定位到目标分区，避免全表扫描 |
| **负载均衡** | 通过合理的分区策略，将数据和请求均匀分布到所有节点 |
| **透明性** | 对上层应用透明，分区的划分、迁移由系统自动管理 |
| **独立容错** | 每个分区可独立配置副本数，单个分区故障不影响其他分区 |
| **可再均衡** | 节点增删时，分区可动态迁移，保证集群负载持续均衡 |
| **与复制结合** | 分区通常与复制结合使用，每个分区在多个节点上存有副本 |

## 原理

### 分区的基本原理

分区的核心目标是将大量数据分散到多个节点上，每个节点只负责一部分数据。其基本原理如下：

```mermaid
graph TB
    Client[客户端请求] --> Router[路由层/分区感知]
    Router --> PartitionFunc[分区函数]
    PartitionFunc --> P1[分区 1<br/>节点 A]
    PartitionFunc --> P2[分区 2<br/>节点 B]
    PartitionFunc --> P3[分区 3<br/>节点 C]
    PartitionFunc --> P4[分区 4<br/>节点 D]
```
...
| 策略 | 原理 | 优点 | 缺点 | 典型系统 |
| --- | --- | --- | --- | --- |
| **基于关键字区间** | 按关键字排序，每个分区负责一段连续区间 | 支持高效的区间查询 | 易产生热点 | `Bigtable`、`HBase`、`2.4 前 MongoDB` |
| **基于哈希值** | 对关键字取哈希，按哈希值范围分区 | 数据分布均匀，避免热点 | 丧失区间查询能力 | `Cassandra`、`MongoDB`（哈希分片） |
| **一致性哈希** | 哈希值组成环，节点在环上占位 | 节点增删时迁移量小 | 可能数据分布不均 | `Cassandra`、`Dynamo` |
| **复合分区** | 主键第一部分哈希分区，其余部分排序 | 兼顾均匀分布和局部区间查询 | 复杂度较高 | `Cassandra` 复合主键 |

### 分区再均衡流程

```mermaid
sequenceDiagram
    participant NodeA as 节点 A（原有分区1,2）
    participant NodeB as 节点 B（原有分区3,4）
    participant NodeC as 节点 C（新加入）
    participant Coordinator as 协调器

    Coordinator->>NodeC: 1. 新节点加入集群
    Coordinator->>NodeA: 2. 迁移部分分区（如分区2）
    Coordinator->>NodeB: 3. 迁移部分分区（如分区4）
    NodeA-->>NodeC: 4. 数据传输中（双写期）
    NodeB-->>NodeC: 5. 数据传输中（双写期）
    Coordinator->>Coordinator: 6. 更新路由表
    Coordinator->>NodeC: 7. 分区2,4 正式对外服务
```
...
![](https://raw.githubusercontent.com/dunwu/images/master/archive/2024/05/4dff8b7eba844cca96586ae265293ad3.png)
...
![](https://raw.githubusercontent.com/dunwu/images/master/archive/2024/05/11a7dfd099c04e4d930bd2bc076b1d5d.png)
...
- **海量数据存储** - 当数据量超过单机存储能力时，通过分区将数据分散到多台机器。如 `HBase` 存储数百 `TB` 日志数据。
- **高并发读写** - 通过分区将读写负载分散到多个节点，提升整体吞吐量。如电商订单库按用户 `ID` 分片。
- **时序数据** - 监控指标、日志等时序数据按时间分区，便于按时间范围查询和过期数据清理。如 `InfluxDB`、`TimescaleDB`。
- **多租户系统** - `SaaS` 应用按租户 `ID` 分区，实现租户间数据物理隔离。
- **地理分布式部署** - 按地理位置分区，用户数据就近存储，降低访问延迟。如 `Cassandra` 多数据中心部署。
- **冷热数据分离** - 热数据存储在 `SSD` 节点，冷数据迁移到 `HDD` 节点，通过分区策略实现。

## 最佳实践

### 案例一：MySQL 分库分表（基于 ShardingSphere）

当单表数据量超过千万级，查询性能急剧下降，需要进行分库分表。`Apache ShardingSphere` 是一款优秀的分库分表中间件。
  ...
  shardingsphere:
    ...
    datasource:
      ...
      ds0:
        ...
        jdbc-url: jdbc:mysql://127.0.0.1:3306/order_db_0?useSSL=false&characterEncoding=utf8
        username: root
        password: root
      ds1:
        type: com.zaxxer.hikari.HikariDataSource
        driver-class-name: com.mysql.cj.jdbc.Driver
        jdbc-url: jdbc:mysql://127.0.0.1:3306/order_db_1?useSSL=false&characterEncoding=utf8
        username: root
        password: root
    rules:
      sharding:
        ...
        key-generators:
          snowflake:
            ...
            props:
              worker-id: 1
    props:
      sql-show: true  # 打印分片 SQL 日志，便于调试
```
...
public interface OrderMapper {
    ...
    @Insert("INSERT INTO t_order (order_id, user_id, product_code, quantity, amount, create_time) " +
            "VALUES (#{orderId}, #{userId}, #{productCode}, #{quantity}, #{amount}, #{createTime})")
...
> **说明**：`ShardingSphere` 对业务代码完全透明，开发者像操作单表一样编写 `SQL`，中间件自动完成路由、改写和结果归并。分库键选择 `user_id` 保证同一用户的订单在同一库中，分表键选择 `order_id` 保证订单均匀分布。

### 案例二：Kafka 分区设计与消费者分配

`Kafka` 的分区（`Partition`）是其并行处理和水平扩展的基础，合理的分区数和分配策略对性能至关重要。

**（1）创建多分区 Topic**

```bash
# 创建 6 个分区、3 个副本的 topic
...
public class KafkaPartitionProducer {
    ...
    public static void main(String[] args) {
        ...
        try (KafkaProducer<String, String> producer = new KafkaProducer<>(props)) {
            // 方式一：指定 key，Kafka 会对 key 取 hash 自动分区
            // 同一 key 的消息总是进入同一分区，保证顺序性
            ProducerRecord<String, String> record1 =
                new ProducerRecord<>("order-events", "user-1001", "订单创建事件");
            ...
            ProducerRecord<String, String> record2 =
                new ProducerRecord<>("order-events", 3, "user-1002", "订单支付事件");
...
> **说明**：`Kafka` 分区数建议设置为消费者数量的整数倍，保证负载均匀。分区数确定后不能减少（只能增加），需提前规划。

### 案例三：Redis Cluster 分片部署

`Redis Cluster` 通过哈希槽（`Hash Slot`）实现分区，共 `16384` 个槽位，分布到多个节点。

**（1）集群配置（6 节点：3 主 3 从）**

```bash
# 节点配置文件 redis-7000.conf
port 7000
cluster-enabled yes
...
public class RedisClusterExample {
    ...
    public static void main(String[] args) {
        ...
        try (StatefulRedisClusterConnection<String, String> connection = client.connect()) {
            ...
            // Redis 自动根据 key 的 hash slot 路由到对应节点
            sync.set("user:1001", "张三");
            sync.set("user:1002", "李四");
            sync.set("order:2001", "订单数据");

            System.out.println("user:1001 = " + sync.get("user:1001"));
            System.out.println("user:1002 = " + sync.get("user:1002"));

            // 使用 hash tag 强制多个 key 到同一槽位（支持事务和 MGET）
            sync.set("{user:1001}:profile", "个人信息");
            sync.set("{user:1001}:settings", "用户设置");

            // 以下操作可以跨槽位执行（因 hash tag 相同）
            System.out.println("profile = " + sync.get("{user:1001}:profile"));
            System.out.println("settings = " + sync.get("{user:1001}:settings"));
        } finally {
            client.shutdown();
        }
    }
...
> **说明**：`Redis Cluster` 使用 `CRC16` 算法计算 `key` 的哈希槽，槽位范围 `0-16383`。使用 `{}` 括起来的部分作为哈希计算依据，称为 `Hash Tag`，可将相关 `key` 路由到同一节点，便于执行事务和 `MGET` 等批量操作。
...
`MySQL` 分库分表后，发现某个分片数据量和请求量远高于其他分片，导致该节点 `CPU` 和 `I/O` 持续高位，而其他节点资源闲置。

**原因分析**

1. **分片键选择不当**：如按时间字段分片，近期数据集中在同一分片。
2. **数据本身倾斜**：某些热门用户（如大 `V`）产生大量数据，集中在一个分片。
3. **哈希函数分布不均**：哈希函数设计不合理，导致数据分布不均匀。

**解决方案**

1. **选择基数大、分布均匀的字段作为分片键**，如 `user_id` 而非 `create_time`。

2. **对热点 `key` 添加随机前缀**，打散到多个分片：

```java
import java.util.concurrent.ThreadLocalRandom;

public class HotKeySharder {

    /**
     * 对热点 key 添加随机后缀，打散到 10 个分片
     */
    public static String shardHotKey(String originalKey) {
     ...
     * 查询时需聚合所有分片的数据
     */
    public static String[] getAllShardKeys(String originalKey) {
...
3. **使用一致性哈希**，减少节点增删时的数据迁移：

```yaml
# Cassandra 配置一致性哈希分区
# cassandra.yaml
partitioner: org.apache.cassandra.dht.Murmur3Partitioner
num_tokens: 16  # 虚拟节点数，值越大分布越均匀
```
...
`Elasticsearch` 集群新增节点后触发 ` shard rebalancing`，期间集群状态变为 `yellow` 或 `red`，查询延迟剧增甚至超时。

**原因分析**

1. **再均衡并发度过高**：默认配置下同时迁移多个分片，占用大量网络和磁盘 `I/O`。
2. **节点资源不足**：新节点未预热，磁盘 `I/O` 瓶颈导致恢复速度慢。
3. **再均衡期间触发熔断器**：大量请求堆积导致熔断。
...
3. **预热新节点**：先将只读副本分配到新节点，待数据完整后再开放写入。

### 问题三：跨分区查询性能差

**问题描述**

分库分表后，某些查询（如按非分片键查询、统计聚合）需要扫描所有分片，查询耗时是单表的数倍。

**原因分析**

分片键决定了数据的分布，非分片键查询无法定位到特定分片，只能广播到所有分片执行（`scatter-gather`），然后合并结果。分区数越多，查询放大越严重。

**解决方案**

1. **引入二级索引或映射表**，建立非分片键到分片键的映射：
...
-- 查询时：先查映射表获取 user_id，再精确路由到对应分片
SELECT user_id FROM order_user_mapping WHERE order_id = 10086;
-- 假设查到 user_id = 1001
SELECT * FROM t_order WHERE order_id = 10086 AND user_id = 1001;
```

2. **使用 `Elasticsearch` 等搜索引擎构建全局索引**，支持复杂查询：

```java
// 写入 MySQL 的同时异步同步到 ES
@Async
public void syncToElasticsearch(Order order) {
    OrderDoc doc = new OrderDoc();
    doc.setOrderId(order.getOrderId());
    doc.setUserId(order.getUserId());
    doc.setProductCode(order.getProductCode());
    doc.setAmount(order.getAmount());
    doc.setCreateTime(order.getCreateTime());
    // ES 支持任意字段查询，不受分片键限制
    orderRepository.save(doc);
}

// 复杂查询走 ES
public List<OrderDoc> searchOrders(String productCode, BigDecimal minAmount) {
    BoolQueryBuilder query = QueryBuilders.boolQuery()
        .must(QueryBuilders.matchQuery("productCode", productCode))
        .must(QueryBuilders.rangeQuery("amount").gte(minAmount));
    return orderRepository.search(query);
}
```

3. **合理设计 `CQL`（`Cassandra Query Language`）**，利用复合主键支持多种查询模式：

```sql
-- Cassandra 复合主键设计
CREATE TABLE orders (
    user_id bigint,           -- 分区键（哈希分区）
    order_id timeuuid,        -- 聚簇列（区间查询）
    product_code text,
    amount decimal,
