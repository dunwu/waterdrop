---
title: 负载均衡
cover: https://raw.githubusercontent.com/dunwu/images/master/archive/2023/10/9ca8c286daa447b490313f3cc0c23c8d.png
...
![](https://raw.githubusercontent.com/dunwu/images/master/archive/2023/10/46db84c170d549b689a2101ea27a086e.png)
...
![](https://raw.githubusercontent.com/dunwu/images/master/archive/2023/10/14edfaa3a18247fb9466319c61c80cba.png)
...
| 特性           | 说明                                                             |
| -------------- | ---------------------------------------------------------------- |
| 高并发处理     | 通过流量分发，将单机无法承受的并发量分摊到多台机器               |
| 透明扩展       | 对客户端透明，增减后端节点不影响客户端访问                       |
| 健康检查       | 自动检测后端节点健康状态，剔除故障节点，避免请求发往不可用节点   |
| 会话保持       | 通过 IP Hash、Cookie 等方式将同一用户的请求固定到同一后端节点   |
| 动态调度       | 根据后端节点负载情况动态调整分发策略                             |
| 安全防护       | 部分负载均衡器（如 F5、HAProxy）提供防 DDoS、黑白名单等安全功能  |
| 协议支持       | 支持四层（TCP/UDP）和七层（HTTP/HTTPS）协议的负载均衡            |
| 持久化连接     | 支持长连接复用，减少 TCP 握手开销                                |

## 负载均衡的原理

### 四层负载均衡原理

四层负载均衡工作在 OSI 模型的传输层，主要通过修改 IP 地址和端口来实现请求转发，不关心应用层协议内容。

```mermaid
graph LR
    C[客户端] -->|请求: 目标IP=VIP, 端口=80| LB[负载均衡器]
    LB -->|修改目标IP为RealServer1| RS1[后端服务器1]
    LB -->|修改目标IP为RealServer2| RS2[后端服务器2]
    RS1 -->|响应: 源IP=VIP| LB
    LB -->|转发响应| C
```
...
| 模式      | 工作原理                                           | 特点                                 |
| --------- | -------------------------------------------------- | ------------------------------------ |
| NAT 模式  | 修改请求的目标 IP 和端口，响应也经过负载均衡器     | 配置简单，但响应流量受 LB 带宽限制   |
| DR 模式   | 修改请求的目标 MAC 地址，响应直接返回客户端        | 性能高，但要求 LB 和后端在同一网段   |
| TUN 模式  | 通过 IP 隧道转发，响应直接返回客户端               | 支持跨网段，但后端需支持隧道协议     |

### 七层负载均衡原理

七层负载均衡工作在应用层，能够解析 HTTP 协议内容，根据 URL、Header、Cookie、请求体等信息进行路由决策。

```mermaid
graph TD
    C[客户端] -->|HTTP 请求| LB[七层负载均衡器]
    LB -->|解析 URL/Header| D{路由决策}
    D -->|/api/user/**| RS1[用户服务集群]
    D -->|/api/order/**| RS2[订单服务集群]
    D -->|/static/**| RS3[静态资源服务]
    D -->|Cookie=mobile| RS4[移动端专用集群]
    RS1 -->|HTTP 响应| LB
    LB -->|转发响应| C
```
...
- **基于内容的路由**：可根据 URL、Header、Cookie 等精细分流。
- **SSL 卸载**：在 LB 层终结 SSL，后端使用明文 HTTP，减轻后端加解密压力。
- **请求改写**：可修改请求头、重定向、压缩等。
- **更精细的健康检查**：可发送 HTTP 请求检查应用层状态。

### 健康检查机制

负载均衡器通过定期健康检查判断后端节点是否可用：

```mermaid
graph TD
    LB[负载均衡器] -->|每 5s 发送 TCP SYN| RS1[服务器1]
    LB -->|每 5s 发送 TCP SYN| RS2[服务器2]
    LB -->|每 5s 发送 TCP SYN| RS3[服务器3]
    RS1 -->|SYN-ACK 正常| LB
    RS2 -->|无响应/超时| LB
    LB -->|标记 RS2 为 Down| LB
    LB -->|流量只分发到 RS1、RS3| RS1
    LB -->|流量只分发到 RS1、RS3| RS3
```
...
| 检查方式   | 原理                                   | 适用场景             |
| ---------- | -------------------------------------- | -------------------- |
| TCP 检查   | 尝试建立 TCP 连接，成功即认为健康      | 通用，开销小         |
| HTTP 检查  | 发送 HTTP 请求，根据状态码判断健康     | Web 服务，可检测应用层故障 |
| 自定义脚本 | 执行自定义脚本，根据返回值判断         | 复杂业务场景         |

## 负载均衡的应用场景

### Web 应用流量分发

大型网站使用 Nginx 作为反向代理负载均衡器，将用户请求分发到多个 Web 服务器，支撑高并发访问。

### 微服务RPC调用

微服务框架（如 Dubbo、Spring Cloud）在客户端集成负载均衡器，在多个服务提供者实例之间分发 RPC 请求。

### 数据库读写分离

通过负载均衡器将读请求分发到多个只读副本，写请求路由到主库，实现数据库的读写分离。

### CDN 全局负载均衡

DNS 负载均衡作为 CDN 的第一级调度，根据用户地理位置将请求导向最近的边缘节点。

### 消息队列消费者负载均衡

Kafka、RocketMQ 等消息队列通过消费者组机制，将分区（Partition）均匀分配给组内消费者，实现消费负载均衡。

## 负载均衡的最佳实践

### 案例一：Nginx 反向代理负载均衡配置

**场景**：一个 Web 应用部署了 3 个实例，需要通过 Nginx 实现负载均衡，要求支持健康检查、权重分配和会话保持。
...
- `weight` 设置权重，处理能力强的机器分配更高权重。
- `max_fails` 和 `fail_timeout` 配合实现被动健康检查：30 秒内失败 3 次则暂时剔除。
- `proxy_next_upstream` 配置请求失败时的自动重试。
- `keepalive` 开启长连接复用，减少与后端的 TCP 连接开销。

### 案例二：Spring Cloud Ribbon 客户端负载均衡

**场景**：微服务架构中，订单服务需要调用用户服务，用户服务有多个实例，需要在客户端实现负载均衡。
...
**说明**：Ribbon 是客户端负载均衡器，负载均衡逻辑在调用方执行，无需独立的负载均衡服务器。通过 Nacos 服务发现自动获取 `user-service` 的实例列表，Ribbon 根据配置的策略选择实例发起请求。

### 案例三：Spring Cloud LoadBalancer 自定义负载均衡策略

**场景**：默认的轮询策略无法满足需求，需要实现基于元数据的灰度路由——将带有特定 Header 的请求路由到灰度版本实例。
        ...
        return supplier.get(request).next()
            .flatMap(serviceInstances -> {
                ...
                if (request.getContext() instanceof RequestData) {
                    ...
                    grayVersion = data.getHeaders().getFirst("X-Gray-Version");
                }

                final String targetVersion = grayVersion;
...
**说明**：通过自定义 `ReactorServiceInstanceLoadBalancer`，可以根据请求中的灰度标识筛选后端实例，实现精细化的流量路由。相比固定策略，自定义策略可以满足灰度发布、金丝雀发布等复杂场景需求。

## 负载均衡的常见问题

### 问题一：会话丢失导致用户频繁重新登录

**问题描述**：Web 应用使用负载均衡后，用户反馈经常被强制退出登录，需要重新登录。

**原因分析**：默认的轮询或随机算法会将同一用户的请求分发到不同后端服务器。如果 Session 存储在应用本地内存中，不同服务器之间不共享 Session，用户请求被分发到没有其 Session 的服务器时，会被判定为未登录。

**解决方案**：

1. **会话保持（Sticky Session）**：使用 IP Hash 将同一用户固定到同一后端。

```nginx
upstream web_backend {
    ip_hash;  # 基于 IP 哈希的会话保持
...
2. **Session 集中存储**：将 Session 存储到 Redis 等共享存储中，所有后端服务器共享。
...
3. **无状态化（JWT）**：彻底放弃 Session，使用 JWT 令牌将用户状态存储在客户端，服务端无状态。
...
**推荐**：方案 3（无状态化）是最彻底的解决方案，便于水平扩展，方案 2 适合改造既有系统，方案 1 最简单但存在负载不均和单点故障风险。

### 问题二：负载均衡不均，部分机器压力过大

**问题描述**：集群中各机器配置相同，但某些机器 CPU、内存使用率明显高于其他机器，甚至出现过载。

**原因分析**：

1. **使用轮询算法但请求处理时长差异大**：轮询算法均匀分配请求数量，但请求复杂度不同，导致处理慢的机器积压。
2. **长连接导致连接数不均**：使用长连接时，早期建立的连接集中在部分机器上。
3. **会话保持导致流量倾斜**：IP Hash 会话保持时，代理服务器（如 NAT 出口）背后的海量用户被哈希到同一后端。
4. **健康检查误判**：部分机器负载高导致健康检查响应慢，被暂时剔除后又恢复，流量波动。

**解决方案**：

1. **改用最少连接数算法**：动态感知后端连接数，将请求分发到负载最轻的机器。
...
2. **调整健康检查参数**：避免误判，适当放宽响应超时阈值。

```nginx
upstream web_backend {
    server 192.168.1.11:8080 max_fails=5 fail_timeout=60s;
    server 192.168.1.12:8080 max_fails=5 fail_timeout=60s;
    server 192.168.1.13:8080 max_fails=5 fail_timeout=60s;
}
```
...
**问题描述**：负载均衡器宕机后，整个集群不可访问，服务完全中断。

**原因分析**：单台负载均衡器没有冗余，硬件故障、进程崩溃、网络中断等都会导致入口不可用。

**解决方案**：采用 **主备（Active-Standby）** 或 **双主（Active-Active）** 高可用架构。

```
方案一：主备模式
客户端 -> Keepalived VIP -> [主LVS(Active), 备LVS(Standby)] -> 后端集群

方案二：双主模式
客户端 -> DNS轮询 -> [LVS1(Active), LVS2(Active)] -> 后端集群
```

Keepalived + LVS 主备模式配置示例：
...
```bash
# 备节点 keepalived.conf（仅 state 和 priority 不同）
global_defs {
...
# virtual_server 部分与主节点相同
```

**说明**：Keepalived 基于 VRRP 协议实现 VIP 漂移。主节点正常运行时持有 VIP，对外提供服务；主节点故障时，备节点通过 VRRP 心跳感知并接管 VIP，实现自动切换。切换过程对客户端透明，通常在 1-3 秒内完成。
