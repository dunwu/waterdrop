...
**『轮询算法（Round Robin）』的策略是：将请求依次分发到候选服务器**。

如下图所示，负载均衡器收到来自客户端的 6 个请求，(1, 3, 5) 的请求会被发送到服务器 0，(2, 4, 6) 的请求会被发送到服务器 1。

![](https://raw.githubusercontent.com/dunwu/images/master/snap/202310240749618.png)

该算法适合场景：各服务器处理能力相近，且每个事务工作量差异不大。如果存在较大差异，那么处理较慢的服务器就可能会积压请求，最终无法承担过大的负载。

![](https://raw.githubusercontent.com/dunwu/images/master/snap/202310240753080.png)
...
**『加权轮询算法（Weighted Round Robbin）』 在轮询算法的基础上，引入了权重属性，优先将请求分发到权重较高的节点**。这样，就可以针对性能高、处理速度快的机器设置较高的权重，让其处理更多的请求；而针对性能低、处理速度慢的机器则与之相反。一言以蔽之，加权策略强调了——能力越大，责任越大。

如下图所示，服务器 A 设置权重为 5，服务器 B 设置权重为 1，负载均衡器收到来自客户端的 6 个请求，那么 (1, 2, 3, 4, 5) 请求会被发送到服务器 0，(6) 请求会被发送到服务器 1。

![](https://raw.githubusercontent.com/dunwu/images/master/snap/202310240751444.png)
...
**『随机算法（Random）』 将请求随机分发到候选服务器**。

**随机算法适合服务器硬件相同的场景**。学习过概率论的都知道，调用量较小的时候，可能负载并不均匀，**调用量越大，负载越均衡**。

![](https://raw.githubusercontent.com/dunwu/images/master/snap/202310240749652.png)
...
随机算法适合服务器硬件相同的场景。但是，如果服务器硬件条件不相同时，就无法做到负载均衡。

**『加权随机算法（Weighted Random）』 在随机算法的基础上，引入了权重属性，优先将请求分发到权重较高的节点**。这样，就可以针对性能高、处理速度快的机器设置较高的权重，让其处理更多的请求；而针对性能低、处理速度慢的机器则与之相反。一言以蔽之，加权策略强调了——能力越大，责任越大。
...
**『最小活跃数算法（Least Active）』 将请求分发到连接数/请求数最少的候选服务器（目前处理请求最少的服务器）**。
...
![](https://raw.githubusercontent.com/dunwu/images/master/snap/202310240754990.png)

最小活跃数算法会记录当前时刻，每个候选节点正在处理的连接数，然后选择连接数最小的节点。该策略能够动态、实时地反应服务器的当前状况，较为合理地将负责分配均匀，适用于对当前系统负载较为敏感的场景。

例如下图中，服务器 1 当前连接数最小，那么新到来的请求 6 就会被发送到服务器 1 上。

![](https://raw.githubusercontent.com/dunwu/images/master/snap/202310240754658.png)

**『加权最小活跃数算法（Weighted Least Connection）』在最小活跃数的基础上，根据服务器的性能为每台服务器分配权重，再根据权重计算出每台服务器能处理的连接数**。
...
**『哈希算法（Hash）』 根据一个 key （可以是唯一 ID、IP 等），通过哈希计算得到一个数值，用该数值在候选服务器列表的进行取模运算，得到的结果便是选中的服务器**。

![](https://raw.githubusercontent.com/dunwu/images/master/snap/202310240755335.png)

可以保证同一 IP 的客户端的请求会转发到同一台服务器上，常被用于实现会话粘滞（Sticky Session）。

但是 ，哈希算法的问题是：当增减节点时，由于哈希取模函数的基数发生变化，会影响大部分的映射关系，从而导致之前的数据不可访问。要解决这个问题，就必须根据新的计算公式迁移数据。显然，如果数据量很大的情况下，迁移成本很高。

![](https://raw.githubusercontent.com/dunwu/images/master/snap/202310240755678.png)
...
一致性哈希可以很好的解决稳定性问题，可以将所有的存储节点排列在首尾相接的 `Hash` 环上，每个 `key` 在计算 `Hash` 后会顺时针找到邻接的存储节点存放。而当有节点加入或退出时，仅影响该节点在 `Hash` 环上顺时针相邻的后续节点。

![](https://raw.githubusercontent.com/dunwu/images/master/snap/202310240758747.png)
...
增减节点只影响 **哈希环** 中 **顺时针方向** 的 **相邻的节点**，对其他节点无影响。

- **缺点**

**增减节点** 会造成 **哈希环** 中部分数据 **无法命中**。当使用 **少量节点** 时，**节点变化** 将大范围影响 **哈希环** 中 **数据映射**，不适合 **少量数据节点** 的分布式方案。**普通** 的 **一致性哈希分区** 在增减节点时需要 **增加一倍** 或 **减去一半** 节点才能保证 **数据** 和 **负载的均衡**。

增加节点情况：

![](https://raw.githubusercontent.com/dunwu/images/master/snap/202310240759114.png)

减少节点情况：

![](https://raw.githubusercontent.com/dunwu/images/master/snap/202310240759115.png)

哈希环的问题

![](https://raw.githubusercontent.com/dunwu/images/master/snap/202310240812654.png)
...
![](https://raw.githubusercontent.com/dunwu/images/master/snap/202310240800188.png)

![](https://raw.githubusercontent.com/dunwu/images/master/snap/202310240802517.png)
