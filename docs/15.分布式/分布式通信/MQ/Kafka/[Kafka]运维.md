...
Kafka 运维涵盖集群部署、配置调优、监控告警、故障排查、容量规划等多个方面。作为一个高吞吐的分布式消息系统，Kafka 集群通常承载着企业核心的数据管道，一旦故障可能导致上下游多个业务中断，因此运维质量直接影响业务稳定性。

Kafka 运维的难点在于：一是参数众多，Broker、Topic、Producer、Consumer 各层级都有大量可调参数，配置不当会引发性能或可靠性问题；二是故障场景复杂，涉及磁盘、网络、JVM GC、ZooKeeper 等多个维度；三是监控指标分散，需要结合 JMX、日志、命令行工具综合判断。

本文从部署、命令、配置、规划等角度梳理 Kafka 运维要点，并给出生产环境的最佳实践与常见问题排查方案。
...
| 维度 | 说明 |
| --- | --- |
| 部署方式 | 支持单机、集群、KRaft 模式（去 ZooKeeper） |
| 配置层级 | Broker 级、Topic 级、Client 级三层配置，灵活覆盖 |
| 命令行工具 | 提供主题、消费者组、配置、ACL、性能测试等完整 CLI |
| JMX 监控 | 暴露 200+ 指标，便于接入 Prometheus/Grafana |
| 动态配置 | 支持运行时修改 Topic/Broker 配置，无需重启 |
| 滚动升级 | 支持在线滚动升级，业务无感知 |
| 副本重分配 | 支持在线扩缩容时重新分配副本位置 |
| 安全认证 | 支持 SASL、SSL、ACL 多层安全机制 |

## 应用场景

Kafka 运维适用于以下典型场景：

- **生产集群部署**：从零搭建高可用 Kafka 集群，规划 Broker 数量、磁盘、网络。
- **集群扩缩容**：业务量增长时新增 Broker 并迁移副本，下线时安全移除节点。
- **性能调优**：针对高吞吐或低延迟场景调优 Producer/Consumer/Broker 参数。
- **故障排查**：处理消息积压、副本不同步、磁盘告警、Rebalance 异常等问题。
- **安全加固**：启用 SASL/SSL 加密传输，配置 ACL 权限控制。
- **版本升级**：滚动升级到新版本，兼容性验证与回滚预案。
- **多租户管理**：通过配额（Quota）限制租户带宽，避免相互影响。
...
### 案例一：生产集群 Broker 标准配置

**场景**：搭建 6 节点生产集群，承载日均 TB 级消息，要求高可用与高吞吐。
...
# JVM 与 GC（通过环境变量 KAFKA_HEAP_OPTS 设置）
# export KAFKA_HEAP_OPTS="-Xms16g -Xmx16g -XX:MetaspaceSize=256m -XX:MaxMetaspaceSize=512m"
# export KAFKA_OPTS="-XX:+UseG1GC -XX:MaxGCPauseMillis=20 -XX:InitiatingHeapOccupancyPercent=35"
```

**说明**：生产环境核心配置原则：关闭自动创建 Topic、关闭 Unclean 选举、`replication.factor=3` + `min.insync.replicas=2`、多磁盘挂载、JVM 堆 16G + G1 GC。

### 案例二：在线扩容与副本重分配

**场景**：现有 3 节点集群扩容到 6 节点，需将部分副本迁移到新节点。

```shell
# 1. 生成重分配方案
cat > topics-to-move.json << EOF
{"topics": [{"topic": "order-event"}, {"topic": "payment-stream"}],
 "version": 1}
EOF
...
**说明**：重分配期间会触发大量跨 Broker 数据拷贝，建议在低峰期执行；可通过 `throttle` 参数限制迁移速率避免影响业务。

### 案例三：监控指标接入 Prometheus

**场景**：将 Kafka JMX 指标接入 Prometheus + Grafana，实现可视化监控。

```yaml
# prometheus.yml - 使用 JMX Exporter 采集 Kafka 指标
scrape_configs:
  - job_name: 'kafka'
    jmx_exporter:
      # 每台 Broker 部署 JMX Exporter Agent
      host: 'broker1'
      port: 7071
    static_configs:
      - targets:
...
```java
// 关键告警指标（通过 PromQL 查询）
// 1. UnderReplicatedPartitions > 0：副本不同步告警
//    kafka_server_replicamanager_underreplicatedpartitions > 0

// 2. ActiveControllerCount != 1：Controller 异常告警
//    kafka_controller_kafkacontroller_activecontrollercount != 1

// 3. OfflinePartitionsCount > 0：分区离线告警
//    kafka_controller_kafkacontroller_offlinepartitionscount > 0

// 4. ISR 缩减速率：监控副本健康度
//    rate(kafka_server_replicamanager_underreplicatedpartitions[5m]) > 0

// 5. 请求队列积压
//    kafka_network_requestchannel_requestqueuesize > 100
```

**说明**：必须监控的核心指标为 `UnderReplicatedPartitions`、`ActiveControllerCount`、`OfflinePartitionsCount`，任一异常都代表集群健康受损。

## 常见问题

### 问题一：Broker 启动报 Too many open files

**问题描述**：Kafka Broker 启动或运行一段时间后报 `java.io.IOException: Too many open files`，进程退出。

**原因分析**：
1. 操作系统文件描述符限制过低，默认 1024 远不够 Kafka 使用。
2. Partition 数过多，每个 Partition 的每个 Segment 都会占用文件句柄。
3. Segment 文件未及时关闭，句柄泄漏。
...
kafka soft nproc 4096
kafka hard nproc 4096
EOF

# 4. systemd 方式（/etc/systemd/system/kafka.service）
...
**说明**：生产环境建议 `ulimit -n 1000000`；同时控制单 Broker Partition 数不超过 4000，避免句柄爆炸。

### 问题二：Topic 数据积压导致磁盘满

**问题描述**：某 Topic 因消费者下线，消息持续堆积，7 天后磁盘空间告警。
...
kafka-configs.sh --bootstrap-server localhost:9092 \
  --alter --entity-type topics --entity-name problem-topic \
  --add-config retention.ms=3600000  # 缩短到 1 小时

# 2. 观察磁盘释放后，恢复合理保留期
kafka-configs.sh --bootstrap-server localhost:9092 \
  --alter --entity-type topics --entity-name problem-topic \
  --add-config retention.ms=86400000  # 恢复为 1 天
...
kafka-log-dirs.sh --bootstrap-server localhost:9092 \
  --topic-list problem-topic --describe
```

```shell
# 5. 设置磁盘水位告警（通过脚本或 Prometheus）
# 当磁盘使用率 > 80% 告警，> 90% 紧急告警
df -h | grep /data/kafka | awk '{if ($5+0 > 80) print "WARN: " $6 " usage " $5}'
```

**说明**：预防措施包括「磁盘水位告警」+「消费者 Lag 监控」+「Topic 保留大小限制」，三管齐下避免磁盘满。

### 问题三：ZooKeeper 连接超时导致 Broker 频繁掉线

**问题描述**：Broker 日志频繁出现 `Session expired` 和 `Connection loss`，Broker 反复上下线，触发大量 Leader 选举。

**原因分析**：
1. ZooKeeper 集群负载高或 GC 停顿，响应缓慢。
2. `zookeeper.session.timeout.ms` 设置过小，网络抖动即超时。
3. ZooKeeper 与 Kafka 跨机房部署，网络延迟高。
4. ZooKeeper 连接数过多（每个 Kafka Broker/Client 都会连接 ZK）。
...
# 2. 检查 ZK 连接数
echo cons | nc zk1 2181 | grep -c "127.0.0.1"

# 3. ZK GC 调优（zookeeper-env.sh）
export SERVER_JVMFLAGS="-Xms4g -Xmx4g -XX:+UseG1GC -XX:MaxGCPauseMillis=100"
```

```shell
# 4. 长期方案：迁移到 KRaft 模式，去除 ZooKeeper 依赖
# Kafka 3.3+ KRaft 模式生产可用
...
**说明**：长期建议迁移到 KRaft 模式，消除 ZK 依赖；短期通过调大超时时间 + ZK 调优缓解。
