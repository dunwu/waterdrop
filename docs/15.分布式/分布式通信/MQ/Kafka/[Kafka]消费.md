...
Kafka 消费者在实际业务中有以下典型应用场景：

| 场景 | 说明 | 消费模式 |
| --- | --- | --- |
| **实时数据处理** | 消费用户行为数据，实时计算指标 | 流式消费，手动提交 |
| **日志分析** | 消费应用日志，写入 Elasticsearch | 批量消费，自动提交 |
| **消息通知** | 消费事件消息，触发短信/邮件通知 | 顺序消费（按 key） |
| **数据同步** | 消费 CDC 数据，同步到数仓 | 手动提交，确保一致性 |
| **订单处理** | 消费订单消息，更新库存和物流 | 顺序消费，事务提交 |
| **监控告警** | 消费监控指标，触发告警规则 | 低延迟消费 |

## 最佳实践

### 案例一：Spring Boot 整合 Kafka 消费者
...
public class OrderConsumer {
    ...
    public void handleOrder(ConsumerRecord<String, String> record, Acknowledgment ack) {
        try {
            System.out.printf("线程=%s, partition=%d, offset=%d, key=%s, value=%s%n",
...
public class BatchLogConsumer {
    ...
    @KafkaListener(topics = "app-logs", groupId = "batch-log-group")
    public void handleBatch(List<org.apache.kafka.clients.consumer.ConsumerRecord<String, String>> records) {
        System.out.println("本次批量接收消息数: " + records.size());

        // 批量写入数据库或 Elasticsearch
        batchInsert(records);

        System.out.println("批量处理完成");
    }

    private void batchInsert(List<org.apache.kafka.clients.consumer.ConsumerRecord<String, String>> records) {
        // 批量写入逻辑
    }
}
```
...
spring:
  kafka:
    consumer:
      group-id: batch-log-group
      max-poll-records: 1000  # 单次拉取最大记录数
    listener:
      ack-mode: batch  # 批处理完成后提交
      type: batch
```

### 案例三：优雅关闭消费者

在应用关闭时，需要确保正在处理的消息处理完毕，并提交偏移量：
...
public class GracefulShutdownConsumer {
    ...
    public static void main(String[] args) {
        Properties props = new Properties();
        props.put("bootstrap.servers", "localhost:9092");
        props.put("group.id", "graceful-group");
        props.put("enable.auto.commit", "false");
    ...
    private static void processRecord(
        ...
        System.out.printf("处理消息: offset=%d, value=%s%n", record.offset(), record.value());
    }
}
```
...
### 问题一：消费者频繁 Rebalance 导致消费中断

**问题描述**：消费者日志中频繁出现 `Member ... has failed` 和 `Rebalance` 记录，消费经常中断。

**原因分析**：
- `session.timeout.ms` 设置过小，消费者未及时发送心跳就被判定为死亡。
- `max.poll.interval.ms` 设置过小，消费者处理消息耗时超过该间隔。
- 消费者频繁 Full GC 导致长时间停顿。
- 网络不稳定导致心跳丢失。
...
```java
Properties props = new Properties();
props.put("session.timeout.ms", "30000");  // 会话超时 30s
props.put("heartbeat.interval.ms", "10000");  // 心跳间隔 10s
props.put("max.poll.interval.ms", "300000");  // 两次 poll 最大间隔 5 分钟
props.put("max.poll.records", "100");  // 减少 poll 记录数，避免处理超时
```
...
3. 优化消费逻辑，减少单条消息处理时间。将耗时操作异步化。

### 问题二：消息消费重复

**问题描述**：消费者重启后，部分已被处理的消息被重新消费。

**原因分析**：
- 使用自动提交偏移量（`enable.auto.commit=true`），在提交偏移量之前消费者崩溃。
- 发生 Rebalance 时，新消费者从旧的偏移量位置开始消费。
- 手动提交偏移量时机不对（先提交后处理）。

**解决方案**：

1. 关闭自动提交，改为处理完消息后手动提交：

```java
props.put("enable.auto.commit", "false");
```

2. 在消费者端实现幂等性。使用消息 ID 或业务唯一键去重：
...
3. 使用 `ConsumerRebalanceListener` 在 Rebalance 前提交偏移量：
...
consumer.subscribe(Collections.singletonList("topic"), new ConsumerRebalanceListener() {
    ...
    public void onPartitionsRevoked(Collection<TopicPartition> partitions) {
        // 在分区被回收前，提交当前偏移量
        consumer.commitSync();
        System.out.println("分区被回收，已提交偏移量: " + partitions);
    }
...
### 问题三：消费延迟（Consumer Lag）持续增大

**问题描述**：监控显示消费者组的 Lag 持续增大，消费者跟不上生产者的速度。

**原因分析**：
- 消费者实例数不足，无法充分利用分区并发。
- 消费者处理逻辑过慢（如同步调用外部 API）。
- 消费者频繁 Rebalance 导致暂停消费。
- 消费者端配置不合理（如 `fetch.min.bytes` 过大导致等待时间过长）。
...
2. 增加消费者实例或优化消费逻辑。确保消费者实例数等于分区数：
...
3. 调整 `fetch` 参数，减少不必要的等待：

```java
props.put("fetch.min.bytes", "1");  // 有数据立即返回
props.put("fetch.max.wait.ms", "100");  // 最多等待 100ms
props.put("max.poll.records", "500");  // 单次拉取最多 500 条
```

4. 将同步外部调用改为异步，或使用批量处理：

```java
// 优化前：每条消息单独调用 API（慢）
for (ConsumerRecord<String, String> record : records) {
    callExternalApi(record.value());  // 同步调用，耗时
}
...
batchCallExternalApi(batch);  // 批量调用，减少网络往返
```
