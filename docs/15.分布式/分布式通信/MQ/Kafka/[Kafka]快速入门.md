...
![](https://raw.githubusercontent.com/dunwu/images/master/cs/java/javaweb/distributed/mq/kafka/kafka-event-system.png)
...
Kafka 凭借其高吞吐、低延迟、可水平扩展的特性，被广泛应用于以下场景：

| 场景 | 说明 |
| --- | --- |
| **消息队列** | 作为传统的消息中间件，实现系统间异步通信、应用解耦 |
| **日志收集** | 收集各服务的日志，统一汇聚到 Kafka，再由消费者写入 ELK 等日志分析系统 |
| **用户行为追踪** | 收集 Web/App 的用户行为数据（点击、浏览、搜索），供数据分析和推荐系统使用 |
| **流式处理** | 结合 Kafka Streams 或 Flink，实现实时数据处理和 ETL |
| **事件溯源** | 将领域事件持久化到 Kafka，支持事件回放和状态重建 |
| **运维监控** | 收集系统和应用的监控指标数据，实现实时告警和仪表盘展示 |
| **CDC（变更数据捕获）** | 通过 Kafka Connect 捕获数据库变更，实现数据同步到数仓或搜索引擎 |

## 最佳实践

### 案例一：Spring Boot 整合 Kafka 实现日志收集
...
public class LogProducer {
    ...
    public void sendLog(String level, String message) {
        String logEntry = String.format("{\"level\":\"%s\",\"message\":\"%s\",\"timestamp\":%d}",
            level, message, System.currentTimeMillis());
        // 以日志级别作为 key，保证同一级别日志进入同一分区
        kafkaTemplate.send("app-logs", level, logEntry);
    }
}
```
...
public class LogConsumer {
    ...
    public void handleLog(ConsumerRecord<String, String> record) {
        System.out.printf("分区=%d, 偏移量=%d, key=%s, value=%s%n",
            record.partition(), record.offset(), record.key(), record.value());
        // 实际场景中：写入 Elasticsearch 或文件
    }
}
```

### 案例二：使用 Kafka Connect 实现 MySQL 数据同步

Kafka Connect 提供了开箱即用的 Source Connector 和 Sink Connector，无需编写代码即可实现数据同步。

配置 MySQL Source Connector（`mysql-source.json`）：
...
public class KafkaProducerBestPractice {
...
**问题描述**：生产者持续写入消息，但消费者处理速度跟不上，导致 Kafka 中消息大量积压，`consumer-lag` 持续增大。

**原因分析**：
- 消费者实例数少于分区数，部分消费者承担过多分区。
- 消费者处理逻辑耗时过长（如频繁访问数据库、调用外部接口）。
- 消费者频繁发生 Full GC 导致停顿。
- 消费者发生了非预期的 Rebalance，导致消费暂停。

**解决方案**：

1. **增加消费者实例数**：确保消费者实例数不超过分区数（每个分区只能被组内一个消费者消费）：
...
public class ConcurrentConsumer {
    ...
    private static void processRecord(ConsumerRecord<String, String> record) {
        // 业务处理逻辑
        System.out.printf("线程=%s, 处理消息: offset=%d, value=%s%n",
...
**问题描述**：部分消息在传输过程中丢失，消费者无法消费到。

**原因分析**：
- 生产者使用 `acks=0` 或 `acks=1`，Leader 副本宕机导致消息丢失。
- `min.insync.replicas` 设置为 1，当仅有的同步副本宕机时数据丢失。
- 消费者自动提交偏移量后，消息处理失败导致消息"丢失"（实际上是被跳过了）。

**解决方案**：

1. 生产者配置 `acks=all` 并开启重试：
...
2. Broker 端配置 `min.insync.replicas >= 2`：
...
### 问题三：`Topic XXX does not exist` 错误

**问题描述**：生产者或消费者连接 Kafka 时报错 `Topic XXX does not exist`。

**原因分析**：
- Topic 未创建，且 Broker 端 `auto.create.topics.enable` 设为 `false`。
- 客户端连接的 `bootstrap.servers` 地址错误，连接到了错误的集群。
- Topic 名称拼写错误。
...
3. 确认 `bootstrap.servers` 配置正确，且指向目标集群。
