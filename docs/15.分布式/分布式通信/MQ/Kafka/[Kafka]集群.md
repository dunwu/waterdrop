...
![](https://raw.githubusercontent.com/dunwu/images/master/archive/2025/02/ef7f4edeb38b4ff6a961046dfc84b16e.png)
...
![](https://raw.githubusercontent.com/dunwu/images/master/archive/2025/02/c2d7a1f00b2e42418c229d421f5d0f2f.png)
...
![](https://raw.githubusercontent.com/dunwu/images/master/archive/2020/06/936497f0706b49158979a0b10689a187.png)
...
![](https://raw.githubusercontent.com/dunwu/images/master/archive/2020/06/3645c8c5a4ae4ead8b8729a88ecdb1da.png)
...
![](https://raw.githubusercontent.com/dunwu/images/master/archive/2020/06/a44bee9ed24342999e272c9d5a45a6de.png)
...
Kafka 集群的分布式架构适用于以下场景：

| 场景 | 说明 | 集群规模建议 |
| --- | --- | --- |
| **高可用消息队列** | 通过副本机制保证消息不丢失，支持故障自动切换 | 3 节点起步（容忍 1 节点宕机） |
| **大数据流处理** | 支撑高吞吐量的数据管道，对接 Spark/Flink | 根据数据量横向扩展 |
| **日志聚合平台** | 收集多机房、多应用日志，统一处理 | 机房内 3-5 节点，跨机房可扩展 |
| **事件溯源** | 持久化领域事件，支持事件回放 | 副本数 3，开启持久化 |
| **多机房容灾** | 通过 MirrorMaker 实现跨机房数据复制 | 每机房独立集群 |

## 最佳实践

### 案例一：部署 3 节点 Kafka 集群

3 节点是最小生产可用集群，可以容忍 1 个节点宕机。

各节点 `server.properties` 配置：
...
```bash
# 查看集群中的 broker 列表
bin/zookeeper-shell.sh zk1:2181/kafka ls /brokers/ids
...
随着数据量增长，需要扩容分区或迁移副本以平衡负载。

**分区扩容**（只能增加，不能减少）：

```bash
bin/kafka-topics.sh --bootstrap-server kafka1:9092 \
  --alter --topic busy-topic --partitions 12
```

**副本迁移**（将 topic 迁移到新增 broker）：

创建迁移计划文件 `reassignment.json`：
...
### 案例三：Preferred Leader 选举

Preferred Leader 是创建 Topic 时指定的第一个副本。当 Leader 偏离 Preferred Leader 时，会导致部分 Broker 负载不均衡。

触发 Preferred Leader 选举：

```bash
# 对所有 topic 执行 Preferred Leader 选举
bin/kafka-leader-election.sh --bootstrap-server kafka1:9092 \
  --election-type PREFERRED --all-topic-partitions
```

查看哪些分区 Leader 不是 Preferred：

```bash
bin/kafka-topics.sh --bootstrap-server kafka1:9092 --describe | \
  grep -v "Leader:.*Isr" | awk '{
    if ($5 != $7) print $0
  }'
```

> 建议在低峰期执行 Preferred Leader 选举，避免影响生产服务。

## 常见问题

### 问题一：Broker 宕机后部分分区不可用

**问题描述**：某个 Broker 宕机后，部分分区显示 `Leader: none`，生产者和消费者无法读写这些分区。

**原因分析**：
- 分区的所有副本都集中在一个 Broker 上，该 Broker 宕机后没有可用副本。
- `min.insync.replicas` 设置过高，剩余同步副本数不足。
- `unclean.leader.election.enable=false` 且没有同步副本可用。
...
```bash
bin/kafka-topics.sh --bootstrap-server kafka1:9092 --describe --topic my-topic
```

2. 如果副本分布不均匀，使用 `kafka-reassign-partitions` 工具重新分配副本，确保每个分区至少有 3 个副本分布在不同的 Broker 上。

3. 紧急恢复时，可以临时开启 Unclean Leader 选举（可能丢失少量数据）：
...
### 问题二：Controller 故障导致集群无响应

**问题描述**：Controller 所在 Broker 宕机后，集群长时间无法选举出新 Controller，导致无法创建 Topic、分区无法切换 Leader。

**原因分析**：
- ZooKeeper 集群不可用或响应缓慢。
- 网络分区导致 Broker 与 ZooKeeper 断开连接。
- 多个 Broker 同时竞争 Controller 节点，发生"脑裂"。
...
# 查看 Controller 节点
bin/zookeeper-shell.sh zk1:2181/kafka get /controller
```

2. 如果 Controller 节点残留旧数据，手动删除以触发重新选举：

```bash
bin/zookeeper-shell.sh zk1:2181/kafka delete /controller
```

3. 检查 Broker 与 ZooKeeper 的网络连通性：
...
**问题描述**：`UnderReplicatedPartitions` 指标持续大于 0，部分分区同步副本数不足，导致消息写入失败。

**原因分析**：
- Follower 副本同步速度慢，落后 Leader 超过 `replica.lag.time.max.ms`（默认 10s）。
- Broker 间网络延迟高或带宽不足。
- Broker 磁盘 I/O 瓶颈，写入速度跟不上。
- Follower 所在 Broker GC 频繁。

**解决方案**：

1. 监控 UnderReplicatedPartitions 指标：
...
2. 适当增大 `replica.lag.time.max.ms`，给 Follower 更多同步时间：
...
3. 排查 Follower 所在 Broker 的性能瓶颈。检查磁盘 I/O：
...
```bash
# 在 Leader 所在 Broker 上测试到 Follower 的带宽
iperf3 -c follower-host -p 5201
```
