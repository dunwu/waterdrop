...
Kafka 生产者在实际业务中有以下典型应用场景：

| 场景 | 说明 | 推荐配置 |
| --- | --- | --- |
| **日志采集** | 应用日志异步写入 Kafka | `acks=1`，异步发送，开启压缩 |
| **订单消息** | 电商订单消息投递 | `acks=all`，幂等性，重试 |
| **用户行为上报** | 埋点数据上报 | `acks=1`，批量发送，`linger.ms` 调大 |
| **金融交易** | 支付、转账类消息 | `acks=all`，事务，`min.insync.replicas=2` |
| **IoT 数据采集** | 物联网设备传感器数据 | `acks=1`，压缩，批量发送 |
| **CDC 数据同步** | 数据库变更数据捕获 | `acks=all`，事务保证一致性 |

## 最佳实践

### 案例一：高可靠生产者配置（金融场景）

金融场景下，消息可靠性高于一切，需要同时使用幂等性和事务：
...
public class ReliableTransactionProducer {
    ...
    public static void main(String[] args) {
        ...
        // 事务配置
        props.put(ProducerConfig.TRANSACTIONAL_ID_CONFIG, "tx-payment-producer-1");
        ...
        try (KafkaProducer<String, String> producer = new KafkaProducer<>(props)) {
            ...
            try {
                producer.beginTransaction();
                producer.send(new ProducerRecord<>("payment-topic", "order-001", "支付成功"));
                producer.send(new ProducerRecord<>("notification-topic", "order-001", "通知用户支付成功"));
                producer.commitTransaction();
...
日志采集场景下，吞吐量优先于可靠性，可以牺牲少量消息换取更高性能：
...
public class HighThroughputProducer {
    ...
    public static void main(String[] args) {
        ...
        props.put(ProducerConfig.LINGER_MS_CONFIG, 20);  // 等待 20ms 凑批
        props.put(ProducerConfig.BUFFER_MEMORY_CONFIG, 67108864);  // 64MB
        props.put(ProducerConfig.SEND_BUFFER_CONFIG, 131072);  // 128KB TCP 发送缓冲

        try (KafkaProducer<String, String> producer = new KafkaProducer<>(props)) {
            for (int i = 0; i < 100000; i++) {
                String logLine = String.format(
                    "{\"ts\":%d,\"level\":\"INFO\",\"msg\":\"log entry %d\"}",
                    System.currentTimeMillis(), i);
                producer.send(new ProducerRecord<>("app-logs", null, logLine));
            }
            producer.flush();
...
当默认的分区策略无法满足业务需求时，可以实现自定义分区器。例如，按地域将消息分发到不同分区：
...
public class RegionPartitioner implements Partitioner {
    ...
    public int partition(String topic, Object key, byte[] keyBytes,
        ...
        // key 为地域代码，如 "north", "south", "east", "west"
        if (key == null) {
            return 0;
        }
        ...
        switch (region) {
            ...
            default:
                // 其余地域使用 hash 分配到剩余分区
                return Math.abs(key.hashCode()) % (numPartitions - 4) + 4;
        }
    }
...
producer.send(new ProducerRecord<>("orders", "north", "北京订单数据"));
producer.send(new ProducerRecord<>("orders", "south", "深圳订单数据"));
```
...
**问题描述**：生产者发送消息时报 `TimeoutException: Expiring 1 record(s) for xxx:30000 ms has passed since batch creation`。

**原因分析**：
- Broker 不可达或网络不通。
- Broker 端 `message.max.bytes` 限制，消息过大被拒绝。
- 生产者缓冲区已满，新消息无法写入缓冲区。
- Broker 端负载过高，响应缓慢。
...
```java
props.put(ProducerConfig.REQUEST_TIMEOUT_MS_CONFIG, 60000);  // 请求超时 60s
props.put(ProducerConfig.DELIVERY_TIMEOUT_MS_CONFIG, 120000);  // 发送总超时 120s
props.put(ProducerConfig.RETRIES_CONFIG, 5);
props.put(ProducerConfig.MAX_BLOCK_MS_CONFIG, 60000);  // 缓冲区满时阻塞等待时间
```
...
**问题描述**：同一 key 的消息消费时发现顺序错乱。

**原因分析**：
- 生产者开启重试后，如果 `max.in.flight.requests.per.connection > 1`，第一批消息失败重试时，第二批可能已经成功写入，导致乱序。
- 消费者端多线程消费未按 key 分区。

**解决方案**：

1. 如果不需要幂等性，设置 `max.in.flight.requests.per.connection=1`：

```java
props.put(ProducerConfig.MAX_IN_FLIGHT_REQUESTS_PER_CONNECTION, 1);
```

2. 如果需要同时保证顺序和幂等性（Kafka 0.11+），开启幂等性即可：

```java
props.put(ProducerConfig.ENABLE_IDEMPOTENCE_CONFIG, true);
// 幂等性开启后，max.in.flight.requests.per.connection 最大可设为 5
props.put(ProducerConfig.MAX_IN_FLIGHT_REQUESTS_PER_CONNECTION, 5);
```

3. 确保消息携带 key，使相同 key 的消息进入同一分区：
...
**问题描述**：生产者长时间运行后发生 OOM。

**原因分析**：
- `buffer.memory` 设置过大，超过了 JVM 堆内存。
- 生产者发送速度远大于 Broker 消费速度，缓冲区持续膨胀。
- 未正确关闭生产者，导致资源泄漏。

**解决方案**：

1. 合理设置缓冲区大小，不超过 JVM 堆内存的 1/3：
...
```java
KafkaProducer<String, String> producer = new KafkaProducer<>(props);
// 获取 Producer 的 metrics
Map<org.apache.kafka.common.MetricName, ? extends org.apache.kafka.common.Metric> metrics =
    producer.metrics();
metrics.forEach((name, metric) -> {
    if (name.name().equals("buffer-available-bytes") || name.name().equals("buffer-total-bytes")) {
        System.out.println(name.name() + " = " + metric.value());
    }
});
```

3. 使用 `try-with-resources` 确保生产者正确关闭：
