...
因为它按「确认即删」设计，堆积的消息会持续占用磁盘且索引结构对大量未确认消息不友好，性能随堆积量显著下降。海量堆积场景应选择 Kafka 或 RocketMQ，详见本文档『Kafka、RocketMQ、RabbitMQ 如何选型？』。
...
> 🎯 目标等级：L2 ｜ ⏱ 建议用时：10 min ｜ 🏷 标签：MQ / 技术选型

#### 💎 关键结论

选型看三件事：吞吐与堆积、事务与延迟、路由灵活性。日志流/大数据选 Kafka，交易核心选 RocketMQ，复杂路由/中小业务选 RabbitMQ。理由：三者的设计目标分别是“吞吐极致”“业务功能完备”“路由与易用”。

#### ⚡记忆卡片

**口诀**：日志用 Kafka，交易用 Rocket，路由用 Rabbit
**关键词**：高吞吐／事务消息／低延迟／灵活路由／堆积能力
**链路**：明确业务场景（日志/交易/集成）→ 对比吞吐、延迟、事务、路由 → 结合团队生态与运维能力选型
...
> 🎯 目标等级：L2 ｜ ⏱ 建议用时：10 min ｜ 🏷 标签：MQ / 架构对比

#### 💎 关键结论

Kafka 为流式吞吐而生，RocketMQ 为业务消息而生：前者分区日志+ZooKeeper/KRaft 元数据、生态强；后者 CommitLog 混存+NameServer 轻量协调、业务功能开箱即用。理由：注册中心、存储模型、消息类型三个维度的差异，分别对应“吞吐优先”与“业务优先”的设计目标。

#### ⚡记忆卡片

**口诀**：K 流式生态强，R 业务功能全
**关键词**：NameServer／ZooKeeper／CommitLog／事务消息／延迟消息／消息回溯
**链路**：Kafka 分区日志拼吞吐 → 生态对接流计算；RocketMQ 混存+轻量注册中心 → 事务/延迟/过滤开箱即用 → 按场景选型
...
| **延迟消息** | 内置 18 个固定延迟级别（4.x）          | 需外部实现（如流处理或自定义调度）     |
...
- 批量发送/拉取 + 顺序追加写 + 页缓存 + 零拷贝（sendfile），把磁盘 I/O 压到最低；分区并行让吞吐随节点数线性扩展。
- RocketMQ 用 mmap 读写 CommitLog，单机十万级 TPS 已能满足绝大多数交易场景；两者差距主要在极限吞吐与海量分区场景。
...
**为什么 RocketMQ 用 NameServer 而不用 ZooKeeper？**
NameServer 无状态、各自独立、AP 模型，部署运维极简，满足路由发现这类“最终一致即可”的需求；ZooKeeper 的强一致选举对消息路由场景是过重依赖，详见 RocketMQ 相关面试题中的 NameServer 专题（见 [RocketMQ 面试](./RocketMQ/[RocketMQ]面试.md)）。

**两者都支持事务，能互相替代吗？**
不能：Kafka 事务面向“内部消息原子写入+精确一次”，RocketMQ 事务消息面向“本地业务事务与消息投递的原子性（半消息+回查）”，解决的是不同层面的问题，见本文档『MQ 如何实现分布式事务？』。
