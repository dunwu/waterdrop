...
A：见《MQ面试》『Kafka、RocketMQ、RabbitMQ 如何存储数据？』。核心是单 CommitLog 顺序写 + 异步构建 ConsumeQueue/IndexFile 多索引，写入性能不受 Topic 数量影响。
...
- 两道防线组合：Redis `SETNX` 前置初筛扛住高并发重复，DB 唯一约束做最终兑底；两层的键都必须是业务唯一键，见《MQ面试》『如何保证 MQ 消息不重复？』。
