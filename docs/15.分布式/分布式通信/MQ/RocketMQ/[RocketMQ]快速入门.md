...
RocketMQ 采用「`NameServer` + `Broker` + `Producer` + `Consumer`」的四角色架构，NameServer 负责路由发现，Broker 负责消息存储与转发：
...
RocketMQ 的消息存储采用 `CommitLog` + `ConsumeQueue` + `IndexFile` 三级结构，所有主题的消息顺序写入同一个 `CommitLog` 文件，再异步构建消费队列索引：

```mermaid
flowchart LR
    A[Producer 消息] --> B[CommitLog<br/>所有 Topic 共享]
    B --> C[ReputService<br/>异步构建线程]
    C --> D[ConsumeQueue<br/>逻辑消费队列]
    C --> E[IndexFile<br/>按 Key 索引]
    F[Consumer] --> D
    D --> B
    G[按 Key 查询] --> E
    E --> B
...
| 文件 | 作用 | 特点 |
| --- | --- | --- |
| `CommitLog` | 存储所有消息体，每个文件 1GB | 顺序写，性能极高 |
| `ConsumeQueue` | 逻辑消费队列，存储消息在 CommitLog 的偏移量 | 每个 Topic+Queue 一个 |
| `IndexFile` | 按 MessageKey 建立哈希索引 | 支持按 Key 快速查询 |
...
sequenceDiagram
    ...
    P->>NS: 查询 Topic 路由信息
    NS-->>P: 返回 Broker + Queue 列表
    P->>P: 根据策略选择目标 Queue
    P->>B: 发送消息（含 Topic/Tag/Key/Body）
    B->>B: 写入 CommitLog
    B->>B: 异步构建 ConsumeQueue
    B-->>P: 返回 SEND_OK + msgId + queueId + queueOffset
```
...
sequenceDiagram
    ...
    C->>NS: 查询 Topic 路由
    NS-->>C: 返回 Broker + Queue 列表
    C->>C: 负载均衡分配 Queue
    C->>B: 拉取消息（按 ConsumeQueue offset）
    B->>B: 根据 ConsumeQueue 定位 CommitLog
    B-->>C: 返回消息列表
    C->>C: 业务处理
    C->>B: 提交消费进度（ack offset）
```
...
- **电商交易**：订单创建、支付回调、库存扣减等核心交易链路，要求高可靠与顺序消费。
- **金融风控**：交易流水实时采集，配合流计算引擎进行欺诈检测。
- **日志收集**：作为统一日志管道，替代 Kafka 处理日志聚合场景，支持海量消息堆积。
- **分布式事务**：利用事务消息实现微服务间的最终一致性，如订单与积分服务的跨库事务。
- **延迟任务**：订单超时关闭、定时提醒等场景，使用延迟消息替代定时任务。
- **广播通知**：配置变更、消息推送等广播消费场景，所有实例都收到全量消息。
- **顺序消息**：订单状态流转（创建→支付→发货）需严格保序，使用分区顺序消息。
...
**场景**：订单服务创建订单后需通知积分服务加积分，要求「订单成功则积分必加，订单失败则积分不加」。
...
public class OrderTransactionProducer {
    public static void main(String[] args) throws Exception {
        ...
        producer.setTransactionListener(new TransactionListener() {
            ...
            @Override
            public TransactionStatus checkLocalTransaction(MessageExt msg) {
        ...
        Message msg = new Message("order-topic", "TagA",
            orderId,
            ("订单消息:" + orderId).getBytes(RemotingHelper.DEFAULT_CHARSET));
        // 发送事务消息
    ...
    private static void saveOrder(String orderId) { /* 写库逻辑 */ }
    private static boolean orderExists(String orderId) { return true; }
}
```

**说明**：事务消息通过「半消息 + 本地事务 + 回查」机制实现最终一致性；`checkLocalTransaction` 是兜底，Broker 在长时间未收到提交/回滚时会主动回查。

### 案例二：顺序消息（订单状态流转）

**场景**：同一订单的创建、支付、发货消息必须按顺序消费。
...
public class OrderedProducer {
    public static void main(String[] args) throws Exception {
        ...
        String[] tags = {"create", "pay", "ship"};
        // 同一 orderId 的消息发往同一 Queue
        for (int i = 0; i < 10; i++) {
            String orderId = "ORD" + (i % 3);  // 3 个订单
            for (int j = 0; j < tags.length; j++) {
                Message msg = new Message("order-seq-topic", tags[j],
                    orderId.getBytes(RemotingHelper.DEFAULT_CHARSET));
                // 关键：使用 MessageQueueSelector 按 orderId 路由到固定 Queue
                SendResult result = producer.send(msg, (mqs, m, arg) -> {
                    String key = (String) arg;
                    int index = Math.abs(key.hashCode()) % mqs.size();
                    return mqs.get(index);
                }, orderId);
                System.out.printf("orderId=%s, tag=%s, queue=%d, offset=%d%n",
                    orderId, tags[j],
...
public class OrderedConsumer {
    public static void main(String[] args) throws Exception {
        ...
        consumer.setConsumeFromWhere(ConsumeFromWhere.CONSUME_FROM_FIRST_OFFSET);
        consumer.subscribe("order-seq-topic", "create || pay || ship");
        // 关键：使用 MessageListenerOrderly 保证单 Queue 顺序消费
        consumer.registerMessageListener((MessageListenerOrderly) (msgs, context) -> {
            for (MessageExt msg : msgs) {
                System.out.printf("线程=%s, queue=%d, tag=%s, body=%s%n",
                    Thread.currentThread().getName(),
                    msg.getQueueId(), msg.getTags(), new String(msg.getBody()));
            }
            return ConsumeOrderlyStatus.SUCCESS;
        });
        consumer.start();
...
**说明**：顺序消息核心是「发送端按 key 路由到同一 Queue + 消费端用 `MessageListenerOrderly` 串行消费」；`MessageListenerOrderly` 会自动加锁，保证同一 Queue 单线程消费。

### 案例三：延迟消息（订单超时关闭）

**场景**：下单后 30 分钟未支付，自动关闭订单。
...
public class DelayMessageProducer {
    public static void main(String[] args) throws Exception {
        ...
        Message msg = new Message("order-timeout-topic", "TagA",
            "ORD123456",
            "订单超时检查".getBytes(RemotingHelper.DEFAULT_CHARSET));
        // 设置延迟级别：18=2h，16=2m，14=10m，13=5m
        // 开源版支持 18 个级别，默认 "1s 5s 10s 30s 1m 2m 3m 4m 5m 6m 7m 8m 9m 10m 20m 30m 1h 2h"
        msg.setDelayTimeLevel(16);  // 2 分钟后投递
        SendResult result = producer.send(msg);
        System.out.printf("发送延迟消息: msgId=%s, delayLevel=%d%n",
            result.getMsgId(), 16);
        producer.shutdown();
    }
}
...
public class DelayMessageConsumer {
    public static void main(String[] args) throws Exception {
        ...
        consumer.setNamesrvAddr("localhost:9876");
        consumer.subscribe("order-timeout-topic", "TagA");
    ...
    private static boolean isOrderPaid(String orderId) { return false; }
    private static void closeOrder(String orderId) { /* 关单逻辑 */ }
}
```

**说明**：开源版 RocketMQ 延迟消息仅支持固定级别（1s~2h）；RocketMQ 5.x 支持任意时间延迟（`messageDelayLevel` 配合 `deliverTime`）。

## 常见问题

### 问题一：消费堆积（消息积压）

**问题描述**：消费者处理速度跟不上生产者，消息持续堆积，Broker 磁盘告警。

**原因分析**：
1. 消费者实例数 < Queue 数，并发度不足。
2. 消费逻辑慢（如外部接口超时、慢 SQL）。
3. 消费者频繁 Rebalance，处理中断。
4. 消费者 OOM 或 Full GC 停顿。

**解决方案**：

```java
// 1. 提升消费并发度：增加 consumeThreadMin/Max
...
```java
// 2. 临时扩容：增加消费者实例（实例数不超过 Queue 数）
// 假设 Topic 有 16 个 Queue，则最多 16 个消费者实例并发
// 若仍不够，需先扩 Queue 数量
```

```shell
# 3. 扩展 Queue 数量
mqadmin updateTopic -b localhost:10911 -t problem-topic -r 32 -w 32
```

```java
// 4. 降级方案：堆积严重时，转储到新 Topic 用更多消费者处理
// 简单消费者转发
consumer.registerMessageListener((MessageListenerConcurrently) (msgs, context) -> {
    for (MessageExt msg : msgs) {
        // 转发到新 Topic（分区更多）
        producer.send(new Message("problem-topic-v2", msg.getTags(), msg.getKeys(), msg.getBody()));
    }
    return ConsumeConcurrentlyStatus.CONSUME_SUCCESS;
});
```

**说明**：根本解法是「扩 Queue 数 + 扩消费者实例 + 优化消费逻辑」三者结合；临时方案可转发到新 Topic 应急。

### 问题二：消息丢失（Producer 发送失败）

**问题描述**：生产环境偶发消息丢失，日志显示 `SEND_FAIL` 或超时。

**原因分析**：
1. 同步发送未配置重试，或异步发送回调未处理异常。
2. Broker 刷盘策略为异步（ASYNC_FLUSH），宕机时未刷盘消息丢失。
3. 主从同步为异步（ASYNC_MASTER），主节点宕机未同步消息丢失。
4. 网络抖动导致请求超时。
...
try {
    ...
    if (result.getSendStatus() != SendStatus.SEND_OK) {
        saveToCompensationTable(msg);  // 落库补偿
    }
} catch (Exception e) {
    saveToCompensationTable(msg);
}
```

```properties
# Broker 配置：高可靠场景使用同步刷盘 + 同步双写
flushDiskType=SYNC_FLUSH        # 同步刷盘
brokerRole=SYNC_MASTER          # 主从同步双写
```

**说明**：金融场景推荐 `SYNC_FLUSH` + `SYNC_MASTER`，性能损失约 30% 但可靠性最高；普通业务可用 `ASYNC_FLUSH` + `ASYNC_MASTER` + 消息补偿表兜底。

### 问题三：消费者 Rebalance 频繁导致消费中断

**问题描述**：消费者组频繁触发 Rebalance，消费停顿，消息延迟飙升。

**原因分析**：
1. 消费者心跳超时（`consumeTimeout` 或网络抖动）。
2. 消费处理耗时过长，超过 `consumeTimeout` 被判定为「死亡」。
3. 消费者实例频繁上下线（如 K8s 滚动更新）。
4. NameServer 与 Broker 心跳异常。
...
consumer.setNamesrvAddr("localhost:9876");
// 消费超时调大（默认 15 分钟）
consumer.setConsumerTimeout(30 * 60 * 1000L);
// 消费线程数调大，避免单消息阻塞
consumer.setConsumeThreadMin(20);
consumer.setConsumeThreadMax(64);
// 单次消费消息数减少，控制单批耗时
...
```java
// 优雅退出：注册 ShutdownHook 主动注销
Runtime.getRuntime().addShutdownHook(new Thread(() -> {
    consumer.shutdown();
    System.out.println("消费者优雅退出");
}));
```

```java
// 幂等消费兜底：Rebalance 可能导致重复消费
consumer.registerMessageListener((MessageListenerConcurrently) (msgs, context) -> {
    for (MessageExt msg : msgs) {
        String msgKey = msg.getKeys();
        // 用 Redis SETNX 或数据库唯一键保证幂等
        if (!redisTemplate.opsForValue().setIfAbsent("consumed:" + msgKey, "1", 24, TimeUnit.HOURS)) {
            continue;  // 已消费，跳过
        }
        processBusiness(msg);
    }
...
**说明**：Rebalance 期间消息会重复投递，业务必须幂等；`consumerTimeout` 应大于单条消息最长处理时间的 2 倍。
