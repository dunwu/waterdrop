...
- **时间序列数据**：存储 IoT 设备、日志、监控等时间序列数据，支持高写入叶量和快速时间范围查询。
- **社交网络数据**：存储用户动态、评论、关注关系等，支持高并发读写和水平扩展。
- **订单与交易数据**：存储电商订单、金融交易等数据，支持高可用和跨地域复制。
- **物联网数据**：存储传感器数据、设备状态等，支持海量数据写入和快速查询。

## 最佳实践

- **合理设计分区键**：根据查询模式设计分区键，避免热点分区；使用组合分区键分散数据。
- **控制分区大小**：单个分区不宜过大（建议 < 100MB），避免查询时扫描过多数据。
- **避免反模式**：避免无索引查询、大分区、过多 TTL 等反模式，影响性能和稳定性。
- **定期维护**：执行 nodetool repair、compaction 等维护操作，保持集群健康。

## 常见问题

**Cassandra 和 HBase 如何选择？**

Cassandra 适合写多读少、高可用场景，支持跨地域复制；HBase 适合读多写少、复杂查询场景，与 Hadoop 生态集成更好。

**为什么 Cassandra 不支持 JOIN？**

Cassandra 是分布式数据库，JOIN 操作会导致大量数据在网络间传输，性能很差。建议通过数据建模（如宽表、物化视图）避免 JOIN。
