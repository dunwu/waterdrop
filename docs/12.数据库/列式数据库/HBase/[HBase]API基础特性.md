...
![](https://raw.githubusercontent.com/dunwu/images/master/archive/2023/03/6f0b2ddbf9564892899e0f22c6f652a2.png)

Connection 对象和实际的 Socket 连接之间的对应关系如下图：

![](https://raw.githubusercontent.com/dunwu/images/master/archive/2023/03/3ee00998d3064952a69b0e5cf4888ab7.png)
...
- **海量数据存储**：通过 Java API 实现海量数据的批量导入、查询、更新和删除。
- **实时数据分析**：使用 Scan 和 Filter 实现复杂的数据查询和分析。
- **数据迁移与同步**：通过 Get/Put/Delete API 实现数据在不同系统间的同步和迁移。
- **自定义业务逻辑**：结合 HBase API 实现自定义的业务逻辑，如计数器、时间线等。
...
常见原因：1) 未设置过滤器导致全表扫描；2) setCaching 设置过小导致频繁 RPC；3) 未设置 setBatch 导致单次返回数据过多。

**Get 和 Scan 如何选择？**

Get 用于获取单行数据，性能更好；Scan 用于获取多行数据，支持范围查询和过滤器。获取单行用 Get，获取多行用 Scan。
