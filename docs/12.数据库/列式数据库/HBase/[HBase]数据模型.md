...
- **日志数据存储**：使用 Rowkey 包含时间戳，存储服务器日志、应用日志等时序数据。
- **用户画像存储**：使用用户 ID 作为 Rowkey，存储用户的多个属性（如姓名、年龄、兴趣等）。
- **订单数据存储**：使用订单 ID 作为 Rowkey，存储订单详情、状态、物流等信息。
- **设备监控数据**：使用设备 ID + 时间戳作为 Rowkey，存储设备的传感器数据和状态信息。

## 最佳实践

- **Rowkey 设计原则**：避免热点（使用盐值、哈希前缀）、控制长度（建议 10-100 字节）、考虑查询模式。
- **列族数量控制**：列族数量建议 1-3 个，过多列族会影响性能。
- **合理设置版本数**：根据业务需求设置 VERSIONS，避免过多版本占用存储空间。
- **利用稀疏性**：HBase 的稀疏性允许表设计得非常灵活，不必为每个 Cell 都填充数据。

## 常见问题

**HBase 和关系型数据库的区别？**

HBase 是列式存储、支持海量数据、水平扩展；关系型数据库是行式存储、支持复杂查询和事务。HBase 适合大数据场景，关系型数据库适合 OLTP 场景。

**为什么 HBase 不支持 SQL？**

HBase 设计目标是海量数据存储和快速读写，不支持复杂查询和事务。如果需要使用 SQL，可以通过 Phoenix 或 Hive on HBase 实现。
