...
- **表管理**：使用 create、list、describe、disable、drop 等命令管理表的生命周期。
- **数据操作**：使用 put、get、scan、delete 等命令进行数据的增删改查。
- **集群运维**：使用 status、version、whoami 等命令查看集群状态和版本信息。
- **调试与测试**：使用 Shell 命令快速验证数据、测试查询逻辑，辅助开发和调试。

## 最佳实践

- **批量操作使用 scan 过滤器**：通过 FILTER 参数实现复杂筛选，避免全表扫描。
- **合理设置缓存大小**：scan 时使用 CACHE 参数控制每次返回的行数，平衡性能和内存。
- **避免大事务**：Shell 中避免一次性执行大量操作，分批处理。
- **定期清理数据**：使用 delete 或 truncate 清理过期数据，释放存储空间。

## 常见问题

**如何查看表的结构？**

使用 describe 'table_name' 命令查看表的列族、版本数、TTL 等配置信息。

**如何批量删除数据？**

使用 scan + deleteall 命令，或使用 truncate 命令清空整表数据。注意：批量删除会影响性能，建议在低峰期执行。
