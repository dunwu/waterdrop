...
- **时间序列数据建模**：使用 Rowkey 包含时间戳，支持时间范围查询和数据归档。
- **社交网络数据建模**：使用用户 ID 作为 Rowkey，存储用户动态、关注关系等。
- **物联网数据建模**：使用设备 ID + 时间戳作为 Rowkey，存储传感器数据。
- **日志数据建模**：使用主机名 + 日志类型 + 时间戳作为 Rowkey，支持日志查询和分析。

## 最佳实践

- **Rowkey 设计原则**：避免热点（使用盐值、哈希前缀）、控制长度（建议 10-100 字节）、考虑查询模式。
- **列族数量控制**：列族数量建议 1-3 个，过多列族会影响性能。
- **合理设置 TTL**：根据数据生命周期设置 TTL，自动清理过期数据。
- **避免反模式**：避免大 Cell、过多版本、全表扫描等反模式。

## 常见问题

**为什么 Rowkey 不能单调递增？**

单调递增的 Rowkey 会导致数据集中在同一个 Region，造成热点。建议使用盐值、哈希前缀或反转时间戳等方式分散数据。

**HBase 适合什么场景？**

HBase 适合海量数据存储、随机读写、高并发场景，如日志、时序数据、社交网络等。不适合复杂查询、事务操作、JOIN 等场景。
