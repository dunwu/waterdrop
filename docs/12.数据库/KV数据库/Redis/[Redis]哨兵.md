...
![](https://raw.githubusercontent.com/dunwu/images/master/archive/2025/03/07a6cfd0e3bf4a1e8b694e0ee9c2e0a8.png)
...
![](https://raw.githubusercontent.com/dunwu/images/master/archive/2025/03/a8fbb3526acb4bdcaee4abd9cef2633a.png)
...
![](https://raw.githubusercontent.com/dunwu/images/master/archive/2025/03/50d68df3184243b5a9b530e841019d75.png)

（3）**将旧的主服务器变为从服务器**

Sentinel Leader 将旧的主服务器标记为从服务器。当旧的主服务器重新上线，Sentinel 会向它发送 `SLAVEOF` 命令，让其成为从服务器。

![](https://raw.githubusercontent.com/dunwu/images/master/archive/2025/03/518e688692494ad4a72d2d6d4ce9ae1a.png)

## 应用场景

- **自动故障转移**：主节点宕机时，Sentinel 自动选举新主节点，无需人工干预，保证服务高可用。
- **监控告警**：Sentinel 持续监控 Redis 实例健康状态，主节点不可达时触发告警通知（邮件、短信、Webhook）。
- **客户端服务发现**：客户端连接 Sentinel 获取当前主节点地址，主从切换后自动重连新主节点，无需修改配置。
- **读负载均衡**：将读请求分散到多个从节点，主节点只处理写请求，提升整体读吐量。

## 最佳实践

- **至少 3 个 Sentinel 节点**：避免脑裂问题，Sentinel 节点数必须为奇数，建议至少 3 个分布在不同的服务器上。
- **合理设置 quorum**：quorum 设置为 Sentinel 总数的一半 + 1，避免网络分区时无法达成共识。
- **down-after-milliseconds 设置**：根据网络状况调整，一般设置为 5-30 秒，避免网络抖动导致误判故障。
- **监控 Sentinel 本身**：Sentinel 也可能宕机，需监控 Sentinel 进程健康状态，确保监控体系可靠。

## 常见问题

**Sentinel 和 Redis Cluster 如何选择？**

Sentinel 适合主从架构，支持自动故障转移但不支持数据分片；Redis Cluster 支持数据分片、高可用、自动故障转移，适合大数据量场景。小规模高可用用 Sentinel，大规模用 Cluster。

**故障转移时间过长怎么办？**

优化 down-after-milliseconds、parallel-syncs 等参数，缩短故障检测和同步时间；确保从节点数据较新，减少同步时间。
