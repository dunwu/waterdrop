...
- **Bitmap 用户行为分析**：利用 Bitmap 记录用户每日登录、签到、活跃状态，实现低成本的用户行为统计。
- **HyperLogLog UV 统计**：统计网站独立访客、商品浏览量等基数，内存占用极低（约 12KB）。
- **GEO 地理位置服务**：实现附近的人、距离计算、地理围栏等功能，如社交应用、外卖配送。
- **Stream 消息队列**：实现轻量级消息队列，支持消费组、消息确认、重试机制，适合简单业务场景。

## 最佳实践

- **Bitmap 控制键长度**：单个 Bitmap 键的 offset 不宜过大，避免内存浪费；offset 范围建议控制在 2^32 以内。
- **HyperLogLog 精度接受**：HyperLogLog 误差约 0.81%，适合对精度要求不高的基数统计，不适用于精确计数。
- **GEO 数据建模**：地理位置数据建议独立存储，避免与其他业务数据混用；定期清理过期数据。
- **Stream 消费组管理**：合理设置消费组数量，及时 ACK 已处理消息，避免消息堆积和内存占用。

## 常见问题

**Bitmap 和 String 的关系？**

Bitmap 本质上是 String 类型的位操作，通过 SETBIT/GETBIT 操作字符串的位。Bitmap 的优势是内存占用极低，适合存储布尔型数据。

**Stream 和 Kafka 的区别？**

Stream 是轻量级消息队列，支持消费组、消息确认，但不支持分区、事务消息、消息压缩等高级特性。简单场景用 Stream，复杂场景用 Kafka。
