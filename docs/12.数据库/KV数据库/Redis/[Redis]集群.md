...
- **大数据量存储**：Redis Cluster 支持数据分片，将数据分散到多个节点，突破单机内存限制。
- **高并发读写**：多节点并行处理请求，提升整体叶量，支撑高并发业务。
- **自动故障转移**：主节点宕机时自动选举新主节点，保证服务高可用，无需人工干预。
- **弹性扩容缩容**：支持在线添加/删除节点，数据自动迁移和重新分布，无需停机。

## 最佳实践

- **合理分片设计**：根据数据访问模式设计分片策略，避免热点节点；使用一致性哈希或 Redis Cluster 的 16384 个槽。
- **监控集群状态**：监控每个节点的内存、连接数、响应时间，及时发现热点节点和故障节点。
- **客户端缓存路由表**：使用 Smart Client（如 Jedis Cluster、Lettuce）缓存路由表，减少重定向开销。
- **避免跨槽操作**：Multi-key 操作要求所有 key 在同一个槽，使用 hash tag 强制路由到同一槽。

## 常见问题

**Redis Cluster 和 Codis 如何选择？**

Redis Cluster 是官方方案，去中心化、支持自动故障转移；Codis 是第三方方案，提供可视化管理和 Proxy 层。中小规模推荐 Redis Cluster，需要可视化管理可考虑 Codis。

**集群扩容时数据迁移失败怎么办？**

检查网络连通性和磁盘空间，使用 redis-cli --cluster reshard 重新触发迁移。如果失败可手动修复或回滚，建议先在测试环境验证扩容流程。
