...
![](https://raw.githubusercontent.com/dunwu/images/master/archive/2020/01/541d0f134a4548a8bbd49261c28eef9d.png)
...
- **异步日志记录**：通过订阅 Redis 的 keyevent 事件（如 key 过期、删除），将关键操作异步写入日志系统，实现审计和监控。
- **缓存失效通知**：监听 key 过期事件，在缓存失效时自动触发数据库重新加载或告警，避免缓存击穿。
- **实时消息推送**：利用发布/订阅模式实现简单的实时消息推送，如聊天室、通知广播等场景。
- **系统解耦**：通过事件机制解耦生产者和消费者，如订单创建后触发库存扣减、积分增加等后续操作。

## 最佳实践

- **合理选择事件类型**：只订阅必要的 keyspace/ keyevent 事件，避免订阅所有事件导致性能下降。
- **避免阻塞操作**：事件处理器中避免执行耗时操作，如需复杂处理应异步派发到工作线程。
- **客户端重连机制**：实现客户端自动重连和重新订阅逻辑，防止网络抖动导致消息丢失。
- **监控事件延迟**：通过 PUBLISH 延迟监控或专用监控工具，及时发现事件处理瓶颈。

## 常见问题

**Redis 事件和消息队列的区别？**

Redis 发布/订阅是 fire-and-forget 模式，消息不持久化，订阅者离线时消息丢失；消息队列（如 Kafka、RabbitMQ）支持消息持久化、重试、顺序保证等。简单通知场景用 Redis 事件，可靠消息传递用专业 MQ。

**为什么 Redis 6.0 多线程对性能提升不大？**

Redis 单线程执行命令已经很快（内存操作 + 单线程无锁），瓶颈在网络 IO 而非命令执行。Redis 6.0 多线程仅用于网络 IO 读写，命令执行仍是单线程，因此对计算密集型场景提升有限。
