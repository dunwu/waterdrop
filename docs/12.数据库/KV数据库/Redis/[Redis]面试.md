...
- **读写采用"单线程"模型**：操作天然具有**原子性**。Redis 6.0 后在网络模块中引入了多线程 I/O 机制。
- **支持两种持久化策略**：**RDB** 和 **AOF**。
...
Redis 从 1.0 到 8.0 经历了六个里程碑版本，核心演进路线为：单机缓存 → 持久化+哨兵 → 集群 → Stream → 多线程 I/O → Functions → 向量搜索。

#### ⚡记忆卡片

- **口诀**：一零单，二八持，三零群，五零流，六零线，七零函
...
- **Q：Redis 7.0 和 8.0 有哪些重要新特性？** → Redis 7.0 引入 Redis Functions 和 Multi-part AOF，8.0 引入向量搜索和 Hash Field 级别 TTL，见本文档「Redis 7.0 有哪些重要新特性？」和「Redis 8.0 有哪些重要新特性？」。
...
- **Q：什么场景下会选 Memcached？** → 当业务仅需简单的 K-V 缓存、无持久化需求、且操作的数据量超过 100k 时，Memcached 的多线程模型性能更高。
...
> 🎯 目标等级：L2-L3 ｜ ⏱ 建议用时：8 min ｜ 🏷 标签：Redis 内存管理 / 过期策略
...
- **Q：Redis 内存不足时怎么办？** → 触发内存淘汰策略，见本文档「Redis 有哪些内存淘汰策略？」。

### 【中等】Redis 有哪些内存淘汰策略？⭐⭐⭐

> 🎯 目标等级：L2-L3 ｜ ⏱ 建议用时：10 min ｜ 🏷 标签：Redis 内存管理 / 淘汰策略
...
```mermaid
graph TB
    A[内存达到 maxmemory] --> B{选择淘汰策略}
    B -->|不淘汰| C[noeviction - 写入报错]
    B -->|淘汰过期键| D{数据分布特征}
    B -->|淘汰所有键| E{数据分布特征}
    D -->|访问频率不均| F[volatile-lru / volatile-lfu]
    D -->|访问频率均匀| G[volatile-random]
    D -->|尽早释放| H[volatile-ttl]
    E -->|访问频率不均| I[allkeys-lru / allkeys-lfu]
    E -->|访问频率均匀| J[allkeys-random]
```
...
- **Q：内存碎片化如何影响淘汰效果？** → 见本文档「Redis 中的内存碎片化是什么？如何进行优化？」。

### 【中等】Redis 持久化时，对过期键会如何处理？⭐⭐

> 🎯 目标等级：L2-L3 ｜ ⏱ 建议用时：8 min ｜ 🏷 标签：Redis 内存管理 / 持久化与过期
...
- **Q：主从复制时过期键如何处理？** → 见本文档「Redis 主从复制时，对过期键会如何处理？」。

### 【中等】Redis 主从复制时，对过期键会如何处理？⭐⭐

> 🎯 目标等级：L2-L3 ｜ ⏱ 建议用时：5 min ｜ 🏷 标签：Redis 内存管理 / 主从与过期
...
- **Q：过期键在持久化时如何处理？** → 见本文档「Redis 持久化时，对过期键会如何处理？」。

### 【中等】Redis 中的内存碎片化是什么？如何进行优化？⭐⭐

> 🎯 目标等级：L2-L3 ｜ ⏱ 建议用时：8 min ｜ 🏷 标签：Redis 内存管理 / 碎片优化
...
- 【L3】碎片率指标解读：< 1.0 表示 Redis 使用了比实际分配更多的内存（可能有 swap）；1.0~1.5 正常范围；> 1.5 需要关注。
- 【L3】主动碎片整理通过 `activedefrag` 开启，Redis 会在后台渐进式地重新分配内存，将碎片率降到可接受范围。

:::

#### 🔀 发散问题

- **Q：如何选择合适的淘汰策略减少碎片？** → 见本文档「Redis 有哪些内存淘汰策略？」。

## Redis 持久化

### 【中等】Redis 支持哪些持久化方式？⭐⭐⭐

> 🎯 目标等级：L2-L3 ｜ ⏱ 建议用时：10 min ｜ 🏷 标签：Redis 持久化 / RDB/AOF
...
graph LR
    ...
    E[AOF 日志] --> F["命令追加到缓冲区"]
    F --> G{appendfsync 策略}
    G -->|always| H[每条命令立即 fsync]
    G -->|everysec| I[每秒 fsync]
    G -->|no| J[由操作系统决定 fsync]
```
...
- 【L3】`BGSAVE` 采用写时复制技术（Copy-On-Write），fork 子进程时不复制实际内存，只在写入时复制修改页。
...
- **Q：AOF 文件过大怎么办？** → 见本文档「AOF 的重写机制是怎样的？」。

### 【中等】AOF 的重写机制是怎样的？⭐⭐⭐

> 🎯 目标等级：L2-L3 ｜ ⏱ 建议用时：10 min ｜ 🏷 标签：Redis 持久化 / AOF 重写
...
```mermaid
graph TB
    A["AOF 文件超过阈值"] --> B["执行 BGREWRITEAOF"]
    B --> C["fork 子进程"]
    C --> D["子进程遍历内存生成新 AOF"]
    C --> E["主线程继续处理命令"]
    E --> F["写命令同时追加到 AOF 缓冲区和重写缓冲区"]
    D --> G["子进程完成新 AOF 文件"]
    G --> H["将重写缓冲区内容追加到新 AOF"]
    H --> I["原子替换旧 AOF 文件"]
```
...
- 【L3】AOF 重写是重建最小命令集：例如对同一个 key 执行 100 次 INCR，重写后只需一条 SET 命令即可还原。
...
- **Q：AOF 重写和 RDB 的 BGSAVE 有什么共同点？** → 两者都通过 fork 子进程实现非阻塞操作，都利用 Copy-On-Write 机制减少内存开销。

## Redis 批处理

### 【中等】Redis 支持事务吗？⭐⭐

> 🎯 目标等级：L2-L3 ｜ ⏱ 建议用时：8 min ｜ 🏷 标签：Redis 批处理 / 事务
...
- **Q：Pipeline 和事务有什么区别？** → Pipeline 不保证原子性，只是批量发送减少 RTT；事务通过 MULTI/EXEC 保证命令连续执行。见本文档「Redis Pipeline 能保证原子性吗？」。

### 【中等】Redis Pipeline 能保证原子性吗？⭐⭐

> 🎯 目标等级：L2-L3 ｜ ⏱ 建议用时：8 min ｜ 🏷 标签：Redis 批处理 / Pipeline
...
- 【L4】Cluster 模式下 Pipeline 的所有 Key 必须在同一个 Slot，可用 `{hash_tag}` 确保路由到同一节点。

:::

#### 🔀 发散问题

- **Q：如何保证 Pipeline 中命令的原子性？** → 使用 Lua 脚本替代，Lua 脚本执行期间不会被其他命令打断。见本文档「Redis Lua 脚本有什么用？」。

### 【中等】Redis Lua 脚本有什么用？⭐⭐

> 🎯 目标等级：L2-L3 ｜ ⏱ 建议用时：5 min ｜ 🏷 标签：Redis 批处理 / Lua
...
- 【L3】Lua 脚本的缺陷：运行时出错中途结束时，已发生的写操作不会撤销，不能实现回滚；Redis Cluster 下无法保证所有 key 在同一 slot，原子性也无法保证。
- 【L4】Redis 7.0 的 Redis Functions 支持持久化存储和版本化管理，解决了 Lua 脚本无法持久化的问题。

:::

#### 🔀 发散问题

- **Q：Lua 脚本和 Pipeline 哪个更快？** → 两者都减少 RTT，但 Lua 脚本额外保证原子性。对于需要原子性的场景优先用 Lua，纯批量发送用 Pipeline。

## Redis 高可用

### 【中等】Redis 如何实现主从复制？⭐⭐⭐⭐

> 🎯 目标等级：L2-L3 ｜ ⏱ 建议用时：12 min ｜ 🏷 标签：Redis 高可用 / 主从复制
...
```mermaid
graph TB
    A["从节点发送 PSYNC"] --> B{主节点判断同步类型}
    B -->|runid 不匹配或 offset 不在积压缓冲区| C["完整重同步 Full Sync"]
    B -->|runid 匹配且 offset 在积压缓冲区内| D["部分重同步 Partial Sync"]
    C --> E["BGSAVE 生成 RDB"]
    E --> F["写命令缓存到复制缓冲区"]
    F --> G["发送 RDB 给从节点"]
    G --> H["从节点清空旧数据并加载 RDB"]
    H --> I["发送复制缓冲区中的增量命令"]
    D --> J["从复制积压缓冲区发送 offset 后的命令"]
    I --> K["进入命令传播阶段"]
    J --> K
```
...
- **一致性的本质**：主从复制是异步的，**Redis 主从架构无法保证不丢数据**，主节点宕机时未传播的写命令会丢失；`WAIT` 命令可实现同步复制语义但会显著降低性能，生产很少使用。
...
- **Q：主从复制和哨兵的关系？** → 哨兵在主从架构基础上实现自动故障转移，见本文档「Redis 哨兵是如何工作的？」。

### 【中等】Redis 哨兵是如何工作的？⭐⭐⭐⭐

> 🎯 目标等级：L2-L3 ｜ ⏱ 建议用时：12 min ｜ 🏷 标签：Redis 高可用 / 哨兵
...
```mermaid
graph TB
    A[哨兵集群] --> B["每 10s 发送 INFO"]
    A --> C["每 1s 发送 PING"]
    B --> D[获取主从节点信息]
    C --> E{节点是否响应?}
    E -->|超时未响应| F[主观下线 SDOWN]
    F --> G{是否为主节点?}
    G -->|是| H["询问其他哨兵"]
    H --> I{"达到 quorum 票数?"}
    I -->|是| J[客观下线 ODOWN]
    I -->|否| K[维持主观下线]
    J --> L["Raft 算法选举 Leader 哨兵"]
    L --> M["Leader 执行故障转移"]
    M --> N["选出从节点提升为主"]
    N --> O["其余从节点认主新主"]
    O --> P["旧主恢复后降级为从"]
...
- 【L4】哨兵选举借鉴 Raft 协议：每个哨兵优先投票给最先发起投票请求的节点，得票超过半数的哨兵成为 Leader。
...
- **Q：脑裂问题如何解决？** → 见本文档「如何解决 Redis 中的脑裂问题？」。

### 【中等】Redis 集群是如何工作的？⭐⭐⭐⭐

> 🎯 目标等级：L2-L3 ｜ ⏱ 建议用时：15 min ｜ 🏷 标签：Redis 高可用 / 集群
...
```mermaid
graph TB
    A[Redis Cluster] --> B["虚拟哈希槽分区"]
    A --> C["主从复制"]
    A --> D["Raft 式故障转移"]
    A --> E["Gossip 协议"]
    B --> F["16384 个 Hash Slot"]
    F --> G["CRC16 mod 16384 路由"]
    C --> H["Master-Slave 数据冗余"]
    D --> I["PFAIL → FAIL → 选主"]
    E --> J["节点间状态传播"]
    G --> K{"槽是否属于当前节点?"}
    K -->|是| L["直接执行命令"]
...
- **Q：集群和哨兵的故障转移有什么区别？** → 哨兵是外部进程管理故障转移，集群是节点内部通过 Gossip + 多数派投票自动完成。
...
- **Q：如何解决脑裂问题？** → 见本文档「如何解决 Redis 中的脑裂问题？」。
...
- `min-replicas-to-write`：主节点在至少有指定数量的从节点确认写操作的情况下才执行写操作。通常设为集群的半数以上。
...
- **Q：为什么 Redis 主从复制是异步的？** → Redis 追求高性能，同步复制会显著增加写延迟。`WAIT` 命令可实现同步语义但会牺牲性能。见本文档「Redis 如何实现主从复制？」。
...
- **Q：Redis 6.0 为什么引入多线程？** → 见本文档「Redis 6.0 之后为什么引入了多线程？」。
...
```mermaid
graph TB
    A["Redis 线程模型"] --> B["核心命令处理 - 单线程"]
    A --> C["多线程 I/O - Redis 6.0+"]
    A --> D["后台线程"]
    B --> E["IO 多路复用 epoll/kqueue"]
    B --> F["命令解析"]
    B --> G["数据读写 - 纯内存"]
    C --> H["多线程并行读取客户端请求"]
    C --> I["多线程并行写回响应"]
...
- **Q：Redis 6.0 多线程改变了什么？** → 见本文档「Redis 6.0 之后为什么引入了多线程？」。
...
- **Q：Redis 的单线程模型是怎样的？** → 见本文档「Redis 单线程模式是怎样的？」。
...
- **Q：RedisBloom 模块能做什么？** → 实现布隆过滤器，解决缓存穿透等问题。见文件2「如何使用 Redis 实现布隆过滤器？」。
...
- **Q：Redis 8.0 有哪些进一步的改进？** → 见本文档「Redis 8.0 有哪些重要新特性？」。
...
Redis 8.0（2025 年发布）带来架构级升级，原生支持向量搜索、Hash Field TTL、触发器、查询引擎等 AI 时代新能力。

#### ⚡记忆卡片

- **口诀**：八零向量触发器，Hash 过期查询引擎
- **关键词**：Vector Search ／ Hash Field TTL ／ Trigger ／ Query Engine ／ Set with scores
- **链路**：7.0 功能完善 → 8.0 AI/ML 原生支持 → 向量搜索 + 触发器

#### 📖 核心知识

Redis 8.0 于 2025 年发布，带来了架构级别的重大升级：

- **Vector Search (向量搜索)**：通过 `HNSW` 索引原生支持向量相似性搜索，适用于 AI/ML 场景下的 RAG（检索增强生成）。
- **Hash Field 级别的 TTL**：支持为 Hash 中的单个字段设置过期时间（`HEXPIRE`、`HTTL` 等命令）。
- **Trigger (触发器)**：支持在 Key 变更时自动触发回调函数，类似数据库触发器。
- **Query Engine (查询引擎)**：支持对 Hash 和 JSON 数据创建索引并执行复杂查询。
- **Set with scores**：Set 类型支持为成员设置分数，实现更灵活的集合操作。
- **JSON Path 增强**：JSON 模块原生集成，支持更丰富的 JSON 路径查询和修改操作。

#### 🔀 发散问题

- **Q：Redis 8.0 的向量搜索适用于什么场景？** → AI/ML 场景下的 RAG（检索增强生成），通过 HNSW 索引实现高效的向量相似性搜索。
...
- **Q：Redis 的哪些设计可以借鉴到其他系统？** → 单线程+事件驱动、COW 快照、渐进式 Rehash、惰性删除等设计思想广泛应用于分布式系统。
...
- **Q：Pub/Sub 和 Stream 有什么区别？** → Stream 支持消息持久化、消费者组、ACK 机制，更适合消息队列场景。见文件2「Redis 如何实现消息队列？」。
...
Cache-Aside（先更新 DB 再删缓存）+ TTL 兖底 + 删除失败重试是主流方案，核心链路可叠加 Canal 订阅 binlog 异步兖底。

#### ⚡记忆卡片

- **口诀**：先更新库再删缓存，TTL 兖底重试删
- **关键词**：Cache-Aside ／ 延迟双删 ／ Canal binlog ／ TTL 兖底
- **链路**：更新 DB → 删除缓存 → TTL 兖底 → 删除失败重试
...
```mermaid
graph TB
    A["缓存一致性策略"] --> B["Cache-Aside 旁路缓存"]
    A --> C["延迟双删"]
    A --> D["异步订阅 binlog"]
    B --> E["读: 先读缓存 → 未命中则读 DB 并写缓存"]
    B --> F["写: 先更新 DB → 再删除缓存"]
    C --> G["先删缓存 → 更新 DB → 延迟 N ms → 再删缓存"]
    D --> H["Canal 监听 binlog → 异步删除/更新缓存"]
```

**Cache-Aside (旁路缓存) - 最常用方案**

- **读操作**：先读缓存，命中则返回；未命中则读 DB，写入缓存并设置过期时间。
- **写操作**：先更新数据库，再删除缓存（而非更新缓存）。
- **为什么是“删除”而不是“更新”缓存？**
  - 并发写时，更新缓存可能出现 ABA 问题，导致数据不一致。
  - 懒加载思想：只有下次访问时才更新缓存，避免无效计算。
...
延迟时间的设定应当大于“读请求耗时 + 缓存写入耗时”，确保并发读请求的旧值已写入缓存后被清除。

**异步订阅 binlog - Canal 方案**

- 通过 Canal 等中间件监听 MySQL binlog 变更，异步删除或更新缓存。
- **优点**：业务无侵入，一致性较好。
- **缺点**：架构复杂度高，存在短暂的不一致窗口。

**经典并发场景推演（面试高频追问）**

先更新 DB 再删缓存（Cache-Aside）并非绝对安全，存在一个低概率的并发漏洞：

1. 缓存刚好过期，请求 A 读 DB，读到**旧值**（此时 A 尚未写回缓存）；
2. 请求 B 更新 DB 为**新值**，并成功删除缓存（此时缓存为空，删除无效）；
3. 请求 A 将**旧值**写入缓存——此后缓存中长期是旧值，直到 TTL 到期。

应对手段：

- **兜底靠过期时间**：TTL 是一致性的最终保底，任何方案都无法做到无窗口的强一致，只能缩短不一致窗口。
- **删除缓存失败重试**：删除缓存这步失败（如网络抖动）会导致长期不一致，应将删除操作投递到消息队列重试，或由 Canal 订阅 binlog 兜底删除。
- **串行化**：对该 key 的读写加分布式锁，可彻底消除并发窗口，但牺牲吞吐，仅适合一致性要求极高的少量场景。

**方案选型结论**：绝大多数业务用 **Cache-Aside + 过期时间兖底 + 删除失败重试** 即可；对一致性要求更高的核心链路，叠加 **Canal 订阅 binlog** 做异步兖底。

#### 🏭 实战场景

::: details

生产案例：某电商订单服务，QPS 5000，采用 Cache-Aside + TTL 30min。曾因删除缓存失败导致缓存与 DB 不一致长达 30 分钟。引入删除失败重试（删除操作投递到 MQ 重试 3 次）+ Canal 监听 binlog 异步删除缓存后，不一致窗口缩短到 1 秒内。
...
- ❌ "先更新 DB 再删缓存绝对安全" → 存在低概率并发漏洞（缓存刚好过期 + 读写并发），最终靠 TTL 兖底。
...
- **Q：缓存穿透、击穿、雪崩如何解决？** → 见本文档「如何解决缓存穿透、缓存击穿、缓存雪崩？」。
...
graph TB
    ...
    B --> E["查询不存在的数据"]
    E --> F["解决方案: 布隆过滤器 / 缓存空值"]
    C --> G["热点 Key 过期时大量并发请求"]
    G --> H["解决方案: 互斥锁 / 逻辑过期"]
    D --> I["大量 Key 同时过期"]
    I --> J["解决方案: 随机过期时间 / 多级缓存"]
```
...
- **Q：如何保证缓存与数据库一致性？** → 见本文档「如何保证缓存与数据库的数据一致性？」。
...
- 【L3】慢查询命令的时间复杂度：KEYS O(n)、HGETALL O(n)、LRANGE O(SN)（S 为范围大小）、SMEMBERS O(n)。
...
- **Q：慢查询和大 key 有什么关系？** → 大 key 操作本身就是慢查询的重要来源，见本文档「Redis 中的 Big Key 问题是什么？如何解决？」。
...
- **Q：大 key 和热点 key 有什么区别？** → 大 key 是 value 过大，热点 key 是访问频率过高。见本文档「如何解决 Redis 中的热点 key 问题？」。
...
- **Q：热点 key 打挂节点怎么办？** → 见本文档「秒杀场景下，热点 Key 突发流量把 Redis 节点打挂了，怎么办？」。
...
1. **本地缓存兕底（最有效）**：应用内用 Caffeine 等做秒级本地缓存，99% 的请求不出 JVM，Redis 压力降几个数量级。配合 Redis 6.0+ 的 client-side caching（RESP3 Tracking）实现本地缓存的失效通知。
...
- **Q：如何实现热点 key 自动探测？** → 客户端 SDK 对访问频次做滑动窗口统计，超过阈值自动升级为本地缓存。服务端可用 `redis-cli --hotkeys`（需 maxmemory-policy 为 LFU）。
