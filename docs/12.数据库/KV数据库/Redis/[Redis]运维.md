...
![](https://raw.githubusercontent.com/dunwu/images/master/archive/2020/07/52cd1149e71940818062aeabeb4f8007.png)

执行结束后，查看状态：

![](https://raw.githubusercontent.com/dunwu/images/master/archive/2020/07/52bc8dd467c04857b038c81f971cad91.png)
...
- **生产环境部署**：根据业务规模选择合适架构（单机/主从/集群），配置持久化、备份、监控等运维策略。
- **性能调优与监控**：通过 INFO、SLOWLOG 等命令分析性能瓶颈，调整 maxmemory、tcp-backlog 等参数。
- **数据迁移与扩容**：使用 redis-shake、redis-port 等工具实现数据在线迁移，支持跨版本、跨架构迁移。
- **故障排查与恢复**：通过日志分析、内存分析、慢查询定位问题，配合 RDB/AOF 备份快速恢复数据。

## 最佳实践

- **规范化部署**：使用 Docker 或 Ansible 自动化部署，统一配置文件、启动脚本、监控告警。
- **定期备份与演练**：每天备份 RDB/AOF 到远程存储，定期进行恢复演练，确保备份有效。
- **监控与告警**：监控内存、连接数、响应时间、慢查询、持久化状态等指标，设置合理阈值。
- **安全加固**：设置密码、禁用危险命令、限制网络访问，定期更新版本修复安全漏洞。

## 常见问题

**Redis 内存碎片率高怎么办？**

开启 activedefrag 自动整理，或在低峰期重启实例。检查是否频繁分配/释放内存，优化数据结构选择。

**Redis 连接数过多导致拒绝连接？**

调整 maxclients 参数，优化客户端连接池配置，及时关闭空闲连接。检查是否有连接泄漏。
