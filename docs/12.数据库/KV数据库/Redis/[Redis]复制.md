...
- **读写分离**：主节点处理写请求，从节点处理读请求，提升系统整体读叶量和并发能力。
- **数据备份与容灾**：从节点实时同步主节点数据，主节点故障时可快速切换到从节点，保证数据不丢失。
- **数据分析与报表**：将从节点用于数据分析、报表生成等读多写少的场景，避免影响主节点性能。
- **跨地域部署**：在不同地域部署从节点，实现就近读取，降低跨地域访问延迟。

## 最佳实践

- **避免主节点过载**：从节点数量不宜过多，过多从节点会导致主节点复制压力大，建议不超过 3-5 个。
- **开启 repl-backlog**：设置足够大的 repl-backlog-size，避免网络抖动后触发全量同步，减少带宽消耗。
- **监控复制延迟**：通过 INFO replication 监控 repl_offset 差异，及时发现复制延迟问题。
- **读写分离注意一致性**：写后立即读可能读到从节点的旧数据，对一致性要求高的场景应读主节点。

## 常见问题

**主从复制延迟大怎么办？**

常见原因：1) 网络带宽不足，考虑压缩或优化网络；2) 主节点写操作过于密集，考虑分片；3) repl-backlog 过小导致频繁全量同步，增大 repl-backlog-size；4) 从节点磁盘 IO 慢，升级磁盘或使用 SSD。

**主节点宕机后数据丢失怎么办？**

Redis 主从复制是异步的，主节点宕机时可能有少量数据未同步到从节点。解决方案：1) 开启 min-slaves-to-write 保证至少有一个从节点同步；2) 使用 AOF 持久化；3) 接受少量数据丢失的 eventual consistency。
