...
- **分布式锁原子操作**：使用 Lua 脚本实现“判断 key 是否存在并设置值”的原子操作，避免并发竞争。
- **限流与计数器**：通过 Lua 脚本实现滑动窗口限流、令牌桶算法，保证原子性和性能。
- **复杂业务逻辑**：将多个 Redis 命令封装成 Lua 脚本，实现事务性操作，避免网络往返。
- **数据清理与迁移**：使用 Lua 脚本批量处理数据，如清理过期数据、格式转换、数据迁移。

## 最佳实践

- **脚本保持简短**：Lua 脚本执行期间会阻塞 Redis 单线程，避免在脚本中执行耗时操作。
- **优先使用 EVALSHA**：先通过 SCRIPT LOAD 加载脚本，再用 EVALSHA 执行，避免重复传输脚本内容。
- **错误处理**：使用 redis.pcall 捕获错误，避免脚本异常导致 Redis 崩溃。
- **避免副作用**：Lua 脚本应保持幂等性，避免引入随机数、时间戳等不确定因素。

## 常见问题

**Lua 脚本执行失败怎么办？**

Redis 的 Lua 脚本要么全部成功要么全部失败，不支持部分回滚。如果脚本执行失败，需要通过 SCRIPT_KILL 终止脚本，或检查脚本逻辑后重新执行。

**如何调试 Lua 脚本？**

可以使用 redis.log 在脚本中打印日志，或通过 redis-cli 的 EVAL 命令测试脚本。生产环境建议在测试环境充分验证后再上线。
