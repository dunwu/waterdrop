...
- **实时消息通知**：发布系统事件（如订单状态变更、库存告警），订阅者实时接收并处理，实现解耦。
- **聊天室与弹幕**：用户订阅特定频道，发送消息时广播给所有订阅者，实现实时互动。
- **缓存失效通知**：监听缓存 key 的变更事件，缓存更新时通知所有订阅者刷新本地缓存。
- **系统监控与告警**：订阅系统指标事件（如 CPU 告警、服务异常），实时触发告警流程。

## 最佳实践

- **合理设计频道命名**：使用层级命名（如 `system:order:created`），便于模式订阅和权限控制。
- **客户端重连机制**：实现自动重连和重新订阅逻辑，避免网络抖动导致消息丢失。
- **避免消息堆积**：发布/订阅是 fire-and-forget 模式，消息不持久化，订阅者离线时消息丢失，关键消息建议用 MQ。
- **监控订阅状态**：通过 PUBSUB 命令监控订阅者数量和频道状态，及时发现异常。

## 常见问题

**发布/订阅和 MQ 的区别？**

发布/订阅是实时广播，消息不持久化，订阅者离线时消息丢失；MQ 支持消息持久化、重试、顺序保证。简单实时通知用发布/订阅，可靠消息传递用 MQ。

**为什么订阅者收不到消息？**

常见原因：1) 客户端未成功订阅（检查网络连接）；2) 频道名称不匹配；3) 客户端已断开未重连；4) Redis 服务端 Pub/Sub 缓冲区溢出。
