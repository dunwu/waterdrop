...
- **数据备份与恢复**：定期生成 RDB 快照备份到远程存储，用于数据恢复、灾难恢复和历史数据归档。
- **AOF 持久化保证数据安全**：对数据安全性要求高的场景（如订单、支付）开启 AOF 持久化，配合 everysec 策略平衡性能与安全。
- **主从复制基础**：主节点通过 RDB/AOF 同步数据给从节点，实现数据冗余和高可用。
- **数据迁移与扩容**：通过 RDB 文件将数据迁移到新集群或新实例，支持在线扩容和架构调整。

## 最佳实践

- **RDB + AOF 混合持久化**：Redis 4.0+ 支持混合持久化，AOF 重写时先写 RDB 格式再追加 AOF，兼顾快速加载和数据安全。
- **合理设置 AOF 刷盘策略**：everysec 是最佳平衡点，每秒最多丢失 1 秒数据；always 最安全但性能最差；no 性能最好但可能丢数据。
- **定期备份与归档**：将 RDB/AOF 文件备份到云存储（S3、OSS），设置合理的保留周期，支持任意时间点恢复。
- **监控持久化状态**：关注 rdb_last_bgsave_status、aof_last_bgrewrite_status 等指标，及时发现持久化失败。

## 常见问题

**RDB 和 AOF 如何选择？**

RDB 体积小、恢复快，适合定期备份和数据迁移；AOF 数据更安全，适合对数据丢失零容忍的场景。建议同时开启，Redis 重启时优先加载 AOF（数据更完整）。

**AOF 文件过大怎么办？**

开启 AOF 重写（auto-aof-rewrite-percentage/ auto-aof-rewrite-min-size），Redis 会自动压缩 AOF 文件。手动执行 BGREWRITEAOF 也可以触发重写。
