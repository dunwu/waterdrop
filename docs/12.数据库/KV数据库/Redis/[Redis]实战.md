...
> 更多分布式锁的实现方式及细节，请参考：[分布式锁基本原理](https://dunwu.github.io/waterdrop/pages/e64d3e45/)
...
- **电商秒杀系统**：利用 Redis 的原子操作和过期时间实现库存扣减、限购校验、秒杀资格验证，支撑高并发场景。
- **社交网络功能**：使用 Set 实现关注/粉丝关系、共同好友计算；使用 ZSet 实现热度排行榜、时间线排序。
- **游戏排行榜**：利用 ZSet 的有序性实现实时排行榜，支持分数更新、排名查询、范围查询等操作。
- **实时计数器**：使用 INCR/DECR 实现页面浏览量、点赞数、评论数等实时统计，支持原子操作和高并发。

## 最佳实践

- **数据结构选型**：根据场景选择合适的数据结构，如计数器用 String、排行榜用 ZSet、标签用 Set、消息队列用 List。
- **合理设置过期时间**：根据业务时效性设置 TTL，避免数据堆积；对缓存使用随机过期时间防止雪崩。
- **避免大 key**：拆分大集合、分片大字符串，避免单个 key 占用过多内存或阻塞 Redis。
- **监控与告警**：监控内存使用率、连接数、响应时间、慢查询等指标，设置合理阈值及时告警。

## 常见问题

**Redis 适合做消息队列吗？**

Redis 的 List 和 Pub/Sub 可以实现简单消息队列，但存在局限：不支持消息持久化、重试、顺序保证、事务消息等。对可靠性要求高的场景建议使用 Kafka、RabbitMQ 等专业 MQ。

**如何优化 Redis 性能？**

1) 使用 Pipeline 批量操作；2) 选择合适的数据结构和编码；3) 避免大 key 和慢查询；4) 合理设置内存和淘汰策略；5) 使用集群分片提升叶量。
