...
- **缓存容量规划**：根据业务数据量和访问模式，合理设置 maxmemory 和淘汰策略，避免内存不足导致服务不可用。
- **冷热数据分离**：使用 volatile-lru/lfu 策略自动淘汰低频访问数据，将内存留给热点数据，提升缓存命中率。
- **内存泄漏排查**：通过 MEMORY USAGE 命令监控 key 内存占用，结合 OBJECT ENCODING 分析数据结构，定位内存异常增长的原因。
- **成本控制**：通过合理的数据结构设计（如使用 ziplist、intset 压缩）减少内存占用，降低服务器成本。

## 最佳实践

- **选择合适的淘汰策略**：纯缓存场景用 allkeys-lru/lfu；需要持久化的 key 用 volatile-lru/lfu；不确定时用 allkeys-lru。
- **监控内存指标**：定期监控 used_memory、mem_fragmentation_ratio、evicted_keys 等指标，及时预警内存问题。
- **避免大 key**：单个 String 不超过 10KB，集合类型元素不超过 1000 个，大 key 会导致阻塞和内存碎片。
- **定期优化内存碎片**：开启 activedefrag，或在低峰期重启实例，减少内存碎片率。

## 常见问题

**Redis 内存占用过高怎么办？**

1) 检查 maxmemory 设置是否合理；2) 分析大 key（redis-cli --bigkeys 或 MEMORY USAGE）；3) 优化数据结构（如用 ziplist 替代 hashtable）；4) 清理过期数据；5) 考虑扩容或分片。

**为什么设置了 maxmemory 但内存仍超标？**

Redis 的 maxmemory 只控制数据内存，不包含缓冲区、复制积压区、Lua 环境等。实际内存 = 数据内存 + 缓冲区 + 进程开销 + 碎片。建议预留 20%-30% 的额外内存空间。
