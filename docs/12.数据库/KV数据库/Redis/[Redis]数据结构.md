...
- **缓存对象存储**：使用 Hash 存储用户对象、商品信息等结构化数据，支持部分字段更新，避免序列化开销。
- **列表与时间线**：使用 List 实现最新消息、评论列表、操作日志等时间线功能，支持快速插入和范围查询。
- **集合运算与标签**：使用 Set 实现标签系统、共同关注计算、去重统计，支持交集、并集、差集运算。
- **有序数据检索**：使用 ZSet 实现排行榜、延迟队列、优先级队列，支持按分数排序和范围查询。

## 最佳实践

- **选择合适的数据结构**：根据访问模式选择数据结构，如频繁更新部分字段用 Hash，需要排序用 ZSet，需要去重用 Set。
- **控制集合大小**：大集合会导致内存占用高和操作延迟，建议单个集合元素不超过 1000-5000 个。
- **利用压缩编码**：小集合自动使用 ziplist/intset 压缩，合理控制集合大小可显著降低内存占用。
- **避免嵌套结构**：Redis 不支持嵌套数据结构，复杂对象应扁平化或使用多个键存储。

## 常见问题

**Hash 和 String 存储对象如何选择？**

Hash 适合存储对象的部分字段更新，内存占用更低（ziplist 编码）；String 适合整体读写，序列化更灵活（JSON、Protobuf）。频繁更新部分字段用 Hash，整体读写用 String。

**为什么 Redis 不支持嵌套数据结构？**

Redis 是单线程模型，嵌套结构会增加命令复杂度，影响性能。Redis 设计哲学是保持简单高效，复杂数据结构应在应用层实现。
