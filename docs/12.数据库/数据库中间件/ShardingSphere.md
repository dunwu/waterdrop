...
![](https://raw.githubusercontent.com/dunwu/images/master/archive/2020/10/dbb9131965314abe8c0696b696fb003e.png)
...
![](https://raw.githubusercontent.com/dunwu/images/master/archive/2020/10/fc4084a0414349c4bb8d4c76bcde8a20.png)
...
![](https://raw.githubusercontent.com/dunwu/images/master/archive/2020/10/aacf33b4205d4ee9b99ca0b2e6e14eb8.png)
...
![](https://raw.githubusercontent.com/dunwu/images/master/archive/2020/10/2fc0dbaacbc4401e8b9e3281ba9cf38b.png)
...
![](https://raw.githubusercontent.com/dunwu/images/master/archive/2020/10/af28709176fa405089e5063f6135e030.png)
...
![](https://raw.githubusercontent.com/dunwu/images/master/archive/2020/10/f576a2b8efd84ebf9ef229f31c03b850.png)
...
- **电商订单系统**：通过分库分表将订单数据分散到多个数据库实例，解决单表数据量过大的性能瓶颈
- **多租户 SaaS 平台**：利用垂直分片将不同租户的数据存储在不同的数据库实例中，实现数据隔离
- **读写分离架构**：通过主从复制 + 读写分离，主库负责写、从库负责读，提升系统整体吞吐量
- **分布式事务场景**：结合 XA 事务或柔性事务（如 BASE、TCC），解决跨库更新的一致性问题

## 最佳实践

- **合理选择分片键**：分片键应选择查询条件中频繁出现的字段，避免全路由查询（广播查询）
- **避免跨库 JOIN**：通过数据冗余（CQRS）或应用层组装来解决跨库关联查询问题
- **控制分片数量**：分片数不宜过多，建议 2 的幂次方，便于后续扩容
- **结合 Proxy 和 JDBC**：对 Java 应用使用 ShardingSphere-JDBC，对异构语言使用 ShardingSphere-Proxy

## 常见问题

**ShardingSphere-JDBC 和 ShardingSphere-Proxy 如何选择？**

JDBC 适合 Java 应用，性能损耗低，无中心化；Proxy 适合异构语言场景，提供统一入口，但会增加网络跳转延迟。

**分片后如何处理全局唯一 ID？**

ShardingSphere 内置雪花算法（Snowflake）和 UUID 策略，也可集成第三方 ID 生成器（如 Leaf、UidGenerator）。

**跨分片查询性能很差怎么办？**

优化方向：合理设计分片键避免跨片查询、使用辅助索引表、将数据同步到 Elasticsearch 等搜索引擎。
