...
InnoDB 的行锁加锁规则非常复杂，核心规则如下：

**原则 1：加锁的基本单位是 Next-Key Lock**

- Next-Key Lock = 记录锁 + 记录前的间隙锁
- 例如索引中有 5、10、15，则 Next-Key Lock 的范围为 (0,5]、(5,10]、(10,15]

**原则 2：查找过程中访问到的对象才会加锁**

- 索引上搜索过程中未访问到的记录不会加锁

**原则 3：唯一索引上的等值查询退化为记录锁**

- `SELECT ... WHERE id = 5 FOR UPDATE` 只锁定 id=5 这一行

**原则 4：唯一索引上的范围查询仍使用 Next-Key Lock**

- `SELECT ... WHERE id > 5 FOR UPDATE` 锁定 (5, +∞)

**原则 5：非唯一索引上的等值查询，Next-Key Lock 退化为记录锁 + 下一个间隙锁**

- 例如 `WHERE name = 'Alice' FOR UPDATE`，除了锁定 Alice 所在行，还会锁定其后的间隙

**实战示例**：

```sql
-- 表 t 有索引 id(主键), 数据: (5,'a'), (10,'b'), (15,'c')

-- 可重复读下:
SELECT * FROM t WHERE id = 10 FOR UPDATE;
-- 加锁: Record Lock on id=10 (唯一索引等值查询)

SELECT * FROM t WHERE id >= 10 AND id < 15 FOR UPDATE;
-- 加锁: Next-Key Lock (10,15] + 间隙锁 (15, +∞)

SELECT * FROM t WHERE id = 20 FOR UPDATE;
-- 加锁: Next-Key Lock (15,20] + 间隙锁 (20, +∞)
-- 但 id=20 不存在, 退化为间隙锁 (15, +∞)
```
