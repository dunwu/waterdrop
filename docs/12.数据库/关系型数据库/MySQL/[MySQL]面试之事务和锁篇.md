...
为了解决以上问题，所以需要事务二阶段提交（`prepare` → `commit`），以确保写入两日志的原子性。
...
- 版本链中的 undo log **不会被立即清理**，而是由 **purge 线程**在确认旧版本对所有活跃事务都不可见（purge view 推进）之后才清理。
- **长事务会阻碍 purge view 推进，导致版本链越来越长**：快照读需要逐级遍历版本链，查询性能劣化；undo 表空间持续膨胀，严重时撑爆磁盘。生产中应监控 `information_schema.innodb_trx` 中存活时间过长的事务并及时 kill。

**L3/L4 追问**：

- `UPDATE` 为什么不走 MVCC 快照读，而是当前读？（当前读 + 记录锁保证并发更新不丢失，否则会发生丢失更新）
- RC 与 RR 创建 ReadView 的时机差异之外，RC 的 semi-consistent read 是什么？（RC 下 UPDATE 读到不满足条件的已提交行时会解锁，减少锁范围）
- MySQL 与 Oracle 的 MVCC 有何差异？（Oracle 基于 undo 段 + SCN，读一致性由 SCN 比较得出；MySQL 基于 undo 版本链 + ReadView，且 InnoDB 用间隙锁辅助防幻读，Oracle 没有间隙锁）
...
- 事务 2 读取数据（创建 ReadView），value 为 100；
- 事务 1 提交事务；
- 事务 2 读取数据（创建 ReadView），value 为 200；
...
::: info MVCC + Next-Key Lock 如何解决幻读
:::
...
- InnoDB 在每个二级索引**页（Page）**上记录了 **`PAGE_MAX_TRX_ID`**，即修改过该页的最大事务 ID。
- 读取时，若 `PAGE_MAX_TRX_ID` 小于当前 ReadView 的最小活跃事务 ID，说明该页的最后修改发生在快照之前，**页内数据直接可见，走覆盖索引即可，无需回表**。
- 否则，需要**回表到聚簇索引**，利用聚簇索引记录的 `trx_id` 和版本链来判断该记录对当前事务是否可见。
...
- “**读未提交**”隔离级别：因为可以读到未提交事务修改的数据，所以**直接读取最新的数据**就好了；
- “**串行化**”隔离级别：**通过加读写锁的方式来避免并行访问**；
- “**读提交**”和“**可重复读**”隔离级别：它们**都是通过 MVCC 来实现的，区别仅在于创建 ReadView 的时机不同**。ReadView 可以理解为一个数据快照。
  - **“读提交”在“每个语句执行前”都会重新生成一个 ReadView**。
  - **“可重复读”在“启动事务时”生成一个 ReadView，然后整个事务期间都在用这个 ReadView**。
...
1. 事务 A、事务 B 同时执行 `SELECT * FROM t WHERE id = 7 FOR UPDATE`：等值查询未命中，两者都加上间隙锁 (5,10)。**间隙锁之间互相兼容**，都能加锁成功。
...
**L3 追问——高并发热点行场景的权衡**：死锁检测的时间复杂度是 O(n)，每个被阻塞的线程都要遍历等待链，大量线程等待同一热点行时检测开销巨大（CPU 飙升）。此时可考虑：关闭 `innodb_deadlock_detect` 并调小 `innodb_lock_wait_timeout`（如 1~5 秒），让等待快速超时失败，由业务重试；或从架构上削减热点行的并发（如队列串行化、拆分热点账户）。
...
- 原因：唯一性保证了不可能再插入相同值的记录，不存在幻读风险，无需间隙锁

**优化 2：唯一索引上的范围查询，向右扫描直到遇到不满足条件的值，扫描到的记录与间隙都加 Next-Key Lock**

- `SELECT ... WHERE id > 5 FOR UPDATE` 从 id=5 右侧开始，一直锁到 +supremum

**退化规则（等值未命中 / 非唯一索引）**

- 唯一索引等值查询，记录**不存在**：Next-Key Lock 退化为**间隙锁**（防止其他事务插入该值）
- 非唯一索引等值查询，记录**存在**：Next-Key Lock 退化为**记录锁 + 其后间隙锁**（向右遍历到第一个不满足等值条件的值，防止插入相同索引值的记录）
- 非唯一索引等值查询，记录**不存在**：退化为间隙锁
...
-- 唯一索引等值未命中 → 退化为间隙锁: (15, +supremum)，阻止其他事务插入 20
...
- 读已提交隔离级别下，Next-Key Lock 中的间隙锁部分被取消，只加记录锁；但有两个例外场景仍需间隙锁：**唯一键冲突检查**和**外键约束检查**。
- 插入意向锁之间互相兼容（不同事务可向同一间隙插入不同值），只与间隙锁冲突——这就是间隙锁会阻塞插入、进而引发死锁的根源。
- 无索引列的 UPDATE/DELETE 为什么会锁全表？（InnoDB 通过隐藏的聚簇索引定位所有行并加锁，再回删二级索引锁，最终表现为锁全表）
