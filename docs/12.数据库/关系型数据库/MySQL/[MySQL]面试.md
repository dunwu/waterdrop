...
- `DROP` 删除数据表，包括数据和结构。在 MySQL 5.7 及之前，表数据存于 `.ibd` 文件，表结构元数据存于 `.frm` 文件，DROP 会直接删除这些文件；**MySQL 8.0 移除了 `.frm` 文件，表结构统一存入 InnoDB 数据字典**。DROP 是 DDL，执行后无法回滚。
...
- `TIMESTAMP` 占用 4 个字节，可表示范围为：`'1970-01-01 00:00:01.000000' UTC` 到 `'2038-01-19 03:14:07.999999' UTC`。存储的是从 `1970-01-01 00:00:00` UTC 到现在的**秒数**（支持微秒精度时小数部分额外占用空间）。
- `INT` 类型就是直接存储从 ‘1970-01-01 00:00:00’ 到现在的**秒数**，本质和 `TIMESTAMP` 一样，因此用 `INT` 不如直接使用 `TIMESTAMP`（后者还自带时区转换能力）。
...
- 页合并非自动：删除记录产生的空间碎片不会自动归还操作系统，需要 `OPTIMIZE TABLE` 或 `ALTER TABLE ... ENGINE=InnoDB` 重建表。
- 自增主键并非万能：分库分表场景全局自增不可行，需用分布式 ID；高并发插入下自增锁（`innodb_autoinc_lock_mode`）也需关注。
...
- **为什么 binlog 不能用于崩溃恢复**：binlog 是逻辑日志，且不保证包含未提交事务的完整物理页信息；崩溃恢复依赖 redo log 的物理重放。反之，redo log 是循环写的固定空间，无法长期保留做时间点恢复，两者互补。
- **一致性保证**：正因为两套日志各司其职，InnoDB 通过**两阶段提交**（redo log prepare → 写 binlog → redo log commit）保证二者逻辑一致，否则主从复制和崩溃恢复会出现数据不一致。
...
- **GTID 复制**（5.6+）：每个事务带全局唯一 ID，主从切换时新主自动识别未执行的 GTID 继续拉取，极大简化了故障切换和主从搭建，是现代 MySQL 高可用的标配。
- **并行复制演进**：5.6 按库并行（单库写入仍是单线程）→ 5.7 基于组提交逻辑时钟（LOGICAL_CLOCK，同组提交的事务可并行）→ 8.0 基于 WRITESET（按行冲突判断，并行度更高）。主从延迟治理的关键就是选对并行复制策略。
- **半同步的两种模式**：AFTER_SYNC（无损复制，主库写 binlog 并等待从库确认后才提交事务，5.7 默认）优于 AFTER_COMMIT（主库先提交再等待确认，存在幻读窗口）。
...
MySQL 5.5 引入了半同步复制（插件形式）：主库只需等待**至少 N 个从库**（可配置）确认即返回。
...
2. **查询缓存**：MySQL 服务器首先检查查询缓存，如果命中缓存，则立刻返回结果。否则进入下一阶段。MySQL 缓存弊大于利，因为失效非常频繁——任何更新都会清空查询缓存，因此 **MySQL 8.0 已彻底移除查询缓存**，缓存职责交给应用层（如 Redis）。
...
graph TB
    ...
    C --> D["优化器: 选择执行计划"]
    D --> E["执行器: 调用存储引擎"]
    E --> F["写入 redo log (prepare)"]
    F --> G["写入 binlog"]
    G --> H["redo log 状态改为 commit"]
    H --> I["两阶段提交完成"]
```
...
1. 执行器拿到数据后，更新内存（Buffer Pool）中的数据页；
2. 写入 redo log，状态置为 **prepare**；同时写入 undo log 用于回滚；
3. 写入 binlog 并刷盘（`sync_binlog` 控制刷盘时机，生产建议设为 1）；
4. 将 redo log 状态改为 **commit**，事务提交完成。

**崩溃恢复规则**：redo log 处于 prepare 状态时，若 binlog 完整（含 Xid 事件）则提交事务，否则回滚——这正是两阶段提交保证两套日志一致的核心机制。

**生产参数组合（数据安全的两大开关）**：`innodb_flush_log_at_trx_commit=1` + `sync_binlog=1`（双 1 配置），每次提交都强制刷盘，性能最低但数据最安全，金融级业务标配；对性能敏感的业务可放宽为 2/1000 等，但需接受宕机丢数据的风险。
...
MySQL 8.0 是一次重大升级，引入了多项核心改进：

**1. 原子 DDL（Atomic DDL）**

- DDL 操作支持原子性，要么完全成功，要么完全回滚
- 基于 InnoDB 数据字典实现，不再出现半完成状态的 DDL
