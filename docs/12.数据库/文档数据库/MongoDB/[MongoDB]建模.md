...
- **电商平台**：商品与评论使用嵌入式文档（子集模式），用户与订单使用引用式，平衡查询性能和文档大小
- **内容管理系统**：文章与标签使用嵌入式，文章与作者使用引用式，支持灵活的内容扩展
- **社交网络**：用户与好友关系使用引用式 + 反规范化，好友动态使用嵌入式数组存储最新 N 条
- **分类目录**：使用实体化路径模型存储商品分类树，支持快速查询分类下的所有子分类

## 最佳实践

- **根据查询模式设计**：以读驱动设计，根据应用的查询模式来决定使用嵌入式还是引用式
- **避免过深嵌套**：嵌套层级不宜过深，一般不超过 3-4 层，否则查询和更新会很复杂
- **控制文档大小**：单个文档不超过 16 MB，对于一对多关系且子文档增长无界时，使用引用式或子集模式
- **适当反规范化**：将频繁查询的关联数据冗余到文档中，避免多次查询，用空间换时间

## 常见问题

**嵌入式和引用式如何选择？**

原则：数据经常一起访问且增长有限用嵌入式；数据独立访问或增长无界用引用式。考虑读取比例、更新频率、数据量等因素。

**如何建模一对多关系？**

子文档少且稳定用嵌入式；子文档多且增长用引用式；子文档极多但只展示部分用子集模式（嵌入最新 N 条 + 引用全部）。

**树形结构怎么建模？**

根据查询需求选择：查找父节点用父引用、查找子节点用子引用、查找祖先用祖先数组、范围查询用嵌套集、全树遍历用实体化路径。
