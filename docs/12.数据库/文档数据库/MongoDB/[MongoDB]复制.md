...
- **高可用架构**：通过 3 节点副本集实现自动故障转移，主节点故障时自动选举新主节点
- **读写分离**：主节点处理写操作，从节点承担读请求，通过读取偏好灵活分配读负载
- **数据备份与恢复**：在从节点上执行备份操作，避免影响主节点的写入性能
- **跨地域容灾**：在不同数据中心部署副本集成员，实现跨地域的数据冗余和灾难恢复

## 最佳实践

- **至少 3 个节点**：避免脑裂问题，确保能选举出主节点（多数派原则）
- **设置合理的读偏好**：默认 primary 保证强一致性，对延迟容忍的场景使用 secondary 分散读负载
- **配置写关注**：重要数据使用 `w: "majority"` 确保写入多数节点，避免数据丢失
- **监控复制延迟**：定期检查从节点的 oplog 延迟，确保复制在可接受范围内

## 常见问题

**复制延迟会影响数据一致性吗？**

会。从节点的数据可能落后于主节点，使用 secondary 读取可能读到旧数据。对一致性要求高的场景应使用 primary 读取。

**如何判断副本集是否正常？**

通过 `rs.status()` 检查各节点状态，确认主节点正常、从节点同步延迟在可接受范围内、无节点处于 STARTUP 或 RECOVERING 状态。

**Oplog 满了会怎样？**

如果 oplog 空间不足，旧的操作记录会被覆盖，导致从节点无法跟上主节点。应监控 oplog 大小，确保其足以容纳从节点同步所需的时间。
