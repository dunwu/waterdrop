...
- **生产环境部署**：安装和配置副本集/分片集群，设置认证、SSL/TLS 加密等安全措施
- **定期备份与恢复**：使用 mongodump/mongorestore 定期备份数据，支持灾难恢复和数据迁移
- **性能监控与调优**：通过 mongostat、mongotop 等工具监控数据库性能，分析慢查询并优化
- **版本升级与迁移**：滚动升级副本集成员，使用 mongoimport/mongoexport 实现跨版本数据迁移

## 最佳实践

- **启用认证**：生产环境必须开启认证（--auth），使用 SCRAM-SHA-256 认证机制
- **配置 WiredTiger 缓存**：设置合理的 wiredTigerCacheSizeGB，避免内存占用过高
- **定期轮换日志**：配置 logRotate，避免日志文件无限增长占满磁盘
- **监控慢查询**：开启 profiling（`db.setProfilingLevel(1, { slowms: 100 })`），定期分析慢查询并优化索引

## 常见问题

**mongodump 和 mongoexport 有什么区别？**

mongodump 导出 BSON 格式，保留所有数据类型信息；mongoexport 导出 JSON/CSV 格式，可能丢失部分 BSON 类型精度。备份用 mongodump，数据分析用 mongoexport。

**如何安全地升级 MongoDB 版本？**

采用滚动升级策略：先升级从节点，然后逐步降级主节点并升级。升级前先备份数据，并在测试环境验证兼容性。

**如何优化 MongoDB 写入性能？**

优化方向：使用批量写入、设置合理的 writeConcern、调整 journalCommitInterval、使用 WiredTiger 引擎的 snappy 压缩。
