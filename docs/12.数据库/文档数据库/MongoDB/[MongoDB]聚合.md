...
- **电商数据分析**：使用 `$group` 聚合统计各分类商品销量、销售额，配合 `$sort` 实现排行榜
- **用户行为分析**：通过 `$match` + `$group` + `$project` 管道分析用户行为路径和转化漏斗
- **日志统计报表**：使用 `$group` + `$dateToString` 按天/周/月聚合日志数据，生成报表
- **数据导出与转换**：使用 `$out` 或 `$merge` 将聚合结果输出到新集合，支持数据转换和 ETL

## 最佳实践

- **尽早 `$match`**：将 `$match` 放在管道最前面，尽早缩小数据集，显著提升性能
- **使用 `$project` 精简字段**：只保留后续管道需要的字段，减少内存消耗
- **配合索引优化**：确保 `$match` 和 `$sort` 的字段有索引，避免全表扫描和内存排序
- **设置 allowDiskUse**：对于大数据量聚合，设置 `allowDiskUse: true` 避免内存超限

## 常见问题

**聚合管道和 Map-Reduce 如何选择？**

优先使用聚合管道。聚合管道性能更好、接口更一致，Map-Reduce 在 MongoDB 5.0 后已被标记为废弃。

**聚合结果有大小限制吗？**

单个文档不能超过 16 MB（BSON 限制）。如果结果集很大，使用游标返回或 `$out`/`$merge` 写入集合。

**如何优化慢聚合查询？**

将 `$match` 前置、使用索引、精简字段、避免不必要的 `$sort`、对大数据集使用 `$sample` 采样。
