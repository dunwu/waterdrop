...
- **用户搜索**：在用户名、邮箱字段上创建复合索引，支持多维度的用户查找和排序
- **地理位置查询**：使用 2dsphere 索引实现“附近的商家”、“周边的人”等地理位置查询
- **日志时间范围查询**：在时间字段上创建索引，支持高效的日志时间范围检索
- **数据自动过期**：使用 TTL 索引自动删除过期数据（如会话、验证码），无需应用层清理

## 最佳实践

- **根据查询模式创建索引**：分析慢查询日志，为频繁查询的条件字段创建合适的索引
- **使用复合索引**：对于多字段查询，创建复合索引并遵循 ESR 原则（Equality, Sort, Range）
- **避免索引过多**：每个索引都会占用存储空间并影响写入性能，定期评估和清理无用索引
- **使用 explain 分析查询**：通过 `explain("executionStats")` 检查查询是否使用了索引，优化查询计划

## 常见问题

**复合索引的字段顺序怎么确定？**

遵循 ESR 原则：先放等值匹配字段（Equality），再放排序字段（Sort），最后放范围查询字段（Range）。

**为什么查询很慢但没有使用索引？**

可能原因：查询条件字段没有索引、索引字段类型不匹配、查询选择了性很差（返回大部分文档时 MongoDB 会选择全表扫描）。

**文本索引和正则查询有什么区别？**

文本索引支持全文搜索和相关性评分，使用 `$text` 查询；正则查询使用 `$regex`，适合模式匹配。文本索引性能更好但功能受限。
