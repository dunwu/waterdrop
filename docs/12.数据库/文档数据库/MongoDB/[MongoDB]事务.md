...
- **电商订单处理**：在订单创建时同时扣减库存和创建订单记录，保证两个操作的原子性
- **资金转账**：从 A 账户扣款同时向 B 账户加款，确保两个操作要么全部成功要么全部回滚
- **多集合数据同步**：用户注册时同时写入用户集合和初始化用户配置集合，保证数据完整性
- **跨数据库事务**：在分片集群上跨多个分片的数据更新，通过分布式事务保证一致性

## 最佳实践

- **优先单文档原子性**：通过嵌入式文档设计减少对多文档事务的需求，单文档操作本身就是原子的
- **设置合理超时**：事务默认 60 秒超时，根据业务复杂度调整 `maxTransactionLockRequestTimeoutMillis`
- **使用 majority 写关注**：事务提交使用 `w: "majority"` 写关注，确保数据持久化到多数节点
- **控制事务大小**：单个事务修改的数据量不宜过大（建议不超过 16 MB），大事务应拆分为多个小事务

## 常见问题

**MongoDB 事务和 MySQL 事务有什么区别？**

MongoDB 支持跨集合、跨数据库、跨分片的多文档事务；MySQL 的事务范围是单个连接内的多表操作。MongoDB 事务默认有 60 秒超时。

**事务会影响性能吗？**

会。多文档事务会带来额外的性能开销，特别是跨分片事务。建议仅在必须保证多文档原子性时使用事务。

**事务中的读操作应该用什么读关注？**

建议使用 `"snapshot"` 读关注，它可以保证事务内的所有读操作基于同一快照，避免不可重复读。
