...
![插入操作](https://raw.githubusercontent.com/dunwu/images/master/archive/2020/09/fdd687f712e748ecbfdafdabdb50b396.svg)
...
![查询操作](https://raw.githubusercontent.com/dunwu/images/master/archive/2020/09/bdc5f330c00845cc8a563d97dda4043c.svg)
...
![更新操作](https://raw.githubusercontent.com/dunwu/images/master/archive/2020/09/7370a8bafe8a442b83992f70acc86ee6.svg)
...
![删除操作](https://raw.githubusercontent.com/dunwu/images/master/archive/2020/09/aca26919c2e942c89c282849b69c6860.svg)
...
![](https://raw.githubusercontent.com/dunwu/images/master/archive/2025/03/ceab93df6c0b4363bec974f4a680b4ec.png)

## 应用场景

- **内容管理系统**：通过 insertMany 批量导入文章、分类等数据，updateOne 更新文章状态、标签
- **用户行为日志**：使用 insertOne 记录用户操作，deleteMany 定期清理过期日志数据
- **数据迁移导入**：结合 bulkWrite 实现大规模数据迁移，支持有序/无序写入策略优化性能
- **实时状态更新**：通过 updateOne 的 `$set` 和 `$inc` 操作符实时更新设备状态、库存数量等

## 最佳实践

- **批量操作优先**：使用 insertMany 和 bulkWrite 代替逐条操作，显著减少网络往返开销
- **合理使用 upsert**：updateOne/updateMany 配合 `upsert: true`，实现“不存在则插入，存在则更新”
- **选择合适的有序/无序模式**：对性能敏感的分片集群场景使用无序批量写入（`ordered: false`）
- **避免全量替换**：使用 `$set` 进行局部更新而非 replaceOne 全量替换，减少写入量

## 常见问题

**插入操作会自动创建集合吗？**

会。如果插入的目标集合不存在，MongoDB 会自动创建该集合。

**_id 字段可以修改吗？**

不能。`_id` 字段一旦设置就不可修改。如果文档未指定 `_id`，MongoDB 会自动生成 ObjectId。

**bulkWrite 的有序和无序有什么区别？**

有序操作串行执行，遇到错误停止后续操作；无序操作可并行执行，单个错误不影响其他操作。无序模式在分片集群上性能更好。
