...
- **内容管理系统**：存储文章、评论、用户信息等非结构化数据，支持灵活的内容模型和快速迭代
- **物联网平台**：存储设备传感器数据、设备元数据，支持海量设备的高并发写入和灵活查询
- **用户画像系统**：存储用户标签、行为特征、兴趣偏好等数据，支持动态字段和灵活的数据结构
- **实时分析系统**：存储应用日志、业务事件，配合聚合管道实现实时数据分析

## 最佳实践

- **合理设计文档结构**：根据查询模式设计文档，将经常一起访问的数据嵌入同一文档，避免过度规范化
- **选择合适的数据类型**：使用正确的 BSON 类型（如 ObjectId、ISODate、Decimal128），而非全部用字符串
- **使用副本集保障高可用**：生产环境至少使用 3 节点副本集，配置合理的写关注和读关注
- **定期备份**：使用 mongodump 定期备份数据，或配置云服务商的自动快照功能

## 常见问题

**MongoDB 支持事务吗？**

支持。单文档操作始终是原子的。从 4.0 版本开始支持多文档 ACID 事务（副本集），4.2 版本支持跨分片事务。

**MongoDB 适合什么场景？**

适合数据模型灵活、需要快速迭代、数据量大且增长快、读多写少的场景。不适合强事务场景（如银行转账）。

**MongoDB 和 MySQL 有什么区别？**

MongoDB 是文档数据库，支持灵活的数据模型和水平扩展；MySQL 是关系型数据库，支持强事务和复杂 JOIN。两者是互补关系。
