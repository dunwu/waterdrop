...
![](https://raw.githubusercontent.com/dunwu/images/master/archive/2025/03/5b87d64ca5b64a8ba607c3b2bd8da9a2.png)

Lucene 允许写入和打开新分段，使其包含的文档对搜索可见，而无需执行完全提交。这是一个比提交到磁盘要轻松得多的过程，并且可以频繁地完成而不会降低性能。

![](https://raw.githubusercontent.com/dunwu/images/master/archive/2025/03/bf7a8bcad1a14d018a992da6447bfa25.png)
...
- **商品搜索**：结合 match、bool 查询实现多维度商品搜索，支持关键词、分类、价格区间等组合条件
- **日志检索**：使用 range 查询时间范围、term 查询日志级别，快速定位特定时间段的错误日志
- **用户行为分析**：通过 wildcard、regexp 查询匹配用户行为模式，分析用户访问路径
- **地理位置搜索**：使用 geo_distance 查询实现“附近的人”、“附近的商家”等场景

## 最佳实践

- **优先使用 filter**：对于不需要评分的条件（如状态、分类筛选），使用 filter context 获得缓存和性能优势
- **避免深度分页**：默认 `max_result_window=10000`，对于深度分页场景使用 search_after 或 scroll API
- **使用 boost 控制相关性**：对重要字段设置更高的 boost 值，使包含重要字段关键词的文档得分更高
- **精简返回字段**：通过 `_source` 参数指定只返回需要的字段，减少网络传输和内存开销

## 常见问题

**match 和 term 查询有什么区别？**

match 查询会对查询文本进行分析（分词），适合全文搜索；term 查询不进行分词，精确匹配词项，适合精确值查询如状态码、标签等。

**为什么 bool 查询中的 must 和 filter 性能不同？**

must 会计算相关性评分，filter 不计算评分且结果可缓存。对于不需要评分的筛选条件，使用 filter 性能更好。

**如何实现分页查询？**

使用 `from` + `size` 参数实现分页，但深度分页（from 很大）性能很差。建议前 10000 条用 from/size，更深使用 search_after。
