...
- **电商智能搜索**：使用 function_score 结合销量、评分、价格等因素调整搜索结果的排序，实现商业化的搜索排序策略
- **内容推荐系统**：通过 more_like_this 查询推荐相似文章，结合协同过滤实现个性化内容推荐
- **输入建议（Suggest）**：使用 completion suggester 实现搜索框的自动补全，提升用户搜索体验
- **数据纠错**：利用 fuzzy 查询和 term suggester 实现“你是不是在找”的拼写纠错功能

## 最佳实践

- **合理组合查询**：使用 bool 查询组合多个子查询，用 must 保证相关性、用 filter 保证性能、用 should 提升可选性
- **使用 search_after 代替深度分页**：对于需要遍历大量结果的场景，使用 PIT + search_after 实现高效游标分页
- **优化全文搜索**：对重要字段使用 multi_match 的 best_fields 或 most_fields 策略，提升搜索相关性
- **控制查询复杂度**：避免过多嵌套的 bool 查询和正则查询，这些会显著增加查询耗时

## 常见问题

**全文查询和词项查询如何选择？**

对 text 类型字段使用全文查询（match、multi_match），会对查询文本进行分析；对 keyword、数值、日期类型使用词项查询（term、terms、range），不进行分词。

**function_score 会影响性能吗？**

会。function_score 需要对每个匹配文档计算额外分数，特别是 script_score 会执行自定义脚本。建议在 filter 中先缩小结果集再使用 function_score。

**如何实现拼写纠错？**

使用 fuzzy 查询允许一定的编辑距离，结合 term suggester 或 phrase suggester 提供拼写建议。对于中文场景，可结合拼音分词器实现拼音纠错。
