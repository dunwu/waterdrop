...
- **索引管理**：通过 REST API 创建、删除、更新索引，管理索引生命周期。
- **文档操作**：使用 PUT/POST/GET/DELETE 实现文档的增删改查操作。
- **集群管理**：通过 Cluster API 查看集群状态、管理节点、调整配置。
- **数据迁移**：使用 Reindex API 实现数据迁移、索引重建、字段转换。

## 最佳实践

- **使用 Bulk API 批量操作**：批量写入、更新、删除文档，提升性能。
- **合理设置路由**：使用 routing 参数控制文档路由到特定分片，优化查询性能。
- **使用 Scroll API 遍历数据**：大数据量查询时使用 Scroll API，避免内存溢出。
- **监控 API 调用**：监控 API 调用频率和响应时间，及时发现性能问题。

## 常见问题

**为什么 Bulk 操作失败？**

常见原因：1) 单次 Bulk 数据量过大，建议控制在 5-15MB；2) 文档格式错误；3) 集群状态异常。检查响应中的 errors 字段定位具体错误。

**如何优雅地删除大量文档？**

使用 Delete by Query API 批量删除，或创建新索引（不含要删除的数据）然后删除旧索引。避免使用循环单条删除。
