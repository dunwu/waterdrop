...
- **电商搜索**：通过自定义分析器对商品名称、描述进行分词，支持同义词扩展（如“手机”与“智能手机”），提升搜索召回率
- **内容平台搜索**：结合 IK 分词器对文章标题、正文进行中文分词，配合停用词过滤和同义词表，实现精准的全文检索
- **日志分析**：使用 standard 分析器处理日志中的错误信息、IP 地址等结构化数据，快速定位问题
- **智能客服**：通过拼音分析器、同义词分析器实现模糊匹配和语义搜索，提升用户搜索体验

## 最佳实践

- **合理选择分析器**：英文内容使用 standard 分析器，中文内容使用 IK 分词器（ik_smart 或 ik_max_word），根据场景选择合适粒度
- **测试分析效果**：使用 `_analyze` API 在开发阶段验证分析器的分词结果，确保符合预期
- **管理自定义词典**：定期更新同义词表和停用词表，对于 IK 分词器可使用热更新机制避免重启服务
- **避免过度分析**：对于精确匹配的字段（如订单号、身份证号）设置为 keyword 类型，不使用分析器

## 常见问题

**ik_smart 和 ik_max_word 有什么区别？**

ik_smart 做最粗粒度的拆分，如“中华人民共和国”会被拆分为一个词；ik_max_word 做最细粒度拆分，如“中华人民共和国”会被拆分为“中华人民共和国”、“中华人民”、“中华”、“华人”等多个词。搜索时建议用 ik_smart，索引时用 ik_max_word。

**为什么搜索不到中文内容？**

常见原因：未安装中文分词插件、字段 mapping 未指定分析器、或搜索时使用了错误的分析器。可通过 `_analyze` API 查看实际分词结果进行排查。

**如何支持拼音搜索？**

安装 elasticsearch-analysis-pinyin 插件，在自定义分析器中配置拼音 tokenizer，可实现如“zs”搜索到“张三”的效果。
