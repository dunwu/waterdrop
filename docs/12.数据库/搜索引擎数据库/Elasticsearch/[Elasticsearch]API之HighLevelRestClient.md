...
- **Java 应用集成**：在 Spring Boot、Spring Data Elasticsearch 中使用 High Level REST Client 操作 Elasticsearch。
- **复杂查询构建**：使用 SearchRequest、SearchSourceBuilder 构建复杂的查询条件。
- **批量操作**：使用 BulkRequest 批量写入、更新、删除文档，提升性能。
- **索引管理**：通过 IndicesClient 管理索引的创建、删除、设置等操作。

## 最佳实践

- **复用 RestClient**：High Level REST Client 是线程安全的，建议在应用中复用同一个实例。
- **合理设置超时**：根据网络情况设置 connectTimeout 和 socketTimeout，避免长时间等待。
- **使用异步 API**：对于非关键路径的操作，使用异步 API 提升应用叶吐量。
- **监控客户端性能**：监控请求响应时间、失败率，及时发现性能问题。

## 常见问题

**High Level REST Client 和 Transport Client 的区别？**

Transport Client 是旧版本客户端，通过 TCP 连接，需要与 Elasticsearch 版本匹配；High Level REST Client 是新版客户端，通过 HTTP 连接，兼容性更好。推荐使用 High Level REST Client。

**为什么 RestClient 连接失败？**

常见原因：1) Elasticsearch 未启动；2) 网络不通；3) 认证信息错误；4) 版本不兼容。检查日志定位具体错误。
