...
- **高并发搜索服务**：通过合理设置分片数、副本数，配合 SSD 存储和 JVM 优化，实现毫秒级搜索响应
- **大规模日志平台**：结合索引生命周期管理（ILM）、冷热架构，降低存储成本同时保证查询性能
- **实时监控系统**：优化写入性能（调大 refresh_interval、使用 Bulk API），支持高吞吐的数据写入场景
- **多租户 SaaS 平台**：通过合理的索引设计和路由策略，实现租户数据隔离和查询性能优化

## 最佳实践

- **JVM 内存设置**：堆内存设置为物理内存的 50%，但不超过 32 GB，剩余内存留给 Lucene 文件系统缓存
- **分片策略**：每个分片大小控制在 10-50 GB，分片数不超过节点数的 3 倍，避免过多小分片
- **索引优化**：使用索引模板、合理设置 refresh_interval（如 30s），禁用不需要的 `_source` 字段以节省存储
- **查询优化**：使用 filter context 替代 query context（不计算评分、可缓存），避免深度分页使用 scroll/search_after

## 常见问题

**为什么搜索性能下降？**

常见原因：分片过多导致查询开销大、JVM 内存不足、深度分页查询、未使用 filter context、索引未优化。可通过 Profile API 分析查询耗时。

**如何确定合适的分片数？**

建议根据数据量来定：单个分片 10-50 GB，分片总数不超过节点数的 3 倍。例如 500 GB 数据、5 个节点，建议 10-15 个分片。

**refresh_interval 设置多大合适？**

默认 1s，适用于需要近实时搜索的场景。对于日志、监控等写入密集型场景，可设置为 30s 甚至 -1（禁用自动刷新），显著提升写入性能。
