...
- **生产环境集群部署**：多节点集群的规划、安装和配置，包括 JVM、网络、存储等基础设施的优化
- **集群升级与迁移**：使用滚动重启、快照备份等策略实现集群的平滑升级和数据迁移
- **故障排查与恢复**：通过集群健康状态、分片分配、节点日志等信息定位和解决集群问题
- **监控与告警**：结合 Prometheus、Grafana 等工具实现集群性能指标的实时监控和告警

## 最佳实践

- **JVM 内存设置**：堆内存不超过物理内存的 50% 且不超过 32 GB，确保有足够内存留给 Lucene 文件系统缓存
- **禁止使用 root 运行**：出于安全考虑，使用专用用户运行 Elasticsearch
- **配置 ulimit**：确保文件描述符限制（nofile 65535）、进程数限制（nproc 4096）、内存锁定（memlock unlimited）
- **使用 X-Pack 安全特性**：启用认证、授权、TLS/SSL 加密，保护集群免受未授权访问

## 常见问题

**集群状态为 yellow 怎么办？**

yellow 表示主分片正常但部分副本未分配。常见原因：节点数少于副本数、节点磁盘空间不足。可通过增加节点或调整副本数解决。

**集群状态为 red 怎么办？**

red 表示部分主分片未分配，数据可能丢失。检查节点是否正常、磁盘是否写满、分片是否有损坏。可使用 `_cluster/allocation/explain` API 查看分片分配失败原因。

**如何安全地重启节点？**

先禁用分片分配、执行 flush 操作，然后重启节点。重启后重新启用分片分配，等待集群恢复 green 状态。
