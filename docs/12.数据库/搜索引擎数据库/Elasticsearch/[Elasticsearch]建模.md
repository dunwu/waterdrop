...
- **电商商品索引**：设计商品名称（text）、价格（float）、分类（keyword）、库存（integer）等字段的 mapping，支持多维度搜索和筛选
- **日志索引设计**：根据日志类型定义不同 mapping，如访问日志包含 IP、URL、状态码，应用日志包含级别、异常信息等
- **社交内容平台**：设计用户发帖、评论等内容的索引 mapping，使用 nested 类型处理嵌套的标签和评论结构
- **监控指标索引**：将 CPU、内存、网络等监控指标设计为数值类型，配合时间字段实现时序数据的存储和聚合

## 最佳实践

- **提前设计 Mapping**：尽量避免依赖动态 mapping，主动定义字段类型以避免类型推断错误（如将数字推断为 text）
- **控制字段数量**：设置 `total_fields.limit`，避免 mapping 爆炸（mapping explosion），单个索引字段数控制在合理范围内
- **慎用 nested 类型**：nested 会显著增加存储和查询开销，优先考虑 object 类型或数据反规范化设计
- **利用 dynamic templates**：对于字段名有规律的场景（如以 `_text` 结尾的用 text 类型），通过 dynamic templates 自动匹配类型

## 常见问题

**动态 Mapping 有什么风险？**

当新字段被自动推断为错误类型（如将本应是 keyword 的字段推断为 text），会影响查询性能且无法修改，需要重建索引。生产环境建议设置 `dynamic: strict` 避免自动创建字段。

**Mapping 可以修改吗？**

已有字段的类型不能修改，但可以添加新字段、修改部分参数（如 search_analyzer）。修改字段类型需要 reindex。

**nested 和 object 有什么区别？**

object 将嵌套对象扁平化存储，无法区分同一数组内不同对象的字段关系；nested 将每个嵌套对象作为独立文档存储，支持精确的嵌套查询，但会占用更多资源。
