...
- **电商数据分析**：通过 terms 聚合统计各分类商品销量，配合 histogram 分析价格分布，支持运营决策
- **日志监控看板**：使用 date_histogram 按时间聚合日志数量，结合 terms 聚合统计错误类型分布
- **用户行为分析**：通过 cardinality 聚合计算独立用户数，结合 pipeline 聚合实现用户行为漏斗分析
- **地理位置聚合**：使用 geohash_grid 聚合将地理数据聚合到网格中，配合 Kibana 实现热力图可视化

## 最佳实践

- **使用 keyword 类型聚合**：对 text 字段聚合性能很差，应确保聚合字段使用 keyword 类型或开启 fielddata
- **控制聚合粒度**：对于 terms 聚合，合理设置 size 参数，过大的 size 会消耗大量内存
- **利用 filter 聚合缩小范围**：先通过 filter 缩小数据集，再进行聚合，显著提升性能
- **pipeline 聚合二次计算**：使用 pipeline 聚合对已有聚合结果进行二次计算（如累计、求导、移动平均）

## 常见问题

**为什么 terms 聚合结果不精确？**

在多分片场景下，每个分片独立计算 top N 结果再汇总，可能导致某些项被遗漏。可通过增大 `shard_size` 参数提升精度，但会消耗更多资源。

**metric 聚合和 bucket 聚合有什么区别？**

bucket 聚合将文档分组到不同的桶中（如 terms、date_histogram）；metric 聚合对桶内文档进行统计计算（如 avg、sum、cardinality）。两者可嵌套使用。

**聚合查询很慢怎么办？**

优化方向：使用 filter 缩小范围、避免在 text 字段上聚合、减少分片数、使用 sampler 聚合采样、合理设置 precision_threshold。
