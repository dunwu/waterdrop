...
- **全文搜索引擎**：利用倒排索引实现关键词搜索，配合分词器对文本内容进行索引，支持高相关性的搜索结果返回
- **日志存储与分析**：通过正排索引（doc_values）支持日志字段的聚合统计，倒排索引支持日志内容的全文检索
- **地理位置搜索**：借助 geo_point 和 geo_shape 类型存储地理坐标，实现附近搜索、地理围栏等场景
- **商品属性筛选**：使用 keyword 类型存储商品分类、品牌等结构化属性，结合 filter 实现精确筛选

## 最佳实践

- **选择合适的数据类型**：精确匹配字段用 keyword，全文搜索用 text，避免对同一字段同时启用两种类型除非必要
- **禁用不必要的索引**：对于仅用于展示不需搜索的字段设置 `index: false`，节省存储空间和索引开销
- **合理设置 _source**：对于纯写入不回读的场景（如日志归档），可禁用 `_source` 节省存储，但会影响重新索引能力
- **利用 index_options**：对于不需要高亮和短语查询的字段，将 `index_options` 设置为 `docs`，减少存储开销

## 常见问题

**倒排索引和正排索引有什么区别？**

倒排索引以词项为键，记录词项出现在哪些文档中，适合全文搜索；正排索引（doc_values）以文档为键，记录文档中每个字段的值，适合排序、聚合操作。

**为什么 keyword 类型不需要分析器？**

keyword 类型将字段值作为整体进行索引，不进行分词处理，适合存储枚举值、标签、IP 地址等精确匹配的数据。

**norms 是什么？什么时候可以禁用？**

norms 存储字段长度信息，用于评分计算。如果字段不用于评分（如 filter 场景），可以禁用 norms 节省内存。
