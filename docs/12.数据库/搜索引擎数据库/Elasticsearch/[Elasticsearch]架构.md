...
- **大规模搜索平台**：多分片多副本架构支持 PB 级数据的分布式搜索，通过水平扩展应对流量增长
- **多数据中心部署**：利用跨集群搜索（CCS）和节点角色分离，实现多数据中心的统一搜索视图
- **实时分析系统**：主分片处理写入，副本分片提供查询能力，读写分离实现高并发实时分析
- **弹性伸缩架构**：通过动态调整副本数、分配节点角色，实现根据流量负载的弹性伸缩

## 最佳实践

- **节点角色分离**：将 master、data、coordinating 角色分离部署，避免资源竞争影响稳定性
- **合理设置分片数**：主分片数在创建索引时确定后不可修改，应根据数据量和节点数提前规划
- **副本数动态调整**：通过 API 动态调整副本数，写入高峰期减少副本提升性能，平稳期增加副本保障高可用
- **使用专属主节点**：至少 3 个专属主节点（避免脑裂），不存储数据，专注于集群管理

## 常见问题

**为什么主分片数不能修改？**

因为文档的路由公式是 `hash(_routing) % number_of_shards`，修改主分片数会导致已有文档无法正确定位。如需修改，必须通过 reindex 重建索引。

**副本分片的作用是什么？**

副本提供高可用（主分片失效时副本接替）和提升查询性能（查询可以在副本上执行，分担主分片压力）。

**什么是脑裂（Split Brain）？**

集群中多个节点同时认为自己 master，导致数据不一致。通过设置 `discovery.zen.minimum_master_nodes`（7.x 前）或使用新的选主机制来避免。
