...
![](https://raw.githubusercontent.com/dunwu/images/master/archive/2026/02/c9fdb355cb464fe29caf64be97db8e9b.png)
...
- **日志收集与分析**：使用 Filebeat 收集应用日志，Logstash 解析和转换，Elasticsearch 存储和索引，Kibana 可视化分析。
- **系统监控与告警**：监控系统日志、应用日志，通过 Kibana 设置告警规则，实时发现异常。
- **安全事件分析**：收集安全日志（如防火墙、IDS、访问日志），分析安全事件和威胁。
- **业务数据分析**：将业务日志导入 Elasticsearch，通过 Kibana 生成报表和可视化图表。

## 最佳实践

- **合理设计索引**：根据数据生命周期设计索引，使用 ILM 策略自动管理索引（滚动、删除、快照）。
- **优化 Logstash 配置**：根据日志量调整 batch size、worker 数量，避免 Logstash 成为瓶颈。
- **使用 Filebeat 代替 Logstash**：简单日志收集场景优先使用 Filebeat，轻量级、资源占用少。
- **监控 ELK 集群**：监控 Elasticsearch 集群健康状态、Logstash 队列、Kibana 性能。

## 常见问题

**ELK 日志延迟大怎么办？**

常见原因：1) Logstash 处理能力不足，增加 worker 或扩容；2) Elasticsearch 写入瓶颈，优化索引配置；3) Filebeat 配置不当，调整 backoff 和 bulk_max_size。

**Kibana 无法连接 Elasticsearch？**

检查：1) Elasticsearch 是否正常运行；2) Kibana 配置的 Elasticsearch 地址是否正确；3) 网络是否连通；4) 防火墙是否放行端口。
