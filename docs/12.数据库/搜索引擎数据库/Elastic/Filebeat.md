...
- **应用日志收集**：收集 Spring Boot、Tomcat 等应用日志，发送到 Logstash 或 Elasticsearch。
- **系统日志监控**：收集系统日志、审计日志，用于安全审计和故障排查。
- **容器日志收集**：在 Kubernetes 中使用 Filebeat DaemonSet 收集容器日志。
- **边缘设备日志**：在 IoT 设备、服务器上部署 Filebeat，收集设备日志。

## 最佳实践

- **合理配置 harvester**：根据日志量调整 harvester 数量，避免过多 harvester 占用资源。
- **使用 multiline 配置**：多行日志（如 Java 异常栈）使用 multiline 配置合并为单条事件。
- **定期清理 registry**：及时清理已删除文件的 registry 条目，避免 registry 文件过大。
- **监控 Filebeat 状态**：监控 Filebeat 队列、harvester 数量、事件发送速率。

## 常见问题

**Filebeat 日志延迟大？**

常见原因：1) harvester 配置不当，调整 backoff 和 bulk_max_size；2) 网络延迟，优化网络配置；3) 输出端（Logstash/ES）处理能力不足。

**Filebeat 和 Logstash 如何选择？**

Filebeat 轻量级、资源占用少，适合简单日志收集；Logstash 功能丰富，支持复杂的数据转换和过滤。简单场景用 Filebeat，复杂场景用 Logstash。
