...
- **JIT 分层编译（Tiered Compilation）**
  - **分层编译**：结合解释器、C1（Client Compiler）和 C2（Server Compiler），按编译层次逐步优化：
    - **C1（Client Compiler）**：快速编译，仅做简单优化（方法内联、去虚拟化），侧重启动速度。
    - **C2（Server Compiler）**：深度优化（逃逸分析、标量替换、循环展开），侧重峰值性能。
  - **分层编译层级**（5 层）：
    - 0 层：解释执行（采集 profiling 数据）。
    - 1-3 层：C1 编译，递增的优化级别。
    - 4 层：C2 编译，最强优化。
...
| `-XX:ReservedCodeCacheSize=240M` | 设置代码缓存大小（默认 240MB） |
