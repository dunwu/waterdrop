...
- **Q：不可变性与常量池有什么关系？** → 内容不变才能安全地被多方共享复用，见本文档「字符串常量池有什么用？」。
...
字符串常量池是 JVM 的特殊内存区，保证相同内容的字面量只存一份：直接赋值（`"abc"`）优先从池中引用，`new String()` 强制在堆建新对象，`intern()` 可手动入池。价值是节省内存（复用）、快速比较（`==`）、哈希优化。

#### ⚡记忆卡片

- **口诀**：字面量入池，new 单建，intern 强入池
- **关键词**：字面量 ／ 复用 ／ `intern()`
- **链路**：字面量赋值 → 查常量池 → 有则复用/无则新建 → `intern()` 手动入池

#### 📖 核心知识

字符串常量池是 JVM 的特殊内存区域，用于存储字符串字面量（如 `"abc"`），确保相同内容的字符串只存一份。**常量池通过复用相同字符串，节省内存并提升性能，直接赋值优先使用池，`new String()` 强制创建新对象。**

1. **节省内存**：相同字符串复用，避免重复创建（如 `String s1 = "hello"` 和 `String s2 = "hello"` 指向同一对象）。
...
::: details
- 【L3】JDK 7 起字符串常量池从永久代移入堆内存，降低永久代 OOM 风险；`intern()` 行为随之变化：JDK 6 会把字符串拷贝进永久代，JDK 7+ 池中无同值时直接登记堆中对象的引用。
- 【L3】常量池本质是哈希表（StringTable），桶数量可用 `-XX:StringTableSize` 调整，入池字符串过多时 `intern()` 查找性能会下降。
- 【L4】业界案例：Twitter 曾对高频重复字符串（如 user_location）调用 `intern()`，公开报道称减少约 20% 堆内存，是常量池省内存的经典实践。
:::

#### 🔀 发散问题

- **Q：`new String("abc")` 会创建几个对象？** → 取决于常量池是否已有 `"abc"`，见本文档「`String s = new String("abc")` 创建了几个字符串对象？」。
- **Q：为什么常量池能安全复用？** → 依赖 String 不可变性，见本文档「String 为什么是不可变的？」。
...
- **Q：常量池的存在前提是什么？** → String 不可变性保证复用安全，见本文档「字符串常量池有什么用？」。
...
- **Q：常量池本身有什么用？** → 复用省内存、加速比较，见本文档「字符串常量池有什么用？」。
