...
![](https://raw.githubusercontent.com/dunwu/images/master/archive/2020/11/7a761b734cb54efd95b27bac02792496.jpg)
...
![](https://raw.githubusercontent.com/dunwu/images/master/archive/2020/11/d89f6ad657644a7196ff56f771777f0c.jpg)
...
![](https://raw.githubusercontent.com/dunwu/images/master/archive/2020/11/0bbda9e89cbb44cfae5d43507fd36bff.jpg)
...
![](https://raw.githubusercontent.com/dunwu/images/master/archive/2020/11/12f14b1925f74965a46be63c450487b9.jpg)
...
![](https://raw.githubusercontent.com/dunwu/images/master/archive/2020/11/7179adc94ae34408babea0bcced891be.jpg)
...
![](https://raw.githubusercontent.com/dunwu/images/master/archive/2020/11/7a865934814f49ae8a3cac84b11fb653.jpg)
...
![](https://raw.githubusercontent.com/dunwu/images/master/archive/2020/11/2ee4e2e0979e4451a762b4ba28a216ed.jpg)
...
- **高并发 Web 服务**：理解 NIO 连接器原理，优化线程池参数，支撑高并发请求处理。
- **WebSocket 长连接**：通过 Upgrade 机制和 WebSocket 支持，实现即时通讯、消息推送等长连接场景。
- **HTTP/2 支持**：Tomcat 9+ 支持 HTTP/2 协议，通过配置 AprOpenSSL 连接器实现多路复用和服务器推送。
- **连接数控制与保护**：通过 LimitLatch 控制最大连接数，防止服务过载崩溃。

## 最佳实践

- **使用 NIO 模式（默认）**：Tomcat 8.5+ 默认 NIO，无需额外配置。避免使用已废弃的 BIO 模式。
- **合理设置 maxConnections**：根据服务器资源调整，默认 10000。如果后端处理能力有限，适当降低以避免堆积。
- **配置 keepAliveTimeout**：设置合理的 keep-alive 超时（如 20s），避免连接长时间空闲占用资源。
- **开启 connectionTimeout**：设置 `connectionTimeout=20000`（20秒），防止慢连接攻击。

## 常见问题

**NIO 和 APR 连接器有什么区别？**

NIO 基于 Java NIO 实现，无需安装额外库，通用性强。APR 基于 C 语言的 OpenSSL，性能略优但需要安装 native 库。大多数场景 NIO 足够。

**为什么连接数没到 maxConnections 但请求变慢？**

可能是线程池 `maxThreads` 不够导致请求排队。连接数和线程数是两个独立参数，连接数控制 TCP 连接，线程数控制并发处理能力。

**Tomcat 如何处理 HTTPS？**

在 Connector 中配置 `SSLEnabled="true"`，指定 keystore 文件和密码。SpringBoot 中只需配置 `server.ssl.*` 属性即可。
