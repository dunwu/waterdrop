...
- **SpringBoot 默认容器选择**：SpringBoot 默认内嵌 Tomcat，如需切换为 Jetty 只需排除 Tomcat 依赖并引入 Jetty starter。
- **嵌入式部署微服务**：将 Tomcat/Jetty 嵌入到应用 JAR 包中，通过 `java -jar` 直接启动，简化部署流程。
- **高并发网关服务**：Spring Cloud Gateway 底层使用 Netty，但传统 Servlet 网关（如 Zuul 1.x）基于 Tomcat/Jetty 运行。

## 最佳实践

- **根据场景选择容器**：Tomcat 生态成熟、文档丰富；Jetty 更轻量、启动更快。一般场景用 Tomcat，高并发/WebSocket 场景考虑 Jetty。
- **使用 NIO 模式**：Tomcat 8.5+ 默认使用 NIO 连接器，比传统 BIO 性能提升显著，确保不要配置为旧的 APR/BIO 模式。
- **调整线程池参数**：根据实际并发量调整 `maxThreads`（默认 200）和 `minSpareThreads`，避免过小导致请求排队。

## 常见问题

**Tomcat 和 Jetty 性能差异大吗？**

在大多数场景下差异不大，Tomcat 8.5+ 的 NIO 模式性能已很优秀。Jetty 在高并发长连接场景下略有优势，但实际性能差异通常不超过 10%。

**如何从 Tomcat 切换到 Jetty？**

在 SpringBoot 中：排除 `spring-boot-starter-tomcat`，引入 `spring-boot-starter-jetty`。配置文件中的 `server.port` 等通用属性无需修改。

## 参考资料

- [Tomcat 官方网站](http://tomcat.apache.org/)
- [Jetty 官方网站](http://www.eclipse.org/jetty/index.html)
