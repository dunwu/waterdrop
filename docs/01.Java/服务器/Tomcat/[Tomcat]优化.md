...
- **生产环境性能调优**：通过调整线程池、连接器、JVM 参数等提升 Tomcat 在高并发场景下的吞吐量和响应速度。
- **启动速度优化**：在容器化部署场景下，优化 Tomcat 启动时间，加快服务就绪速度，缩短滚动更新时间。
- **内存优化**：调整 JVM 堆大小、GC 策略，避免 Full GC 频繁导致的服务停顿。
- **安全加固**：禁用默认应用、隐藏版本信息、配置访问日志，提升生产环境安全性。

## 最佳实践

- **线程池调优**：将 `maxThreads` 设为 500-800（根据服务器 CPU 核数），`minSpareThreads` 设为 maxThreads 的 1/4，`acceptCount` 设为 100-200。
- **启用压缩**：配置 `compression="on"` 开启 Gzip 压缩，设置 `compressableMimeType` 包含 text/html、application/json 等。
- **使用 APR 连接器**：对于静态内容较多的场景，APR 连接器性能优于 NIO，但需要安装额外的 native 库。
- **JVM 参数优化**：生产环境配置 `-Xms` = `-Xmx`，使用 G1GC（`-XX:+UseG1GC`），设置合适的堆大小。

## 常见问题

**Tomcat 启动很慢？**

常见原因：1）`/dev/random` 导致的阻塞（配置 `-Djava.security.egd=file:/dev/./urandom`）；2）webapps 下有过多默认应用；3）JVM 内存不足导致 GC 频繁。

**高并发时请求被拒绝？**

可能原因：1）`maxConnections` 达到上限（NIO 默认 10000）；2）`acceptCount` 队列满了；3）线程池 `maxThreads` 不够。逐步调大这些参数。

**Tomcat 内存溢出的排查思路？**

1）检查 `-Xmx` 是否过小；2）检查是否有内存泄漏（如 ThreadLocal 未清理）；3）分析 Heap Dump（`-XX:+HeapDumpOnOutOfMemoryError`）；4）检查是否有大对象未释放。
