...
> 🎯 目标等级：L2 ｜ ⏱ 建议用时：10 min ｜ 🏷 标签：JavaWeb / OAuth2

#### 💎 关键结论

OAuth 2.0 是授权协议，核心是"委托授权、令牌代密码"：用户不暴露账密，第三方拿 Token 访问资源。四种模式中授权码模式最安全、是绝对主流；Implicit 已被淘汰。授权码 + PKCE 是 SPA/移动端必备，OIDC 在其上补充了身份认证。

#### ⚡记忆卡片

- **口诀**：授权码最安全，PKCE 防截获，OIDC 加认证
- **关键词**：四角色 ／ authorization_code ／ client_secret ／ state ／ PKCE
- **链路**：重定向授权页 → 授权码回调 → 后端换 Token → 持 Token 访问资源
...
- 【L3】`state` 参数用于防 CSRF：回调时校验与发起时一致，防止攻击者伪造回调绑定自己的账号。
- 【L3】PKCE 原理：客户端生成随机 `code_verifier`，发起授权时只传其哈希挑战 `code_challenge`，换 Token 时再提交原文，授权码被截获也无法兑换。
- 【L4】OAuth 2.1 草案进一步移除 Implicit 与密码模式，并强制要求 PKCE。
- 【L4】OIDC 的 `id_token` 是 JWT，客户端可直接解析用户身份，免去 UserInfo 接口调用。

:::

#### 🔀 发散问题

- **Q：OAuth 签发的 Access Token 通常用什么格式？** → JWT，选型与失效机制见本文档「Cookie / Session / Token / JWT 如何选型？」。
- **Q：授权码模式中的重定向与 302 的关系？** → 回调依赖 302 重定向，见本文档「请求转发(forward)和重定向(redirect)的区别？」。
...
- **Q：`sendRedirect` 的底层原理？** → 返回 302 + Location 头，见本文档「请求转发(forward)和重定向(redirect)的区别？」。
...
- **Q：request 作用域的数据如何在转发中共享？** → forward 共享 request 数据，见本文档「请求转发(forward)和重定向(redirect)的区别？」。
...
- **Q：动态包含的底层机制？** → 与 `RequestDispatcher.include()` 相关，转发的区别见本文档「请求转发(forward)和重定向(redirect)的区别？」。
