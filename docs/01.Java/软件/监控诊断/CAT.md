...
![](https://raw.githubusercontent.com/dunwu/images/master/archive/2020/02/b6d069cdc40641498b05ab138b9d081e.png)
...
![](https://raw.githubusercontent.com/dunwu/images/master/archive/2020/02/8702f398b6fa49629cc310f9db5b572f.png)
...
- **实时业务监控**：通过 Transaction 和 Event 消息类型监控核心业务接口（如订单、支付）的 QPS、成功率、响应时间，实时发现业务异常。
- **中间件性能分析**：埋点监控 RPC、数据库、缓存、MQ 等中间件的调用耗时和成功率，快速定位性能瓶颈。
- **告警配置**：配置基于阈值和环比的告警规则，如错误率突增、响应时间超标时自动触发钉钉、邮件、短信告警。
- **容量规划**：通过历史报表数据分析系统容量趋势，为扩容、降级、限流等决策提供数据支撑。

## 最佳实践

- **合理选择消息类型**：Transaction 用于记录耗时操作，Event 用于记录事件次数，Metric 用于业务指标，Heartbeat 用于系统指标，避免滥用导致数据膨胀。
- **控制采样率**：高流量场景下通过采样配置降低数据量，避免监控系统本身成为性能瓶颈；核心接口可配置全量采集。
- **规范命名**：Transaction 的 type 和 name 遵循统一规范（如 type: URL/SQL/CACHE，name: 具体接口名），便于报表聚合和分析。
- **定期清理数据**：配置数据保留策略，定期清理历史报表和原始日志，避免磁盘空间耗尽。

## 常见问题

**CAT 数据量太大导致 HDFS 写满？**

1) 调整采样率，降低非核心接口的数据采集比例；2) 缩短数据保留周期，如从 30 天缩短到 7 天；3) 扩容 HDFS 集群；4) 使用数据压缩或归档到冷存储。

**CAT 客户端对应用性能影响大吗？**

CAT 客户端采用异步批量上报机制，对应用性能影响较小（通常 < 1%）。但如果埋点过多或采样率过高，仍可能增加 CPU 和网络开销。建议只在关键路径埋点，非核心接口降低采样率。
