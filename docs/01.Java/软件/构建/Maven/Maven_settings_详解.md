...
- **多环境配置**：通过 profiles 定义开发、测试、生产环境的仓库地址、插件配置等，使用 `-P` 参数激活对应环境。
- **私服配置**：在 settings.xml 中配置 Nexus/Artifactory 私服地址和认证信息，避免在每个项目的 pom.xml 中重复配置。
- **镜像加速**：配置阿里云等国内镜像加速依赖下载，解决访问 Maven Central 速度慢的问题。
- **团队统一配置**：将 settings.xml 放在共享目录或版本控制中，确保团队成员使用统一的仓库、镜像和配置。

## 最佳实践

- **分离用户和全局配置**：个人开发环境差异放 user settings（~/.m2/settings.xml），团队统一配置放 global settings（${maven.home}/conf/settings.xml）。
- **敏感信息加密**：使用 `mvn --encrypt-master-password` 和 `mvn --encrypt-password` 加密 settings.xml 中的密码，避免明文存储。
- **避免硬编码路径**：使用 `${user.home}`、`${maven.home}` 等内置变量，使 settings.xml 跨平台兼容。
- **定期清理 localRepository**：定期删除本地仓库中的 SNAPSHOT 版本和过期缓存，避免磁盘占用和版本冲突。

## 常见问题

**settings.xml 中 mirror 和 repository 的区别？**

mirror 是仓库的镜像，会完全替代原始仓库地址；repository 是定义一个新的仓库。mirror 的 id 必须和原仓库 id 一致，而 repository 可以自定义 id。

**为什么配置的 profile 没有生效？**

profile 需要被激活才会生效。可以通过 `<activeProfiles>` 显式激活，或在 profile 中定义 `<activation>` 条件（如 JDK 版本、操作系统、属性值等）自动激活。
