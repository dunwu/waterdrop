...
- 【L3】HTTP 与 RPC 的性能差异根源：HTTP 文本协议 + 短连接开销大，Dubbo 默认二进制协议 + TCP 长连接 + 连接复用，内部高频调用场景吞吐高得多，见本文档「HTTP 与 RPC 有什么区别？」。
...
- **Q：HTTP 和 RPC 的性能差异根源？** → 文本 vs 二进制、短连接 vs 长连接，见本文档「HTTP 与 RPC 有什么区别？」。
...
- **Q：具体到框架怎么选？** → 对外 Feign（HTTP），内部 Dubbo（RPC），见本文档「Feign 和 Dubbo 有什么区别？」。
- **Q：网关层用的什么协议？** → Gateway 基于 HTTP/HTTPS（WebFlux），见本文档「什么是 Spring Cloud Gateway？」。
