...
**源码定位**：Sentinel 入口是 `SphU.entry()`，熔断状态机在 `com.alibaba.csp.sentinel.slots.block.degrade.circuitbreaker.CircuitBreaker`（`AbstractCircuitBreaker#tryPass` 处理半开探测），统计基于 `LeapArray` 滑动窗口；Resilience4j 对应 `io.github.resilience4j.circuitbreaker.CircuitBreaker`（核心方法 `acquirePermission`/`onSuccess`/`onError`）；Hystrix 则是 `HystrixCommand#run` + `HystrixThreadPoolProperties` 线程池隔离。故障期间拿到的下游实例地址依赖注册中心的本地缓存与心跳剔除（Nacos/Eureka 内部原理属分布式协同领域，此处一句带过不展开）。
...
1. **超时未设或过大**：Feign 默认 read timeout 是 60 秒（`feign.Request.Options` 默认值），下游假死时每个请求占用上游线程 60 秒，Tomcat 默认 200 个线程很快耗尽。经验法则：超时 = 下游接口 P99 × 2，且不超过上游剩余时间预算的一半。
2. **重试放大故障**：对已故障的下游重试 3 次等于流量放大 3 倍，加速压垮；重试只适合幂等接口 + 偶发网络抖动，且必须与熔断配合。
3. **半开探测被误判**：Sentinel 半开状态只放行 1 个探测请求，若它恰好打到下游唯一故障实例或本身是慢接口，会立刻重新熔断，熔断器反复振荡。可改用 Resilience4j 的 `permittedNumberOfCallsInHalfOpenState`（建议 5-10 次）降低误判。
4. **最小调用数未达标**：异常比例阈值需最小调用数支撑（如 Resilience4j `minimumNumberOfCalls=20`），低流量时即使全失败也不触发熔断。
5. **熔断粒度过粗**：一个熔断器包打所有接口，一个慢接口熔断后正常接口也被连带熔断；应按接口或下游服务维度拆分资源。
6. **降级链路同源**：fallback 查的备用库与被熔断的下游在同一机房/同一实例组，故障时一起挂，降级形同虚设。
...
1. Sentinel 的半开状态与 Hystrix 有何不同？熔断器“振荡”（反复熔断恢复）怎么破？
   两者半开都只放行少量探测请求（Sentinel 是 1 个），探测成功关闭熔断、失败重新计时。振荡的本质是探测样本太少 + 下游处于不稳定边缘态：可增大半开探测次数（Resilience4j `permittedNumberOfCallsInHalfOpenState`）、拉长熔断时长、优先用慢调用比例而非单次异常判定，同时修复下游根因才是治本。
2. 线程池隔离每次调用多一次线程切换，为什么 Hystrix 还默认用它？什么场景必须改信号量隔离？
   线程池的核心价值是支持超时中断（能主动丢弃卡住的调用）和彻底的资源隔离，适合一般 RPC 容错场景。但对高频低延迟调用（数千 QPS、毫秒级 RT），上下文切换开销占比过大，必须改信号量（并发数）隔离；Sentinel 本身选择了信号量隔离 + 并发线程数限流的轻量路线，代价是失去超时中断能力，需用慢调用熔断弥补。
3. 一个合格的降级 fallback 应满足什么条件？为什么“直接返回 null”不算真降级？
   三条：① 有业务含义（默认值、缓存旧数据、引导页），用户可感知但可接受；② 不依赖同一故障源，fallback 链路必须与被熔断的下游物理隔离；③ 可观测，降级次数进监控告警。直接返回 null 会把异常转移给上层或前端，往往引发 NPE 二次事故；降级是“有计划的损失”，不是“把问题抛出去”。

#### 场景题

**场景**：凌晨告警：核心订单服务 RT 从 200ms 飙到 8 秒，错误率 30%，上游网关线程池快打满。你初步判断是下游库存服务引起，如何在 5 分钟内决策止血？
...
**长期方案**：① 所有同步调用默认挂慢调用比例熔断 + 预置好的 fallback，不允许裸调用上线；② 超时规范（P99 × 2）纳入发布 checklist，代码评审必查；③ 库存扣减等强依赖评估改 MQ 异步 + 最终一致；④ 定期故障演练（主动注入下游延迟），验证熔断与降级真能触发，而不是只存在于文档里。
...
- **服务注册发现**：使用 AP 模式（默认），服务实例是临时的，优先保证可用性。
- **配置管理**：Nacos 配置中心内部使用 CP 模式（Raft），保证配置变更的一致性。
- Nacos 1.3.0+ 以后，临时实例用 Distro（AP），持久实例用 Raft（CP），自动选择，无需手动切换。
  ::::
...
- **重试与幂等性**：只有幂等接口（GET、PUT）才适合开启重试，非幂等接口（POST）重试可能导致重复创建。
- **超时级联**：上游服务的超时应**大于**下游服务超时 + 重试时间，避免上游先超时导致级联失败。
- **熔断配合**：重试次数过多可能加剧下游压力，应配合熔断器使用。
  ::::
