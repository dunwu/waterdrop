...
- **企业级 Web 应用**：基于 Spring MVC + Spring + MyBatis 构建分层架构，通过 IoC 管理各层 Bean，用 AOP 实现事务、日志、权限等横切关注点。
- **微服务架构**：Spring Boot 快速构建独立微服务，Spring Cloud 提供服务发现、配置中心、网关等治理能力，实现分布式系统的高效开发。
- **数据访问应用**：通过 Spring Data 模块统一访问关系型数据库、Redis、MongoDB、Elasticsearch 等多种数据存储，屏蔽底层差异。
- **批处理应用**：基于 Spring Batch 处理大批量数据的导入、转换、导出任务，支持断点续传、失败重试等企业级特性。

## 最佳实践

- **遵循 POJO 设计原则**：业务组件不依赖框架特定接口，便于单元测试和框架迁移。
- **优先使用构造器注入**：相比字段注入，构造器注入保证依赖不可变且不为 null，避免隐藏的空指针问题。
- **合理使用 AOP**：仅将日志、事务、权限等横切关注点抽取为切面，避免将业务逻辑放入切面。
- **模块化引入 Spring 依赖**：按需引入 `spring-web`、`spring-data` 等模块，避免引入整个 Spring 导致依赖膨胀。

## 常见问题

- **Spring 为什么是轻量级框架？** Spring 核心 JAR 仅约 2MB，无需应用服务器即可运行，采用 POJO 编程模型，不侵入业务代码。
- **Spring 和 Spring MVC 是什么关系？** Spring 是基础框架（提供 IoC/AOP），Spring MVC 是 Spring 的 Web 模块，专门处理 HTTP 请求和视图渲染。
- **Spring、Spring Boot、Spring Cloud 有什么关系？** Spring 做基础，Spring Boot 做单服务快速开发，Spring Cloud 做微服务集群治理，三者形成递进关系。

## 参考资料

- [Spring 官方文档](https://docs.spring.io/spring-framework/docs/current/reference/html/)
- [《Spring 实战（第 5 版）》](https://book.douban.com/subject/30452609/)
