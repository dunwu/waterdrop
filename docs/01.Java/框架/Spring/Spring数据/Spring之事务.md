...
![](https://raw.githubusercontent.com/dunwu/images/master/archive/2022/09/a5bdd419fda942a18e3c213344d97561.png)
...
![](https://raw.githubusercontent.com/dunwu/images/master/archive/2022/09/f4ac753587944f9da4318d716d9c7b41.png)
...
![](https://raw.githubusercontent.com/dunwu/images/master/archive/2022/09/7e0d35b727ca4fa1898faf2a0b9b4eb1.png)
...
![](https://raw.githubusercontent.com/dunwu/images/master/archive/2022/09/a98d622d71444a7d913c6c40746af554.png)
...
![](https://raw.githubusercontent.com/dunwu/images/master/archive/2020/08/8ec6f288a50241aaa128ac9de90cecae.png)
...
- **订单处理**：在下单时同时操作订单表、库存表、账户表，保证数据一致性。
- **转账操作**：转出和转入操作必须原子执行，任一步失败则全部回滚。
- **分布式事务**：结合 Seata 或消息队列实现跨服务的事务一致性。
- **嵌套事务控制**：通过 `REQUIRES_NEW` 传播级别在独立事务中记录审计日志，不受主事务回滚影响。

## 最佳实践

- **事务粒度要适中**：避免事务范围过大导致锁竞争严重，仅包裹必要的数据库操作。
- **只读事务使用 `readOnly=true`**：对纯查询方法标记只读事务，优化数据库性能。
- **避免在事务中执行 RPC 调用**：网络超时会导致事务长时间占用连接，拖垂数据库。
- **注意 `@Transactional` 失效场景**：同类内部调用、非 public 方法、异常被吞掉时事务不会回滚。
