...
`@Transactional` 基于 **Spring AOP 动态代理**实现，核心流程如下：

1. **代理创建**：Spring 容器启动时，`ProxyTransactionManagementConfiguration` 注册 `TransactionInterceptor`（事务拦截器），对标注 `@Transactional` 的 Bean 创建代理对象。
2. **拦截调用**：调用事务方法时，代理对象拦截调用，进入 `TransactionInterceptor#invoke()`。
3. **开启事务**：通过 `PlatformTransactionManager` 获取数据库连接，设置 `autoCommit=false`，开启事务。
4. **执行业务**：通过反射调用目标方法，执行业务逻辑。
5. **提交或回滚**：
   - 正常返回：提交事务。
   - 抛出异常：根据 `rollbackFor` 配置判断是否回滚（默认只回滚 `RuntimeException` 和 `Error`）。
6. **释放资源**：释放数据库连接，恢复 `autoCommit` 状态。

**关键源码片段**（`TransactionInterceptor#invoke`）：
