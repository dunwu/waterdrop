...
- **定时任务调度**：通过 `@Scheduled` 实现定时报表生成、数据同步、缓存刷新等任务。
- **延迟任务执行**：通过 `fixedDelay` 或 `initialDelay` 实现启动后延迟执行的任务。
- **复杂调度策略**：通过 Cron 表达式实现每周一、每月 1 号等复杂调度规则。
- **异步任务执行**：结合 `@Async` 将调度任务放入独立线程池执行，避免阻塞主线程。

## 最佳实践

- **配置独立线程池**：避免使用默认单线程，通过 `TaskScheduler` 配置多线程防止任务互相阻塞。
- **Cron 表达式测试**：使用在线工具验证 Cron 表达式是否符合预期。
- **分布式环境防重复执行**：多实例部署时，使用分布式锁或 `ShedLock` 防止任务重复执行。
- **记录任务执行日志**：监控任务执行时间和结果，及时发现异常。
