...
![](https://raw.githubusercontent.com/dunwu/images/master/archive/2022/10/6990fb67f30c4f31997201ed55d308d4.png)
...
![](https://raw.githubusercontent.com/dunwu/images/master/archive/2020/07/b060f41ae1e74bfcb7d43cad537d121e.png)
...
- **企业级 Web 应用**：通过 IoC 容器统一管理 Controller、Service、DAO 层 Bean，实现分层解耦，降低模块间耦合度。
- **微服务架构**：每个微服务独立维护自己的 IoC 容器，通过接口注入实现服务间的松耦合协作。
- **插件化架构**：利用 `@Conditional` 条件装配，根据运行环境动态加载不同的 Bean 实现，支持功能模块的热插拔。
- **测试驱动开发**：IoC 容器的依赖注入特性使单元测试可通过 Mock 对象替换真实依赖，实现隔离测试。

## 常见问题

- **循环依赖如何解决？** Spring 通过三级缓存解决 setter 注入的循环依赖；构造器注入的循环依赖无法解决，需改为 setter 注入或使用 `@Lazy`。
- **`BeanFactory` 和 `ApplicationContext` 有什么区别？** `BeanFactory` 是基础容器，提供基本的 DI 功能；`ApplicationContext` 是其子接口，增加了国际化、事件发布、资源访问等企业级功能，实际开发推荐使用后者。
- **如何避免 Bean 的循环依赖？** 架构设计上避免循环引用，或使用 `@Lazy` 延迟加载、事件驱动、`ApplicationContextAware` 等方式解耦。
- **Spring 的 IoC 容器启动慢如何优化？** 可通过 `@Lazy` 延迟加载非关键 Bean、减少不必要的组件扫描、优化 Bean 初始化逻辑等方式加速。
