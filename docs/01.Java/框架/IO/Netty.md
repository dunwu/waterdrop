...
- **使用池化的 Direct Buffer**：开启 `PooledByteBufAllocator` 减少内存分配开销，避免频繁 GC。
- **合理设置 Pipeline 中 Handler 顺序**：编解码器在前、业务处理器在后，避免不必要的解码开销。
- **注意资源释放**：确保 `ByteBuf` 在使用后正确释放（调用 `ReferenceCountUtil.release()`），防止内存泄漏。
- **使用心跳机制维持长连接**：通过 `IdleStateHandler` 检测空闲连接，配合心跳包保持连接活性。
- **优雅关闭**：调用 `shutdownGracefully()` 而非 `shutdown()`，确保在关闭前处理完队列中的任务。
