...
- **中小型项目快速开发**：MyBatis 学习曲线平缓，配置简单，适合中小型项目快速迭代开发。
- **需要精细 SQL 控制的场景**：对于复杂的报表查询、多表关联查询，MyBatis 允许直接编写优化 SQL，比全自动 ORM 更灵活。
- **存量数据库系统**：对于已有大量存储过程或复杂 SQL 的老项目，MyBatis 可以直接复用现有 SQL，迁移成本低。
- **互联网高并发场景**：结合连接池、缓存、读写分离等机制，MyBatis 在高并发互联网应用中表现出色。

## 最佳实践

- **使用 MyBatis-Plus 提升效率**：MyBatis-Plus 提供通用 CRUD、分页插件、代码生成器等功能，可显著减少重复工作。
- **SQL 集中在 XML 中**：将 SQL 语句写在 Mapper XML 文件中而非注解中，便于维护、审核和复杂 SQL 编写。
- **使用 PageHelper 分页插件**：避免手动拼接分页 SQL，支持多数据库方言，简化分页逻辑。
- **配置合理的缓存策略**：单表查询开启二级缓存，多表关联查询关闭缓存以防止数据不一致。
- **使用 `<sql>` 片段复用 SQL**：将常用的查询字段、条件抽取为 SQL 片段，通过 `<include>` 引用。

## 常见问题

**MyBatis 与 Hibernate 如何选择？**

MyBatis 适合需要精细 SQL 控制、团队 SQL 能力强、项目复杂的场景；Hibernate 适合对象模型丰富、CRUD 操作多、追求开发效率的场景。中小型团队和互联网项目通常更倾向 MyBatis。

**`#{}` 和 `${}` 的区别是什么？**

`#{}` 是预编译参数占位符，MyBatis 会将其替换为 `?` 然后通过 `PreparedStatement` 设置参数，可防止 SQL 注入。`${}` 是字符串替换，直接将参数值拼接到 SQL 中，仅用于动态表名、列名等场景，需注意安全风险。

**MyBatis 如何实现多数据源配置？**

可通过 `AbstractRoutingDataSource` 实现动态数据源路由，在运行时根据业务需求切换数据源。结合 AOP 注解或 ThreadLocal 实现数据源上下文传递。分库分表场景可结合 ShardingSphere 实现。
