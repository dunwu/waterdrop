...
在 Java 中，**JVM 可以理解的代码就叫做字节码**（即扩展名为 `.class` 的文件），它不依赖任何特定的处理器，只面向虚拟机。Java 语言通过字节码的方式，在一定程度上解决了传统解释型语言执行效率低的问题，同时又保留了解释型语言可移植的特点。所以， Java 程序运行时相对来说还是高效的（不过，和 C、 C++，Rust，Go 等语言还是有一定差距的），而且，由于字节码并不针对一种特定的机器，因此，Java 程序无须重新编译便可在多种不同操作系统的计算机上运行。
...
`BigDecimal` 可以实现对浮点数的运算，不会造成精度丢失。通常情况下，大部分需要浮点数精确运算结果的业务场景（比如涉及到钱的场景）可以通过 `BigDecimal` 来处理。
...
| **维度**       | **成员变量**                            | **局部变量**                      |
| :------------- | :-------------------------------------- | :-------------------------------- |
| **归属**       | 属于类或对象实例                        | 属于方法/代码块/参数              |
| **修饰符**     | 可被 `public`/`private`/`static` 等修饰 | 仅支持 `final`                    |
| **final 修饰** | 必须显式初始化（若被 `final` 修饰）     | 可被 `final` 修饰，无需强制初始化 |

**内存分配与生命周期**

| **维度**     | **成员变量**                                               | **局部变量**                   |
| :----------- | :--------------------------------------------------------- | :----------------------------- |
| **存储位置** | `static` 成员在方法区；非 `static` 在堆内存                | 栈内存                         |
| **生命周期** | 随对象创建/销毁（非 `static`） 或随类加载/卸载（`static`） | 随方法调用开始/结束而创建/销毁 |

**初始化规则**

| **维度**       | **成员变量**                            | **局部变量**                     |
| :------------- | :-------------------------------------- | :------------------------------- |
| **默认值**     | 自动赋类型默认值（如 `int` 默认为 `0`） | 必须手动初始化，否则编译报错     |
| **final 要求** | 必须显式赋值（直接初始化或通过构造器）  | 使用前赋值即可，无强制初始化要求 |
...
- 类/对象级作用域，支持多种修饰符
- 内存开销较大，生命周期长
- 自动初始化，`final` 修饰时需强制赋值

**局部变量**：

- 方法级作用域，仅支持 `final`
- 内存高效，生命周期随方法调用，只是临时存在
- 必须手动初始化，无默认值
...
| **带参方法**     | 可接受基本类型/对象参数   | `void setAge(int age)`       |
| **可变参数方法** | 参数数量可变（`...`语法） | `void print(String... strs)` |
