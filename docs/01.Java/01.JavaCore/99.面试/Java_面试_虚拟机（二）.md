...
**Minor GC **是专门针对**年轻代**（Young Generation，包括 Eden 和 Survivor 区）的垃圾回收过程。Minor GC 的目标是**高效回收短期对象**，避免频繁扫描老年代，减少全局停顿时间。
...
### 【中等】为什么 Java 中某些年轻代和老年代的垃圾收集器不能组合使用？比如 ParNew 和 Parallel Old
...
**G1 垃圾回收器采用记忆集（Remembered Set）机制**。
