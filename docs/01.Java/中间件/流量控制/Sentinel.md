...
- **流量控制**：控制 QPS/并发数阈值，防止系统被过多请求压垂，适用于秒杀、促销等高并发场景。
- **熔断降级**：当依赖服务不稳定时自动熔断，返回降级数据，避免级联故障。
- **热点参数限流**：针对特定热点参数（如商品 ID、用户 ID）进行限流，防止热点数据击穿。
- **系统自适应保护**：根据系统负载（Load、CPU 使用率、入口流量等）自动调整限流策略，保障系统稳定。
- **网关限流**：在 Spring Cloud Gateway 或 Zuul 中集成 Sentinel，实现 API 网关层的统一限流。

## 最佳实践

- **合理配置流控规则**：根据压测结果确定 QPS 阈值，避免阈值过低影响正常业务或过高失去保护作用。
- **使用动态规则源**：将流控规则持久化到 Nacos、Apollo 或 ZooKeeper，支持运行时动态调整而无需重启。
- **结合 Dashboard 使用**：通过 Sentinel Dashboard 实时监控资源流量、熔断状态，并可视化配置规则。
- **区分资源粒度**：将核心资源和非核心资源分别配置不同的限流策略，优先保障核心服务。
- **预热流量**：对于启动较慢的服务，使用“预热”模式的流控规则，逐步增加流量避免冷启动问题。

## 常见问题

**Sentinel 与 Hystrix 的核心区别是什么？**

Sentinel 基于滑动窗口算法统计流量数据，支持更丰富的流控规则（流量控制、熔断降级、热点参数、系统自适应）；Hystrix 基于线程池隔离，功能相对单一。Sentinel 资源开销更小，更适合国内互联网场景。

**Sentinel 如何实现流量统计？**

Sentinel 使用滑动时间窗口算法，默认 1 秒分为 2 个 500ms 的样本窗口。通过 `LeapArray` 数据结构实现高效的时间窗口滑动，在保证统计精度的同时控制内存开销。

**Sentinel 规则不生效怎么排查？**

检查：1）资源名称是否一致；2）规则是否成功加载（查看 `sentinel-record.log`）；3）是否使用了异步调用（需手动传递 context）；4）Dashboard 与应用的端口通信是否正常。
