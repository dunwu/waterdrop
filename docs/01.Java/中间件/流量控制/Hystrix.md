...
- Alibaba Sentinel
  ...
  ![](https://raw.githubusercontent.com/dunwu/images/master/202602082221986.webp)
  ...
  ![](https://raw.githubusercontent.com/dunwu/images/master/202602082221001.webp)
  ...
  ![](https://raw.githubusercontent.com/dunwu/images/master/202602082221254.webp)另外，
...
- **服务降级**：当依赖服务不可用或响应超时时，返回默认值或缓存数据，保证核心业务不受影响。
- **服务熔断**：当错误率达到阈值时自动熔断，避免级联故障，一段时间后尝试半开状态探测恢复。
- **服务隔离**：通过线程池隔离或信号量隔离，防止单个服务的延迟耗尽整个系统的线程资源。
- **服务限流**：在高并发场景下通过限流控制请求速率，保护系统不被压垂。

## 最佳实践

- **合理配置超时时间**：超时时间应略高于服务的平均响应时间，避免正常请求被熔断。建议为平均响应时间的 2-3 倍。
- **使用线程池隔离而非信号量隔离**：对于远程调用场景，线程池隔离可以提供更好的隔离效果和资源保护。
- **配置合理的熔断阈值**：`circuitBreaker.errorThresholdPercentage` 默认 50%，可根据业务重要性调整。
- **fallback 方法必须有意义**：fallback 应返回有用的降级数据（如缓存、默认值），而非简单抛异常或返回 null。
- **结合仪表盘监控**：通过 Hystrix Dashboard + Turbine 实时监控熔断器状态、请求量、错误率等指标。

## 常见问题

**Hystrix 和 Sentinel 如何选择？**

Hystrix 已进入维护模式（不再活跃开发），且基于线程池隔离，资源开销较大；Sentinel 是阿里巴巴开源的活跃项目，支持更丰富的流控规则（流量控制、熔断降级、热点参数限流等），且资源开销更小。新项目推荐使用 Sentinel。

**Hystrix 线程池隔离和信号量隔离的区别？**

线程池隔离为每个服务创建独立的线程池，支持超时和异步，但线程上下文切换开销大；信号量隔离基于计数器，轻量级但不支持超时，仅适合同步调用且服务时间可控的场景。

**熔断器的工作机制是怎样的？**

熔断器有三种状态：Closed（正常）、Open（熔断）、Half-Open（半开）。当错误率达到阈值时转为 Open，拒绝所有请求；等待 `sleepWindow` 时间后转为 Half-Open，允许少量探测请求；若探测成功则恢复为 Closed，否则重新 Open。
