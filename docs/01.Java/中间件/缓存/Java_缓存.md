...
- **Spring Boot 应用缓存**：通过 `@Cacheable`、`@CachePut`、`@CacheEvict` 注解实现声明式缓存，简化缓存代码。
- **数据库查询缓存**：缓存频繁查询但更新不频繁的数据（如用户信息、商品详情），减轻数据库压力。
- **多级缓存架构**：通过 JetCache 等框架实现本地缓存 + 分布式缓存的多级缓存，兼顾性能和一致性。
- **接口结果缓存**：对第三方 API 调用结果进行缓存，避免重复请求，降低接口延迟。

## 最佳实践

- **优先使用 Spring Cache 抽象**：通过 `@Cacheable` 注解实现缓存，与底层缓存实现（Redis、Caffeine、Ehcache）解耦。
- **合理设置缓存过期时间**：根据数据更新频率设置 TTL，避免缓存长期不更新或频繁失效。
- **注意缓存一致性**：更新数据时同步清除或更新缓存，使用 `@CacheEvict` 或 `@CachePut` 确保数据一致。
- **防范缓存穿透、击穿、雪崩**：使用空值缓存防止穿透、互斥锁防止击穿、随机过期时间防止雪崩。
- **生产环境使用 Redis 作为缓存后端**：结合 Spring Data Redis，支持集群、持久化、发布订阅等高级特性。

## 常见问题

**Spring Cache 与直接使用 Redis 有什么区别？**

Spring Cache 是缓存抽象层，通过注解简化缓存操作，但功能有限（不支持复杂的缓存策略）；直接使用 Redis（如 RedisTemplate、Jedis）更灵活，可以实现分布式锁、发布订阅、延迟队列等高级功能。

**缓存穿透、击穿、雪崩如何解决？**

穿透：缓存空值或布隆过滤器；击穿：互斥锁（`SETNX`）或逻辑过期；雪崩：过期时间加随机值、多级缓存、服务降级。

**JetCache 和 Spring Cache 如何选择？**

Spring Cache 是标准抽象，集成简单，适合大多数场景；JetCache 支持多级缓存、缓存统计、自动刷新等高级功能，适合对缓存有精细控制需求的场景。
