...
![](https://raw.githubusercontent.com/dunwu/images/master/archive/2022/09/61412782d82d4b3eadaf84d85b4ce852.png)
...
![](https://raw.githubusercontent.com/dunwu/images/master/archive/2022/09/c17218f8e0cf407a94249822d967373e.png)
...
- **高并发 Web 应用**：通过连接池复用数据库连接，避免每次请求都创建/销毁连接，显著提升响应速度。
- **微服务架构**：每个微服务配置独立的连接池，通过 HikariCP 等高性能连接池确保服务间数据库访问的高效性。
- **数据访问层优化**：结合 Spring Data JPA/MyBatis，连接池作为底层基础设施提供稳定的数据库连接管理。
- **多数据源场景**：通过配置多个 DataSource Bean 实现读写分离、主从切换等多数据源访问。

## 最佳实践

- **首选 HikariCP**：HikariCP 是当前性能最优的开源连接池，Spring Boot 2.x+ 默认使用。避免使用 C3P0（性能较差）。
- **合理配置池大小**：`maximum-pool-size` 建议设置为 CPU 核数 × 2 + 磁盘数，过大反而降低性能。
- **设置合理的超时时间**：`connection-timeout` 设置为 5-30 秒，避免长时间等待；`idle-timeout` 和 `max-lifetime` 根据数据库配置调整。
- **开启连接检测**：配置 `connection-test-query` 或使用 JDBC4 驱动自动检测，避免获取到已失效的连接。
- **监控连接池状态**：通过 Spring Boot Actuator 或 JMX 监控连接池的使用率、等待队列等指标。

## 常见问题

**为什么需要数据库连接池？**

数据库连接的创建和销毁是耗时操作（涉及 TCP 握手、认证等），连接池通过复用连接显著减少开销。同时连接池可以控制最大并发连接数，防止数据库因连接过多而过载。

**HikariCP 为什么比其他连接池快？**

HikariCP 通过字节码精简、无锁设计、自定义集合类、避免不必要的 synchronized 等优化手段，实现了极低的开销。其核心代码量小、设计精简，避免了其他连接池的过度抽象。

**连接泄露如何排查？**

开启 HikariCP 的 `leak-detection-threshold`（如 60000ms），超过该时间未归还的连接会被记录警告日志。结合 APM 工具分析慢查询和长事务，定位泄露根源。
