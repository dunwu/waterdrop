...
> 不了解 Java 默认的序列化机制，可以参考：[Java 序列化](https://dunwu.github.io/waterdrop/pages/ce9efc62/)
...
- **RPC 远程调用**：Dubbo、gRPC 等 RPC 框架使用二进制序列化（如 Hessian、Protobuf）传输数据，性能优于 JSON。
- **分布式缓存存储**：Redis 等缓存系统中使用 JDK 序列化或 Protostuff 存储 Java 对象。
- **消息中间件**：在 Kafka、RocketMQ 中使用二进制格式传输消息，提升吞吐量。
- **文件存储**：将 Java 对象序列化为字节流持久化到文件或数据库。

## 最佳实践

- **优先使用 Protobuf/Protostuff**：性能优于 JDK 序列化，且生成的字节流更小。
- **避免 JDK 原生序列化**：JDK 序列化存在安全漏洞（反序列化漏洞），且生成的字节流大、性能差。
- **实现 `Serializable` 时指定 `serialVersionUID`**：避免类结构变更导致反序列化失败。
- **注意 `transient` 关键字**：敏感字段或不需要序列化的字段使用 `transient` 标记。

## 常见问题

**JDK 序列化和 Protobuf 有什么区别？**

JDK 序列化生成的字节流大、性能差，且存在安全漏洞；Protobuf 是 Google 开发的高效二进制序列化协议，字节流小、跨语言、性能优秀，但需要定义 `.proto` 文件。

**为什么 JDK 反序列化会有安全问题？**

JDK 反序列化会执行对象的 `readObject()` 方法，恶意构造的字节流可能触发任意代码执行。解决方案：使用白名单过滤、升级 JDK、或使用更安全的序列化框架。

**`serialVersionUID` 的作用是什么？**

`serialVersionUID` 是序列化版本号，用于验证序列化与反序列化的类是否一致。如果不显式指定，JVM 会根据类结构自动生成，类结构变更会导致自动生成的 UID 变化，从而反序列化失败。
