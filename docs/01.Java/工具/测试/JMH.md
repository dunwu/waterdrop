...
- **API 性能对比**：比较同一功能的不同实现（如 StringBuilder vs String 拼接、Jackson vs Gson 序列化）的吞吐量和延迟。
- **热点函数优化验证**：对已通过 Profiler 定位的热点方法进行优化后，使用 JMH 定量验证优化效果。
- **并发性能评估**：通过 `@Threads` 和 `@State` 测试数据结构在不同并发度下的性能表现，如 ConcurrentHashMap 的读写性能。
- **算法复杂度验证**：对不同输入规模（通过 `@Param`）测试算法的实际执行时间，验证理论复杂度与实际表现是否一致。

## 最佳实践

- **充分预热**：JIT 编译需要调用次数积累，设置足够的 Warmup iterations（建议 3–5 轮），确保测量阶段已进入稳定状态。
- **避免死码消除**：Benchmark 方法的返回值或状态必须被使用，否则 JIT 会消除无用代码。使用 `Blackhole.consume()` 或返回结果。
- **Fork 多个进程**：设置 `@Fork(2)` 以上，避免单次 JVM 启动的偶然因素影响，提高结果的可重复性。
- **选择合适的 Mode**：延迟敏感场景用 `AverageTime` / `SampleTime`，吞吐量场景用 `Throughput`，冷启动场景用 `SingleShotTime`。

## 常见问题

**测试结果显示 Score 差异很大？**

可能是预热不足、后台进程干扰、CPU 调频等导致。确保预热充分，关闭无关进程，将 CPU 设置为性能模式，多次 Fork 取平均值。

**JMH 和 `System.currentTimeMillis()` 手动计时的区别？**

手动计时精度低，受 JIT 编译、GC 停顿、死码消除等影响。JMH 提供纳秒级精度，自动处理预热、分叉、统计，结果更可靠。

**为什么不能用 `System.out.println` 在 Benchmark 方法中？**

IO 操作会严重影响测量结果，且 JIT 无法对含副作用的方法进行优化。应使用 `Blackhole` 消费结果或让方法返回计算结果。
