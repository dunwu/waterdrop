---
title: Java 其他工具
date: 2026-07-21 22:34:30
categories:
  - Java
  - 工具
  - 其他
tags:
  - Java
  - 工具
permalink: /pages/f4d82915/
hidden: true
index: false
---

# Java 其他工具

> 收录 Java 开发中常用但不便于归类的工具库，包括日志、工具包、反射、邮件、HTML 解析、图片处理、条形码等。

## 📖 内容

- [Java 日志](Java日志.md) - 关键词：`JCL`、`Log4j`、`Log4j2`、`Logback`、`SLF4J`
- [Java 工具包](Java工具包.md) - 关键词：`commons-lang`、`commons-collections`、`commons-io`、`guava`
- [Reflections](Reflections.md) - 关键词：`反射`、`注解扫描`、`类扫描`
- [JavaMail](JavaMail.md) - 关键词：`邮件`、`SMTP`、`POP3`、`IMAP`、`MIME`
- [Jsoup](Jsoup.md) - 关键词：`HTML 解析`、`DOM`、`CSS 选择器`、`数据抓取`
- [Thumbnailator](Thumbnailator.md) - 关键词：`图片处理`、`缩放`、`旋转`、`水印`
- [ZXing](Zxing.md) - 关键词：`条形码`、`二维码`、`QR Code`、`编解码`

## 📚 资料

- [Apache Commons 官网](https://commons.apache.org/)
- [Guava Github](https://github.com/google/guava)
- [SLF4J 官网](http://www.slf4j.org/)
- [Logback 官网](http://logback.qos.ch/)
- [Log4j2 官网](https://logging.apache.org/log4j/2.x/)
- [Reflections Github](https://github.com/ronmamo/reflections)
- [JavaMail 官网](https://javaee.github.io/javamail/)
- [Jsoup 官网](https://jsoup.org/)
- [Thumbnailator Github](https://github.com/coobird/thumbnailator)
- [ZXing Github](https://github.com/zxing/zxing)

## 🚪 传送

◾ 💧 [钝悟的 IT 知识图谱](https://dunwu.github.io/waterdrop/) ◾ 🎯 [钝悟的博客](https://dunwu.github.io/blog/) ◾
