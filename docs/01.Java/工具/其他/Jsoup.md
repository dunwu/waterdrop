...
- **网页数据爬取**：解析网页 HTML，通过 CSS 选择器提取商品价格、新闻标题、评论内容等结构化数据。
- **HTML 内容清洗（XSS 防护）**：使用 `Jsoup.clean()` + `Whitelist` 过滤用户输入的 HTML，防止跨站脚本攻击。
- **SEO 分析工具**：解析页面的 meta 标签、标题、链接结构，用于 SEO 检查和网站审计。
- **HTML 转 Markdown/纯文本**：将富文本 HTML 转换为纯文本或 Markdown 格式，用于内容导出或搜索索引。

## 最佳实践

- **指定 baseUri 解析相对链接**：解析网页时传入 baseUri，通过 `attr("abs:href")` 获取绝对路径 URL。
- **使用 CSS 选择器而非 DOM 遍历**：`doc.select("div.content > p")` 比逐层 `getElementById` + `getElementsByTag` 更简洁高效。
- **设置合理超时时间**：通过 `Jsoup.connect(url).timeout(5000)` 设置超时，避免网络异常时线程阻塞。
- **异常处理不可忽略**：`connect().get()` 会抛出 IOException，需捕获处理网络异常、404、500 等场景。

## 常见问题

**解析中文网页乱码？**

确保指定正确的字符编码：`Jsoup.parse(file, "UTF-8", baseUri)`。对于网络请求，Jsoup 会自动从 HTTP 头和 meta 标签检测编码，但某些网站声明不一致时需手动指定。

**Jsoup 能执行 JavaScript 吗？**

Jsoup 只解析静态 HTML，不执行 JavaScript。如果页面内容通过 JS 动态渲染，需使用 Selenium/Playwright 等浏览器自动化工具获取渲染后的 HTML。

**如何爬取需要登录的页面？**

先通过 Jsoup 发送登录请求获取 Cookie，然后在后续请求中携带 Cookie：`.cookie("sessionid", "xxx")`。
