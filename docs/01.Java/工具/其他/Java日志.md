...
![](https://raw.githubusercontent.com/dunwu/images/master/cs/java/javalib/log/log4j2-architecture.jpg)
...
![](https://raw.githubusercontent.com/dunwu/images/master/cs/java/javalib/log/slf4j-to-other-log.png)
...
![](https://raw.githubusercontent.com/dunwu/images/master/cs/java/javalib/log/logback-configuration.png)
...
![](https://raw.githubusercontent.com/dunwu/images/master/cs/java/javalib/log/logback-appender.png)
...
- **业务操作日志**：记录用户登录、订单创建、支付等关键业务操作，便于审计和问题排查。
- **异常监控告警**：通过 ERROR 级别日志记录异常堆栈，结合 ELK/Loki 等日志聚合平台实现实时监控和告警。
- **性能分析**：在关键方法入口/出口记录耗时日志，分析接口响应时间和慢查询。
- **链路追踪**：结合 MDC（Mapped Diagnostic Context）注入 traceId，在分布式场景下串联完整调用链路。

## 最佳实践

- **统一使用 SLF4J 门面**：代码中只使用 `slf4j-api`，通过桥接包统一到底层实现（Logback/Log4j2），避免多日志框架冲突。
- **使用占位符而非字符串拼接**：`log.debug("id: {}, name: {}", id, name)` 而不是 `log.debug("id: " + id)`，避免无效拼接开销。
- **按级别分文件输出**：配置 ERROR/WARN/INFO 分别输出到不同文件，便于快速定位问题，避免在海量日志中查找。
- **日志文件滚动策略**：配置按时间+大小的滚动策略，设置保留天数，避免磁盘空间被日志占满。

## 常见问题

**多个日志框架冲突，日志重复输出？**

检查 classpath 中是否有多个日志实现（log4j + logback），使用 `mvn dependency:tree` 排查，通过 `<exclusion>` 排除多余的日志实现，保留一个底层实现 + SLF4J 桥接包。

**Log4j2 和 Logback 如何选择？**

Log4j2 在异步日志场景下性能更优（10 倍+吞吐量），且支持插件扩展。Logback 配置更简单，SpringBoot 默认集成。对性能要求高的场景选 Log4j2，一般场景选 Logback。

**日志级别设置多少合适？**

生产环境建议 INFO 级别，需要调试时动态调整为 DEBUG。开发环境用 DEBUG。避免在生产环境使用 TRACE，会产生大量日志影响性能。
