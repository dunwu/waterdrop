...
| Java 数据类型 | 自动装箱拆箱有哪些陷阱？ | 中等 | ⭐⭐ | ✅ | L2 | 答出装箱机制、Long 缓存范围 [-128,127]、对象头 24 字节计算正确、AutoBoxCacheMax 仅对 Integer 生效；但内存估算停留在稳态常驻，未答出分配速率与 YGC 频率的关联，JFR/MAT 实操不熟。补强重点：① 高并发下对象分配速率估算（QPS × 单对象字节数，关注存活时间而非常驻）② 线上实时定位分配热点工具（JFR Allocation Profiler）③ MAT 从对象关联到业务代码的具体视图操作 |
...
| Map | HashMap 底层实现原理是什么？ | 困难 | ⭐⭐⭐⭐⭐ | ✅ | L2 | tableSizeFor 经引导后完整推算出位运算过程及结果 8，具备位运算基础。但本题出题方式有失当——tableSizeFor 属纯实现细节，考的是记忆力而非穿透力，与面试官提示词"不是知道，而是设计过、踩过坑"的定位不符。候选人实际能力应通过场景化问题（如扩容 rehash 的 CPU 尖刺、高并发下 HashMap 失效场景）重新评估。补强方向：HashMap 扩容机制（JDK 8 高低位拆分 (e.hash & oldCap)==0）、并发失效场景（并发 put 导致数据丢失/死循环） |
| Map | JDK 1.8 对 HashMap 做了哪些改动？ | 困难 | ⭐⭐⭐⭐ | ✅ | L2 | 答出树化阈值为 8、红黑树 O(logN) 与链表 O(N) 的查找效率权衡、8 以下树化成本不划算。符合 L2"读过源码、知道树化"标准。本题未覆盖 L3 必须命中项（ArrayList 扩容代价 / HashMap rehash 高低位拆分 / 大数据量选型），非降级而是考察面未触及。补强方向：HashMap 源码注释中的泊松分布概率分析（P(k=8)≈0.00000006），理解树化是兜底机制而非常态路径 |
